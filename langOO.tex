...
Class descriptions consist of field declarations, method declarations, and ghost   declarations.
...
\\
\\
\inferenceruleNN {Field\_Heap} {
 \M,\,  \sigma, \SE \ \hookrightarrow\   \alpha \ \hspace{2cm} \sigma(\alpha,\f)=v
}
{
...
\\
\\
\inferenceruleNN {Field\_Ghost}
{
 \M,\, \sigma, \SE_0 \lp \rp \ \hookrightarrow\   \alpha
\\
 \M,\, \sigma, \SE_i \lp \rp \ \hookrightarrow\   v_i\ \ \ \ i\in\{1..n\}
 \\
{\mathcal{G}}host
(\M, {\ClassOf {\alpha} {\sigma}}, {\f}) \  =  
\ \f\lp \prg{par}_1, \ldots \prg{par}_n \rp \lb\ \SE \ \rb
  \\
  \M,\, \sigma[\prg{par}_1\mapsto v_1, .... \prg{par}_n\mapsto v_n], \SE    \hookrightarrow\   v 
}
{
 \M,\,  \sigma, \ \SE_0.\f \lp \SE_1,....\SE_n\rp \hookrightarrow  \ v
}
\\ \\
\inferenceruleNN {If_True} 
{
 \M,\,  \sigma, \SE \ \hookrightarrow\   \prg{true}  \hspace{2cm}   \M,\,  \sigma, \SE_1 \ \hookrightarrow\   v
}
{
 \M,\, \sigma, \kwN{if}\ \SE\  \kwN{then} \ \SE_1 \ \kwN{else} \ \SE_2\  \hookrightarrow\ v
}
\\ \\ 
\inferenceruleNN {If\_False} 
{
 \M,\,  \sigma, \SE \ \hookrightarrow\   \prg{false}  \hspace{2cm}   \M,\,  \sigma, \SE_2 \ \hookrightarrow\   v
}
{
 \M,\, \sigma, \kwN{if}\ \SE\  \kwN{then} \ \SE_1 \ \kwN{else} \ \SE_2\  \hookrightarrow\  v
}
\\ \\ 
\inferenceruleNN {Equals\_True} 
{
 \M,\,  \sigma, \SE_1 \ \hookrightarrow\  v  \hspace{2cm}   \M,\,  \sigma, \SE_2 \ \hookrightarrow\   v
}
{
 \M,\, \sigma, \SE_1 =  \SE_2 \hookrightarrow\ \prg{true}
}
\\ \\
\inferenceruleNN {Equals\_False} 
{
 \M,\,  \sigma, \SE_1 \ \hookrightarrow\  v  \hspace{2cm}   \M,\,  \sigma, \SE_2 \ \hookrightarrow\   v'  \hspace{2cm}  v\neq v'
}
{
...
\end{array}
$
\caption{The Value of Simple Expressions, where we take $\SE.\f$  as a shorthand for $\SE.\f\lp\ \rp$}
