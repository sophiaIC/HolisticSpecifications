...
%components 
\sd{the code} \emph{can} do --- the \emph{sufficient} conditions to
...
% \sophia{used to say  "invoke that operation" I think it is better to use one term.}
  \sd{and obtain the associated effect.}
...
\sd{are concerned  with the \emph{overall} behaviour of a component,  in all possible 
...
what are the conditions \sd{without which} an effect will not happen. }
By adopting holistic specification techniques,
programmers can explicitly define what their components should \sd{not} do,
making it easier to write
%  \sd{code}
% that support the construction of 
\sd{robust and reliable programs.}

\sd{In this paper we argue the need for such holistic specifications,
 propose a specification language for such holistic specifications, give a formal semantics and discuss several
...
literature where an holistic specification would provide obvious
