...
\jm[]{In Sec. \ref{s:outline} we used proof of assertion encapsulation
in our proof of adherence to \Nec specifications. In this section we 
define the semantics of assertion encapsulation and discuss the requirements 
for a proof system for assertion encapsulation. We do not define an encapsulation 
proof system as encapsulation is not the central focus of this paper, and \Nec logic 
is parametric on such a system. We do however informally discuss a rudimentary 
encapsulation proof system that we use to prove a practical example adheres to its \Nec specification.
}
...
\jm[]{For the purposes of proving examples, we define a rudimentary system for proving encapsulation in Apdx. \ref{s:encap-proof}, Fig. \ref{f:asrt-encap}.}
...
the encapsulation of assertions containing access, most importantly 
...
are never returned from method bodies \cite{confined}. 
%% \susan[I would omit this]{Again, we omit the detailed description of this
%% simple type system.
%% }
%%%Killed by KJX beccause **we don't have the f..king type system!**
}
...
The full rules can be found in Apdx. \ref{a:necSpec}, and are not presented here 
...
This however would not be sound as in general a \jm[used to say: ``transition from
