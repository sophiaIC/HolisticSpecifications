...
\subsection{\jm[]{Part 3:} \Nec Specifications for Single Steps}
...
\end{mathpar}
\caption{Single-Step \Nec Specifications, $OnlyIf\_1$}
...
%
\subsection{\jm[]{Part 4:} \Nec Specifications for Emergent Behaviour}
\label{s:emergent-proof}

The final step is to raise per-step \Nec specifications to 
...
a sound encapsulation inference system, $\proves{M}{\givenA{A}{\encaps{A'}}}$,
 ...
 the \Nec logic according to the rules in Figures  \ref{f:classical->singlestep},  and \ref{f:only-if-single}, and \ref{f:only-if},  and \ref{f:only-through},   then, for    all modules $M$, and all \Nec specifications  $H$:
