...
As already stated in Sec. \ref{s:outline}, the first step in proving adherence to a \Nec specification
...
\jm[]{We define a rudimentary system for proving encapsulation in Apdx. \ref{s:encap-proof}, Fig. \ref{f:asrt-encap}.}
...
that $\wrapped{x}$ is always encapsulated. Thus, if only internal
objects have access to $x$, then internal computation is required in order
to gain access to it. The full description of this system is given in
