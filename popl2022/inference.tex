\jm[fixed position of section header]{
...
\jm[]{does not rely on the specifics of an encapsulation algorithm, only it's soundness.}
...
\jm[]{That is, for some method $m$ in class $C$, we construct a necessity specification of the form:
...
We provide this subset in \jm[fixed figure reference. used to be 2]{Fig. \ref{f:classical-syntax}}. That is, classical assertions
...
This implies that we can also have \jm[]{guarantees} of  
...
These are rules whose conclusion have the  form Single-Step Only \jm[]{If}. 
...
This rule is sound, 
\jm[I think the best way to enforce this is to assume that this has been checked by the type system]
...
\end{figure}
\jm[fixed problem with \textsc{If1-$\wedge$I}]{}
...
\jm[]{(a) \textsc{If1-$\vee$I1} states that any execution
...
\jm[]{it} states that if the satisfaction of some assertion changes over time, 
...
proceed from intermediate states are themselves \jm[]{necessary} intermediate states. 
