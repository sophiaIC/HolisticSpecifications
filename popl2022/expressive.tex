...
\jm[]{\subsubsection{Safe}
\cite{FASE} used an unrealistic but demonstrative running example of a Safe, where a treasure 
was secured within a \texttt{Safe} object, and access to the treasure was only granted by 
providing the correct password. The code snippet below provides the original definition of \texttt{Safe}
from \cite{FASE}.}
...
\end{lstlisting}
\jm[]{To secure the \texttt{Safe} against improper leaking of either the password and treasure,
\cite{FASE} require that a necessary precondition to the treasure ever being removed from the safe 
is current knowledge of the password by some external object. Unfortunately the above definition 
does not satisfy this property, as it is possible for the secret of one \texttt{Safe} to be hidden
within another \texttt{Safe}, whose secret is externally known. One fix to this is by using types to 
...
\end{lstlisting}
\jm[]{Using \Nec, we express \texttt{SafeSpec}, that ensures the safety of the version of \texttt{Safe} with types.}
\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
