...
We now give a sketch of \jm[]{how the novel concepts we developed in  \Nec logic, 
and mentioned in \S \ref{intro:this:work} interact with each other to allow us to reason about 
necessary conditions and emergent behaviour. \Nec logic in this 
...
While this is how \Nec logic is structured, it is not necessarily how 
humans think about proofs. This section aims to provide a perspective of the 
structure of the logic.}
\jm[]{
\Nec logic consists of 4 components:
\begin{description}
\item[Part 1: Assertion Encapsulation.] 
We conceptually split the concept of \emph{Assertion Encapsulation} 
from the \emph{Per-Step Specifications} described in \S \ref{intro:this:work}
and the first part of a \Nec logic proof.
An assertion $A$  is
...
\includegraphics[width=0.9\textwidth]{diagrams/dependency.png}
   ...
   components that are not included in this paper.
    }
...
that is, the property that no object has access to the password may only 
...
% 
%
\item[Part 2: Per-method conditions]   \
%
\vertsp\\
{\fbox{\parbox{\linewidth}{
Each method within \ModC (\prg{set} and \prg{transfer}) is proven to have the following necessary conditions:
\begin{description}
\item[(D)] If the balance reduces, then the method called was
...
\item [(J)] 
By \textbf{(A)} we know that step must be an external call to \prg{transfer} with the correct password.
\item [(K)]
...
a call to \prg{set} with the current password. Thus, by \textbf{(H)} we know tha t
