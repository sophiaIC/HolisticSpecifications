...
Note that whether a module is external or internal only depends on how the modules  are
 ...
 $\reduction{\prg{M\_A}}{\prg{M\_B}}{...}{...}$ the external module is \prg{M\_A},
 while in  $\reduction{\prg{M\_B}}{\prg{M\_A}}{...}{...}$  the external module is \prg{M\_B}.}
 
  there is nothing inherent that makes a module internal or external, 
...
\begin{definition}
Assertions ($A$) \scd{in} % the basic specification language, 
...
\scd{We also use several shorthands:} % Definition \ref{def:chainmail-semantics} 
...
%\noindent
\scd{ $\satisfiesA{M}{\sigma}{\access{x}{y}}$ (\ref{cAccess})
...
arguments (case \ref{c2}).
 $\satisfiesA{M}{\sigma}{\calls{x}{y}{m}{z_1, \ldots, z_n}}$  (\ref{cCall}) 
...
a module $M$ satisfies an assertion $A$, if any state  $\sigma$
...
\begin{definition}[Assertion Encapsulation, Wrapping and Types]
...
We use ${\sigma' \triangleleft \sigma}$ to indicate the state as in $\sigma'$ but
with the variable bindings as in the top frame from $\sigma$ -- more in 
\sophiaPonder{TOSO ref}. 
For example, 
\\
\strut \hspace{1cm}
