...
\item[Part 1: Emergent behaviour] \jm[]{is behavior that arises out 
of any number of steps and interactions with a module interface.}
We infer the \emph{emergent} behavior out of the conditions of % many possible 
single steps.  
...
\jm[]{From the level of emergent behavior, our proof of (S3) follows the following steps:
\begin{description}
\item [(A)]
...
\end{description}
\end{description}
Of the above three proof steps, (\textbf{B}) and (\textbf{C2}) require us to delve deeper  and reason about
the necessary conditions for state to change due to a single-step, and further, what state requires internal computation to change.}
...
\jm[]{
Our proof of \textbf{(C2)} also considers single-step conditions, and takes the following form:
\begin{description}
\item[(G)]
...
For our proof, we establish  for each method of \prg{Mod3} (\ie each method in \prg{Account} and \prg{Password}) that if the method is called, then 
 \textbf{(E)},  if it  causes the  balance to reduce, then the method called was
 \prg{transfer} and the correct password was provided, and 
  \textbf{(H)} it will not provide external accessibility to the password.
...
by \prg{Mod3}, and  \textbf{(G)},   external accessibility to an account's password may 
