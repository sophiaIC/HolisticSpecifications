...
\label{s:outline}
We now give an outline of our approach using the bank account example,   where funds may be transferred between accounts only when sending the account's password.


 \subsection{Bank Account -- three modules}
\label{s:bank}
  
Module \prg{Mod1}   consists   classes  \prg{Password} and  \prg{Account}
...
through a ``classical''
specification  in terms of pre- and post-conditions ---
...
The critical point of this \NecessitySpecification is that it is
...
This gives \NecessitySpecifications the
...
%Moreover, 
adherence to   \NecessitySpecifications is not monotonic:
Adding a method to a module does not necessarily preserve adherence to a \NecessitySpecification.
And while separate methods may adhere to a  \NecessitySpecification, their combination does
not necessarily do so. Thus, \NecessitySpecifications capture a module's \emph{emergent behaviour}. 
}
...
only be obtained through  {Mod3}-internal calls -- that is, if the property that
no external object has access to the password can only be invalidated through an internal call.
 
\item[Part2] We establish  for each method of \prg{Mod3} (i.e. each method in \prg{Account} and \prg{Password}) that if the method is called, then 
...
number of steps, then at some intermediate step  \prg{transfer} was called with   the correct password.
...
From \textbf{(f)} we obtain that  \textbf{(i)}  external accessibility to the password will not be provided by any sequence of step.
Using  \textbf{(h)} and  \textbf{(i)} we obtain    we establish that
...
We addressed that   challenge % of the inference of necessary conditions 
...
\item[Part3]
Here we infer   \emph{per-step-condtions}, i.e., a 
 necessary condition given an effect and
...
For $M$-encapsulated effects, we can infer such one-step
