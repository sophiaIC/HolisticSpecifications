...
$M$ is programmed in such a robust manner\sophia[We had agreed no to say "robust", but I think it is OK here. Alternatively, say defensively robust?]{} that execution of $M$ together with \emph{any} \externalM 
...
Moreover, because we only require that the guarantees are upheld while 
  the  \externalM module  is executing, we develop an \emph{external steps} semantics, \sophia{I like the term} where
...
We  now outline the proof that version III of \prg{Account} adheres to $(HolisticSpec)$,
