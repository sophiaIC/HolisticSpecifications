...
Moreover, because we only require that the guarantees are upheld while 
  the  \externalM module  is executing, we develop an \emph{external \jm[]{states}} semantics, \sophia{I like the term} where
...
We  now outline the proof that \prg{Mod3} adheres to \jm[]{$(\prg{NecessityBankSpec})$},
...
\jm[push the module through this section. eg. ``Account internal'' -> ``Mod3 Internal'']{}
\begin{description}
\item[P1:] We establish that the balance 
 may change only   through \jm[]{\prg{Mod3}}-\internalC calls.  

\item[P2:] 
For each method of \jm[]{\prg{Mod3} (i.e. methods of \prg{Account} and \prg{Password})},  we establish that   
...
that  \jm[]{\prg{Mod3}} satisfies \jm[]{$(NecessityBankSpec)$}
...
%
In \textbf{P1}, the assertion \prg{a:Account\! $\wedge$\! a.balance=bal} is encapsulated by \prg{Account}.
...
combined with any internal module. This step is crucial; namely, remember that while \jm[]{\prg{Mod2}} adheres to
the guarantee from the \textbf{P3}, it does not adhere to  $(NecessitySpec)$.   
Our Logic of Necessity allows us to combine  several such one-step necessary conditions, and obtain a several-step \jm[]{necessity specification}.
