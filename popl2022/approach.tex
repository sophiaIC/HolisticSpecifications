...
With external steps semantics,  the executing object (\prg{this}) is always   external. 
  ...
  -- \sd{further details on call-backs in \S\ref{s:related}. }
...
\sd{We will now outline the key ingredients of  our logic with which we prove
that modules obey \Nec specifications. 
% In this logic, 
We will use the auxiliary concept that % of \emph{assertion encapsulation}: 
...
The \Nec logic is based on the crucial insight that the specification\\
...
\\
 ...
 Three breakthroughs helped us address   that challenge:}
...
\sophiaPonder[read "particular encapsulation
mechanism" -- I think it is wrong]{the way we ascertain whether an assertion
...
\sd{or even develop custom  logics.}
