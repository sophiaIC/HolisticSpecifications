...
\begin{description}
\item [(a)]
\jm[]{a logic for raising assertion encapsulation and classical Hoare specifications
to necessity specifications for any single module interaction (\S\ref{s:module-proof});}
\item [(b)]
\jm[]{a logic for raising single step necessity specifications for a module to an arbitrary number of steps
...
\end{description}
\item
  a mechanised \jm[]{Coq} proof of soundness of the logic (\S\ref{s:soundness});
\item
  a \jm[]{mechanised Coq} proof that the bank account example obeys the necessity
