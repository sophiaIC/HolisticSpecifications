...
In this Section we outline our \sd{approach}:  
we  revisit  our running example, the Bank Account (\S\ref{s:bank}),  
\sd{introduce the three necessity operators (\S \ref{s:approach:necopers})},  \sd{give 
the \Nec specs} (\S \ref{s:bankSpecEx}),
   outline how we model the open world (\S\ref{s:concepts}), 
and outline the structure of our proof system and % outline 
how we reason 
...
\sd{We need  a specification that rules out \ModB while permitting \ModA and
...
1) having \prg{access} to an object ($\access{\prg{o}}{\prg{o'}}$) which means that $\prg{o}$ has a reference to $\prg{o'}$,
 or 2) \prg{calling} a method with on a receiver with certain arguments, 
...
We now \sd{go back to our quest for} a specification that rules out \ModB while permitting \ModA and
...
In \Nec  syntax, and \sd{recalling \S \ref{intro:this:work}, and \ref{s:approach:necopers},}
...
%% T9hu having access to the password is a necessary condition for the balance to reduce.
 ...
 \sd{\SrobustA does not fit the bill: all three modules   satisfy  it.
...
\subsubsection{How \sd{useful} is  \SrobustB?}
\label{sec:how}

\sd{One might think that \SrobustB was not useful: Normally, there will exist somewhere in the heap
...
\sd{The method \prg{cautious} has as  argument an external object \prg{untrusted}, of unknown provenance.
...
thus,  during execution of  line 7, or   line \sd{9} the balance may decrease.
}

\sd{Assume that class \prg{Account} is from a module which satisfies \SrobustB. 
Assume also that the code in line \sd{7} does not leak the password to \prg{untrusted}. Then no external object
...
between \emph{\internalC} calls  (from either an internal or an external object)  made % from \externalO objects
 ...
 \sd{Looking at the code snippet from \S \ref{sec:how}, the call to \prg{set} on line 6 is an 
...
Note that we \sd{currently} do not support calls from
\internalO objects to \externalO objects.

\sd{For the purposes of the current work we are only interested in one internal, and one external module.
But the interested reader might ask: what if there are more than one external modules?
The answer is that from the point of view of the internal module, all external modules 
are considered as one through
a module linking operator with  the expected semantics -- full details in  \S \ref{app:loo}. 
But from the external module's viewpoint, there may be more than one external module; for example, in \S \ref{sec:how}, 
module $\ModParam{1}$ in external to the module   implementing class \prg{Account}, and the module 
...
\includegraphics[width=\textwidth]{diagrams/dependency.png}
   \caption{
   Parts of \Nec Logic \sd{and their Dependencies}. 
...
\begin{description}
\item[(D)] If the balance \sd{decreases}, then 
