...
In \Nec  syntax:\sophiaPonder[we are repeating something from section 1]{}
...
For illustration, we  outline  a proof that  \prg{Mod3} adheres to (S$_{\prg{robust}_2}$).
...
single steps.
This part is crucial;   \sd{for instance,} while \prg{Mod2} satisfies  
(S$_{\prg{robust}_2}$) for one single step, it does not satisfy it for any number of steps. More in \S\ref{s:emergent-proof}.

\vertsp
%\jm{e.g. In the case of (S3) and \prg{Mod2}, emergent behavior refers to the interaction 
%the \prg{set} method (for setting the password), the \prg{transfer} method, and the account's balance.}

\jm[]{\fbox{\parbox{\linewidth}{
Our proof of \sd{\prg{Mod3}'s adherence to (S$_{\prg{robust}_2}$)} follows the following steps:
\begin{description}
\item [(A)]
...
\begin{description}
\item[(E)] Each method of \prg{Mod3} (\ie each method in \prg{Account} and \prg{Password}),
if it  causes the  balance to reduce, then the method called was
 \prg{transfer} and the correct password was provided
\item[(H)] Each method of \prg{Mod3} will not provide external accessibility to the password.
\end{description}
}}}
...
only be obtained through calls to \prg{Mod3} -- that is,  the property that
