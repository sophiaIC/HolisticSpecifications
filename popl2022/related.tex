...
\sophiaPonder[chopped: Both Miller and
...
%
%
\sophiaPonder[Chopped: "\Nec specifications embody a similar encapsulation
...
states semantics." it needs more explanation]{}
...
\sophiaPonder[choped "and sums can be computed over collections," do not think is crucial]{
...
Like \Nec, \sd{{{\sc{VerX}} \cite{VerX} and  \emph{Chainmail} \cite{FASE} also work on problem-specific guarantees.}
...
\langle {\color{blue}\texttt{next}}\, \prg{a.balance<bal} \, \rangle $\\
 ...
 \\
% all the rest unnecessary at the end of paper\sophiaPonder[expalined and used similar syntax to below, and explained. But  now it get too too long :-(]{That is, if in the next state the balance of \prg{a} decreases, then the current state is 
...
\sd{Moreover, temporal operators in {\sc{VerX}}   and  \emph{Chainmail}  are first class, \ie may appear in any assertions 
...
would subvert \Nec, % and introduce several instances of unsoundness, 
\sophiaPonder[chopped]{in} particular a reference to $x$ could always be guessed
% $\wrapped{x}$ 
% it would not require interaction with internal code in order to 
%gain access to 
thus the assertion $\wrapped{x}$ \sd{would no longer be encapsulated}.
...
\sd{This would be an adaptation of   \citeauthor{BraemEilersMuellerSierraSummers21}'s approach,
...
%but rather on parts of method bodies. 
One \sophiaPonder[chopped: approach that would be a useful
simplification would be to use the simplification that]{} \sd{useful simplification was proposed by 
 \citeasnoun{Permenev}:} in
