...
\cite{gordonJefferyRobustSafety} use a type system to verify correspondence 
assertions for security protocols in the language Spi. While both their domain 
and approach are tangential to that of \Chainmail, they introduce a safety 
...
the presence of ``an arbitrary hostile opponent``. Robust safety has subsequently 
become a common statement of safety in the open world in the security literature \cite{ddd, Bugliesi:resource-aware}.
}
...
proof techniques for proving a compiler preserves robust safety. Both the work of 
\cite{abate2019journey} and \cite{robustSafetyPatrignani} are orthogonal to 
that of this paper, however they are related. \Chainmail is concerned with 
...
offer strong starting point for future work in the implementation of 
\Chainmail as a tool.
}

\jm[]{
\cite{vanproving} show how capability machines can be used to 
enable formal verification of a full system with a complex notion 
of untrusted code, allowing different nested layers to establish different 
security properties. The authors 
...
\cite{vanproving} tackle a different problem to that of \Chainmail in this paper. 
...
\Chainmail on the other hand is intended to be agnostic of the low-level detail,
...
By enforcing encapsulation, \Chainmail{all} % used to say both
...
%\textbf{(ideally also say something about proof status)}}
  ...
  \paragraph{Incorrectness Logic.} \cite{IncorrectnessLogic,IncorrectSeparation},
  based on a Reverse Hoare Logic \cite{reverseHoare}, 
can specify preconditions under which program states will result. 
While the classical Hoare triple $\{ P \}\, C\, \{ Q \}$ expresses that starting 
at a state satisfying $P$ and executing   $C$ is guaranteed to reach only states that satisfy $ Q $,
the incorrectness triple $[ P_i ]\, C_i\, [ Q _i ]$ expresses that starting at a
 state satisfying $P_i$ and executing  $C_i$ is guaranteed to reach 
...
Therefore, both classical triples and incompleteness triples are about sufficient conditions,
and the difference is whether they consider $Q$ in a sound or a complete way.
The fact that  $[ P_i ]\, C_i\, [ Q _i ]$ gives sufficient conditions for reaching all of $Q_i$ 
makes these triples well-suited for finding bugs. However,
$P_i$ and $C_i$ are not necessary fore reaching $Q_i$, and therefore 
eliminating the $P_i/C_i$ case from the code cannot guarantee the absence of $Q_i$
bug. Neither Incorrectness Logic, 
nor Hoare Logic are concerned with \NecessitySpecifications.
