...
%% \sd{\SpecO assertions are} guarantees upheld throughout program execution. 
...
%% \SpecO\ specifications can cross-cut the code they are
...
%% % traditional object-oriented JML assertions. In contrast, \SpecO\
...
%% %\SpecO\ is not as flexible as AspectJML. 
...
%% %% % SD removed the below, because I do not think it is important
%% %  AspectLTL
%% %has a formal definition, as does \SpecO; unlike \SpecO,
...
%% (e.g.\ the subject must notify its observers). \SpecO's control
%% %SD: not "control flow"
...
Like \SpecO, Chainmail is able to express specifications of \emph{access}, \emph{provenance}, and \emph{control}. Unlike \SpecO, Chainmail  has a rich set of temporal operators and  includes assertions about \emph{space}. However we 
present a considerably simpler semantics, that does not rely on significant variable renaming,
or restricting execution to the top frame of the stack. 
...
\cite{swapsies}. 
% In contrast, our work focuses on the semantics of the  \SpecO\ specification
% language and how it can be used to provide holistic specifications for
% robust programs.
%\sd{Their work 
%does not support the concepts of control, time, or space, as in \SpecO, but it 
...
More recently, \cite{ddd}  designed OCPL, a logic
...
\ \cite{schaeferCbC} have recently
...
encapsulation boundaries.  \cite{Banerjee:2005} demonstrated that by
ensuring confinement, ownership
systems can enforce representation independence.
%(a property close to ``robust safety'') some time ago.

 
\susan[]{This paragraph needs a rewrite.}\SpecO\ differs from Swasey, Schaefer's, and Devriese's work in a number of ways:
...
while \SpecO users only need  to understand  first order logic and 
the holistic operators presented in this paper.}
% While \SpecO's $\Using{}{}$ is related to Banerjee
...
operations that are at the core of \SpecO's approach.
}
...
%Similar to \SpecO, VerX has a specification language based on temporal logic.  
%VerX offers temporal operators for execution in the past
%(always, once, prev) but only within a past modality, while \SpecO\ has two temporal operators, both existential, but with both past and future modalities.   
%and 
VerX includes  temporal operators, predicates that
model the current invocation on a contract (similar to \SpecO's
``calls''), access to variables
and  sums (only) can be computed over collections. 
\SpecO\ is strictly more expressive as a
...
specifications for permission (``access''), %space (``in'') 
and viewpoint (``external'') which have no analogues in VerX. 
Unlike \SpecO, VerX includes a practical tool that has
...
\susan[Should this be with the behavioural spec subsection]{}
%\jm{
\paragraph{Incorrectness Logic.} \cite{IncorrectnessLogic} defined a Hoare
...
In comparison, \SpecO, as with Hoare Logic, is concerned with correctness
(as seen in the exemplars of Section \ref{s:examples}). 
Extending the comparison, \SpecO differs from both Hoare Logic and Incorrectness,
