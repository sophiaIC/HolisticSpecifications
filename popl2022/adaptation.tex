...
\extrasmall{\prg{set}$\mapsto$\prg{a2}}\\
\extrasmall{\prg{x}$\mapsto$\prg{5}}\\
\end{tabular}
&
\extrasmall{\prg{y:=set.add(x);...}}
\end{tabular}
\endgroup
\end{lrbox}
...
&
\begin{tabular}{l}
\extrasmall{\prg{this}$\mapsto$\prg{a2}} \\
\extrasmall{\prg{y}$\mapsto$\prg{5}}
\end{tabular}
&
\extrasmall{\prg{if(this.contains(y)) ...}}
\end{tabular}
\endgroup
\end{lrbox}
...
\begin{tikzpicture}[->,>=to,shorten >=1pt,auto,node distance=7mm,
					decoration = snake]
\initialStateNode{\tiny{$\satisfies{\prg{MyMdl}, \sigma}{\access{\prg{this}}{\prg{set}}}$}}{A}{\usebox\stateA}; 
%\etcNode{blahA}{A};
\stateNode{\tiny{$\notsatisfies{\prg{MyMdl}, \sigma'}{\access{\prg{this}}{\prg{set}}}$}}{B}{right = of A}{\usebox\stateB};
\stateNode{\tiny{$\satisfies{\prg{MyMdl}, \adapt{\sigma'}{\sigma}}{\access{\prg{this}}{\prg{set}}}$}}{adapted}{below = of B}{\usebox\AadaptB};
	\draw [decorate, ->]
		(A) -- (B);
%	\draw [decorate, ->]
%		(blahA) -- (B);
