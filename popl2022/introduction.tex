...
\sd{However, (S$_{\text{correct}}$) does not make the   bank module 
...
Condition   
(S$_{\text{robust}_1}$) is \sd{crucial for robustness, but it is not strong enough:
...
%While crucial, (S$_{\text{robust}_1}$) is not strong enough to ensure robustness of 
 % an account.
\sd{Namely,} (S$_{\text{robust}_1}$) does not take  account of the module's \emph{emergent behavior}.
...
{ %For robustness, 
 ...
 \begin{quote}
(S$_{\text{robust}_2}$)\  \ The balance of an account does not \jm[]{\emph{ever}} decrease in the future unless some external 
object \jm[]{\emph{now}} has access to the account's current password.
\end{quote}
\jm{(S$_{\text{robust}_2}$) is a much stronger guarantee than (S$_{\text{robust}_1}$).}\sophiaPonder[mathematically speaking, it is not!]{}
