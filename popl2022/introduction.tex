...
{Software needs} to be both {\emph{correct}} ({programs do what they
  are supposed to}) and {\emph{robust}} ({programs \emph{only} do what
  they are supposed to}). {We use robustness to refer
...
%they will not  {be able to} steal my money 
 {that my money will not be stolen} \cite{ooToSecurity,miller-esop2013}. 
...
More \sophiaPonder[chopped "discussion"]{}   in \S\ref{s:bankSpecEx}. 
\sophiaPonder[moved: We also support two further ...]{}
%\Nec operators:
%\\
%$ 
...
Unlike  \emph{Chainmail}'s temporal operators, 
 ...
 \vspace{.1in}
\sd{The second main contribution is  a logic that would enable us to prove that code 
...
\sophiaPonder[deleted: We require  \funcSpecs to have explicit framing.]{}
Moreover, in line  with other work in the literature \cite{Permenev, Grossman, Albert}, we forbid 
calls out to  { \color{blue}\texttt{external}} objects (ie unknown objects
whose code has not been checked). We discuss callbacks in further detail in \S\ref{s:related}. 
\sophiaPonder[moved: "While our work is based on  ..." to related work]{}
