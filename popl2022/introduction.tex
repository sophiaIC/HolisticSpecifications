...
For example, consider the \prg{Account1} class modeling a bank account
with a balance and a password, where funds may be transferred
between accounts only with the sending account's password:
...
\prg{Account2} allows any client to reset its password at any time, while
\prg{Account3} first requires the existing password in order to change it.
...
%
are enough to drain \prg{Account2} without supplying the password.
...
We need to rule out \prg{Account2} while permitting \prg{Account3} and
\prg{Account1}. The catch is the leak in \prg{Account2} is the result
of  \emph{emergent} behaviour from the interactions of the \prg{set}
and \prg{transfer} methods --- even though \prg{Account3} also has a
