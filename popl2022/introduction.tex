...
%Condition  -- dropped the word, so as to fill in more; SD
...
that a malicious client might instead empty the balance of the account \cite{ELang,miller-esop2013}.
...
Hoare triples, for ``bad things  not
happening'', a multitude of different guarantees have been proposed, differing in the level 
of granularity, the target programming language or calculi, intended use, and the ways by which 
...
Recently,  {\sc{VerX}}  \cite{VerX}  and   \emph{Chainmail}  \cite{FASE} 
 ...
 and in addition, about \emph{access} (whether an
...
\paragraph{What we \emph{really} need} However, while (**) is a necessary condition  to confidently pass 
 ...
 (**) does not take  account of the module's \emph{emergent behaviour}.
...
The work in this paper is the first to express necessity specifications, including those like (***), and 
also proove adherence to such specifications.
...
This example demonstrates, among other things, that 
%the specification need  refer  to  individual methods in a module.
%Moreover, 
adherence to a Necessity Specification is not monotonic, in the following two senses:
Adding a method to a module does not necessarily preserve adherence to a Necessity Specification,
and while separate methods may adhere to a  Necessity Specification, their combination does
not necessarily do so: Necessity Specifications capture a module's emergent behaviour.
