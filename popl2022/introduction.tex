...
\jm[]{
Note, having \prg{access} to an object refers to having a reference to that object. This is one of the capability operators that we have 
...
Unlike  \emph{Chainmail}'s temporal operators, 
 ...
 are \jm[]{not first} class, and may not appear in the assertions  {(\eg  ${A_{curr}}$)}. 
...
\jm[]{{{Moreover, in line} with other work in the literature\jm[]{\cite{Permenev, Grossman, Albert}},} we forbid 
``callbacks'' out to  { \color{blue}\texttt{external}} objects (ie unknown objects
whose code has not been checked). We discuss callbacks in further detail in \S\ref{s:related}.}  While our work is based on 
