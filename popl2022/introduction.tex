...
{Software needs} to be both {\emph{correct}} ({programs do what they
 ...
 \sd{We use the term \emph{robust} as a generalisation of \emph{robust safety}~\cite{gordonJefferyRobustSafety, Bugliesi:resource-aware,ddd}  whereby a module or process or ADT is \emph{robustly safe} if its execution preserves some safety guarantees even when run together with unknown,   unverified, potentially malicious client code. 
...
%While 
\Scorrect  describes  {the \emph{correct use} of the  \sophiaPonder[we said "the module"
but I think tis is not what we meant]{particular function}, but is \emph{not} concerned with the \sd{module's} \emph{robustness}.}
...
\sd{without calling \prg{transfer} and} without providing the password.
  Calling \prg{transfer}   with the  correct password is 
a \emph{necessary condition} for \sd{(the effect of)} reducing the account's  balance.
...
\vspace{.1in}
 {
The \sd{first main} contribution of \Nec\  
\sd{is} three novel operators that merge
...
Using this operator 
{and Chainmail's capability operators that we have adopted into 
\Nec}, 
...
Unlike  \emph{Chainmail}'s temporal operators, 
 ...
 obeys  specifications {talking of system-wide properties.}
\sd{The challenge is that our specifications are logically equivalent to the
intersection of an infinite number of Hoare triples, \emph{i.e.,} 
$\onlyIf{A_1} {A_2} {A_3}$ is 
logically equivalent
% with 
{to}
 $\forall \prg{stmts}. \{ A_1 \wedge \neg A_3\} \prg{stmts} \{ \neg A_2 \}$.
 And there do not exist logics to reason about such infinite intersections.}

\sd{We addressed the challenge} through three observations: (1) proofs of \Nec specifications
\sophiaPonder[chopped: " in the open world"]{} can be based around encapsulation and permission,
 (2) \Nec specifications of emergent behaviour can be built up from \Nec specifications on individual 
 functions, which (3) in turn can be \sd{obtained} from \emph{traditional} \funcSpecs and encapsulation.
 %
...
\sd{and offer several off-the-shelf solutions.}
\sophiaPonder[chopped about callbacks, and added to section 2.4]{}
% In line  with other work in the literature \cite{Permenev, Grossman, Albert}, we forbid 
% calls from internal to  { \color{blue}\texttt{external}} objects (ie unknown objects
% whose code has not been checked) -- \sd{further details on call-backs in \S\ref{s:related}. }
...
%
The contributions of this \sd{work} are:
\begin{enumerate}
...
A proof in our logic % the bank account 
  that our bank module \sd{obeys} % the \Nec specification 
