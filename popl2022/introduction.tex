...
% robust = The degree to which a system or component can function correctly in the presence 
% of invalid inputs or stressful environmental conditions.  
{Correctness is} \jm[not sure what to do here wrt macro]{classically} specified
%formally 
...
\jm[]{A proof logic that is able to express properties such as \SrobustA and \SrobustB must tread a fine line: temporal operators are needed along with operators 
that describe encapsulation and the shape of the heap, while also being simple enough that proof rules might be devised.}
...
\item
Some assertions are ``\emph{encapsulated}'' by a module, and require calls to that module to invalidate them.
\item
...
satisfaction \jm[]{and} encapsulation\jm[removed:, and the type system]{}.
We require  
