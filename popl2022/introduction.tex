...
\end{quote}
\sophiaPonder[]{Assuming termination,} the precondition is a \emph{sufficient} condition for the code snippet to
make the good thing happen: given  \sophiaPonder[should we drop termination the 2nd time?]{termination and} the precondition (eg providing the right 
...
guarantee  that
\begin{quote}
(**)\  \ The balance of an account does not decrease unless the \prg{transfer} function was called 
...
This work introduces \Nec, \ \sophiaPonder[]{which consists} of a  specification language for expressing a module's safety guarantees such as (***), and a logic \sophiaPonder[replaced its by a]{}
...
\sophiaPonder[shorter]{}
  capability operators, but simplified it, \sophiaPonder[]{by} removing temporal operators and introducing necessity operators. These simplifications enabled us to develop our proof logic. 
...
per-method
 ...
 emergent behaviour.
%\susan[Remove this as it is too similar to the next sentence.]{We outline the proof for a bank example in  \S\ref{s:approach}.}
\sophiaPonder[as per Susan]{More in \S\ref{s:approach}.}
