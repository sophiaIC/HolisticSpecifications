...
broadly paraphrasing \cite{Lamport77}
...
\emph{Good things happening} are typically specified formally 
 ...
 For example,
 \begin{quote}
(*)\ \ Calling the \prg{transfer} function using the correct password will transfer the money.
\end{quote}
...
Recently,  {\sc{VerX}}  \cite{VerX}  and   \emph{Chainmail}  \cite{FASE} 
 ...
 through the arguments to a function call),    about \emph{provenance} (whether an object
...
\emph{Chainmail}'s %approach to add
  ...
  simplifying it by removing temporal operators and introducing necessity operators. 
...
\sophiaPonder[]{operators. We now show one of these operartors} 
% shown below.
...
\end{lstlisting}
%  %      $A$          from ${A_{curr}}$ to ${A_{fut}}$ onlyIf ${A_{nec}}$          from ${A_{curr}}$ to ${A_{fut}}$ onlyThrough ${A_{nec}}$
The  form   \sophiaPonder[]{form above} says that % $A$ always holds;  $A$ is an invariant.
%The second form says that  
a change from a current state satisfying $A_{curr}$ to a future
...
or \emph{intermediate} conditions for \emph{any} transitions from a $A_{curr}$ -state t  $A_{fut}$ 
states.
}
