...
Specification \SrobustA gives   the guarantee that it is not possible to  take money out of the account through some method other than \prg{transfer} which did not
...
\\
 ...
 {\sc{VerX}} comes with a symbolic 
  execution system which can demonstrate adherence to  specifications, but doesn't have a proof logic, % to prove adherence,
...
%$\access{\texttt{o}}{\texttt{o'}}$).
 ...
 \susan[Probably the encapsulation statement should be the last introduced as a 'pruning'? technique.]{\Nec specifications are high level system wide properties. Our problem was how to develop a logic that would enable us to prove that code 
...
logically equivalent
 ...
 with an infinite number of Hoare triples.
We want to leverage prior work on program verification, however, Hoare logics only reason about given, known, statements.
 To solve that problem, we had three breakthroughs:
 \begin{itemize}
 \item
 First, we  leverage Hoare logic to reason about one particular method call. Namely, the \Nec specification
$ \onlyIfSingle {A_1 \neg A3 \wedge {\calls{\_}{\prg{x}}{\prg{m}}{\prg{ys}} } } {\ A_2} {A_3}$ is logically equivalent with the  Hoare triple 
$ \{ A_1 \wedge \neg A_3 \} \prg{x.m(ys)} \{ \neg A_2 \}$.
 \item Second, if an assertion $A_2$  is \emph{encapsulated} by a module (ie the only way we can go from a 
 state that satisfies it to a state that does not, is through a call to a method in that module), then
 $ \onlyIfSingle {A_1 \wedge  A_2} {\neg A_2} {A_3}$ is logically equivalent with the finite conjuction
 $ \onlyIfSingle {A_1 \wedge A_2 \wedge {\calls{\_}{\prg{x}}{\prg{m}}{\prg{ys}}}\  } {\neg A_2} {A_3}$
...
``callbacks'' out to external objects; also %and
