...
robust (``bad things will never happen'') % (safety), 
 ...
 \sophiaPonder[added parentheses and commas]{}
%\vspace{.03in}
...
\sophiaPonder[weakened]{Most} of the approaches outlined so far were not developed with
...
However, while guarantee (S2) is   necessary \sophiaPonder[dropped: condition]{}  to confidently pass 
 ...
 \sophiaPonder[drop: or what if the module allows me to override the
...
With (S3), I can confidently pass my account to some untrusted client who
 ...
 thus can be expressed without reference to the particular API. 
  ...
  Both {\sc{VerX}} and  \emph{Chainmail} can express (S2), but {\sc{VerX}}  cannot express (S3). 
  Namely, (S3)  requires the concepts of provenance and permission.
On the other hand, {\sc{VerX}} can prove adherence to  specifications through symbolic 
  execution, while  \emph{Chainmail}   lacks a proof system. 
   \sophiaPonder[after Sergio]{Neither can correspondence assertions express (S3): they lack
...
\emph{Chainmail}'s %approach to add
  ...
  simplifying it by removing temporal operators and introducing  {our novel}
