...
bad things guarded by that condition cannot happen. \susan [ I have a real problem with the last sentence. We don't have necessary pre-conditions for bad things in the paper - for instance we don't have a necessary condition ``Passwords can be changed by anyone.'' which we show cannot happen. I want to just remove the sentence.]{}
...
from an $A_1$-state to and $A_2$-state.\sophia[Shall we also talk of onlyIf1, and onlythrough? I vote no - Susan agree]{}

Our assertions $A$ allow for the usual logical connectives and quantifiers
 (e.g. $\wedge$, $\forall$), the usual assertions about the state (e.g. \prg{x.f > 3}), and 
...
%\footnote{Perhaps omit some of the lines here, but we do need them all in the full discussion}
 ...
 In version {\sf{II}} and version {\sf{III}} the \prg{Account} class has an additional method, \prg{set}, which enables the resetting of the password.
...
% the possible effects of the combinations of these actions. 
 ...
 \susan[]{I removed the number on the module name because it isn't referred to and doesn't need to be}
 \begin{figure}[hbtp]
 \begin{tabular}{lll}
