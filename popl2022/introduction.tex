...
In \jm{the \emph{open world},} 
%complex 
modules need to be both \jm[]{\emph{correct}} (\jm{programs do what they are supposed to}) and  % (liveness)  
\jm{\emph{robustly safe}} (\jm{programs don't do what they aren't supposed to, even in the presence of untrusted code}). % (safety), 
%\vspace{.03in}
...
\jm{Correctness is} typically specified formally 
 ...
 For example,
 \begin{quote}
(S1)\ \ Calling \prg{transfer} using the correct password will transfer the money.
\end{quote}
...
\begin{quote}
(S2)\ \ An account's balance does not decrease unless \prg{transfer} was called 
...
inforation control flow \cite{Zdancewic:Myers:01,noninteferenceOS}, 
...
While guarantee (S2) is necessary to confidently pass 
 an account into untrusted code, it is \emph{not} 
\jm[feels odd to use this word given the sufficient/necessary paradigm we set up]{sufficient} \jm{for ensuring robust safety of the bank module}.
...
With (S3), I can confidently pass my account to some untrusted client who
 ...
 balance, and a password.
  ...
  Both {\sc{VerX}} \cite{VerX} and  \emph{Chainmail} \cite{FASE} can express (S2) but
  {\sc{VerX}}  cannot express (S3), because 
...
This work introduces \Nec, which consists of a specification language for expressing a module's safety guarantees such as (S3), and a logic 
...
\jm{Further, beyond unforgeability and privacy, \Nec is agnostic about the specifics
of module implementation, only that module code can be verfied using some Hoare logic. This is a significant advantage, as it makes \Nec broadly applicable to any such languages that use Hoare logic to establish correctness.}
...
{We provide a description of the formalization of (S3) % in \Chainspec  
...
We will \jm{further} discuss these limitations further as we place \Nec into the context of 
