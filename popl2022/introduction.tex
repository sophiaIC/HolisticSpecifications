...
%complex 
{Software needs} to be both  {\emph{correct}} ({programs do what they are supposed to}) and  % (liveness)  
{\emph{robust}} ({programs don't do what they aren't supposed to, even in the presence of untrusted \sd{clients|}}). % (safety), 
%\vspace{.03in}
%\jm{or more specifically, modules need to be \emph{robust}(modules are safe, even in the presence of untrusted code).}
 ...
 postcondition. 
 For example, \sd{for a bank module,  we may specify for bank accounts that:}
 \begin{quote}
\Scorrect\ \ Calling \prg{transfer} using the correct password will transfer the money.
\end{quote}
...
However, \Scorrect does not make the   bank module 
 \emph{robust}; for example, what  
 if it were possible to \sd{take money out of the account through some otther method than \prg{transfer} which did not
 require knowledge of the passwoird?}
%  take ten percent of the  balance  from an account  through a further method defined in the module?
...
Condition   
\SrobustA is \sd{crucial for robustness, but it is not strong enough:
it does not give   enough guarantees in order to confidently pass the bank account into untrusted code, 
...
%While crucial, (S$_{\text{robust}_1}$) is not strong enough to ensure robustness of 
 % an account.
\sd{Namely,} \SrobustA does not take  account of the module's \emph{emergent behavior}.
 What if the module provided further methods which leaked the password,  
 {or allowed for it to be arbitrarily changed}?
 \jm{That is, the intent of \SrobustA (knowledge of the password is required to decrease the balance) may be 
 subverted by other procedures in the bank account interface.
 While no single procedure call is capable of breaking the intent of \SrobustA, a sequence of calls might.}
{ %For robustness, 
 \sophiaPonder[dropped "for robustness", as I think there are many kinds of robustness"]{What} we really need is
 \begin{quote}
\SrobustB\  \ The balance of an account does not \jm[]{\emph{ever}} decrease in the future unless some external 
object \jm[]{\emph{now}} has access to the account's current password.
\end{quote}
\jm{\SrobustB more accurately provides the guarantee needed to secure the bank account than \SrobustA does.}\sophiaPonder[mathematically speaking, it is not!]{}
With \SrobustB, I can confidently pass my account to some untrusted client who
 ...
 balance, and a password. \jm{Finally, \SrobustB takes into account 
...
of granularity,   target  language or calculi, and intended use. \sd{To name a few,}
...
\sd{Our work is related} to  {\sc{VerX}} \cite{VerX} and  \emph{Chainmail} \cite{FASE}.
Both these approaches can express necessary conditions
  like \SrobustA. For this, they \sd{both employ
...
\\
 ...
 \\
\sd{ However, to express \SrobustB, one also needs what we call \emph{capability operators}, which talk about 
...
\sd{Temporal operators in {\sc{VerX}}   and  \emph{Chainmail}  are fist class, ie may appear in any assertions 
...
\emph{Chainmail}'s %approach to add
  capability operators, but had a crucial insight: the flexibility afforded through the
  unrestricted combination of temporal operators with all other logical connectives 
 creates an expressive power that is unused in the applications, and makes
 the development of a proof system very difficult.
 Therefore, we replaced the temporal operators with  {our three novel}
...
Using this operator, we can formulate  \SrobustB % in \Chainspec  
as
...
\end{lstlisting}
Namely, a transition from a \sd{current} state where an account's balance is \prg{bal}, to a \sd{future} state where 
it has decreased, may \emph{only} occur if \sd{in the current state} some unknown client object  has access to that account's password. 
More discussion in \S\ref{s:bankSpecEx}. 

  
Unlike  \emph{Chainmail}'s temporal operatots, 
...
\jm[Not sure if it makes sense to put this here. My thinking was that it might help with the intuition of the logic early on if the form can be lined up with the english definition of (S3). \sd{I do not follow the argumet. to discuss}]{}
...
% in \Chainspec (\S\ref{s:expressiveness}). 
 \item
%\Chainlogic, a 
A \jm[]{novel} logic for proving a module's adherence to 
...
\jm[I don't know if we need to state parametricity on encapsulation as a limitation. In some ways it is a strength, i.e. we make no assumptions about privacy and features of the underlying language, only in so far as we need to express our examples.\sd{I dropped this dicussion}]{}
...
satisfaction, encapsulation, and the type system.
 {\sd{Moreover, in line} with other work in the literature,} we forbid 
