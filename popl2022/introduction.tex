...
{\emph{robust}} ({programs don't do what they aren't supposed to, even in the presence of untrusted code}). % (safety), 
%\vspace{.03in}
%\jm{or more specifically, modules need to be \emph{robust}(modules are safe, even in the presence of untrusted code).}
 {This work  addresses the robustness of modules: \sd{how it can be specified, and how adherence to the specification can be verified}.}
...
\sd{However, \Scorrect does not make the   bank module 
 \emph{robust}; for example, what if it were possible to take ten percent of the
 balance  from an account
 through a further method defined in the module?
 A first approach to specify robustness would require}
...
% Hoare triples,  
\sophiaPonder[dropped the mention of correctness here]{A multitude of different 
...
\jm[is this true??? what claim can we make here? \sd{why not the cali you made?}]{However, to our knowledge, 
...
.
  ...
  Both {\sc{VerX}} \cite{VerX} and  \emph{Chainmail} \cite{FASE} can express \SrobustA but
  {\sc{VerX}}  cannot express \SrobustB, because 
...
%     \sd{Moreover,}  correspondence assertions cannot express (S$_{\text{robust}_2}$): they lack
...
\emph{Chainmail}'s %approach to add
  capability operators, \sophiaPonder[we should have introduced the capability operators before talking about them]{}
...
of module implementation, only that module code can be verified using some Hoare logic. This is a significant advantage, as it makes \Nec broadly applicable to any such languages that use Hoare logic to establish correctness.\sophiaPonder[it is not
...
{We provide} a description of the formalization of \SrobustB % in \Chainspec  
...
\end{lstlisting}
A transition from a \sd{current} state where an account's balance is \prg{bal}, to a \sd{future} state where 
