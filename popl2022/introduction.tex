...
through \citeasnoun{Hoare69} triples: a  precondition, a code snippet, and a
 ...
 \begin{quote}
   \Scorrect\ \ $\triangleq$
{\scriptsize \lstinline*{pwd=src.pwd} src.transfer(dst,pwd) {src.bal=src.bal-100}* $\wedge$ \ldots}\\
Calling \prg{transfer} on \sd{an account with the correct password} will transfer the money.
\end{quote}
...
have all been proposed to support robustness.
%all ensure some level of robust safety. 
\Nec is the first approach that is able to express and prove
...
\\
$ 
\strut \hspace{.4in} \onlyIfSingle {A_{curr}} {A_{fut}} {A_{nec}}. \strut \hspace{.4in} \onlyIfSingle {A_{curr}} {A_{fut}} {A_{intrm}}
$ 
\\
\sd{The  first says    that 
...
Unlike  \emph{Chainmail}'s temporal operators, 
 ...
 Thus, we \sophiaPonder[Drop: "believe that we"]{have reached} a  sweet spot between expressiveness and 
 provability.
...
\susan[]{We faced the challenge  how to develop a logic that would enable us to prove that code 
 adhered to  specifications \sd{talking of system-wide properties.} The Eureka moment was the realisation that all the information we required was hiding in the classical specifications in the program functions.}
...
\sd{This leaves the challenge that no Hoare logics support such infinite conjunctions.
 Three breakthroughs lead to our solution of that challenge:}
 \begin{description}
 \item
 [Per-method specs] 
  The Hoare triple 
$ \{ A_1 \wedge \neg A_3 \} \ \prg{x.m(ys)}\  \{ \neg A_2 \}$ is logically equivalent with the specification
...
\sophiaPonder[slightly different in main text, have simplified here; hope it does not create confusion]{the
\emph{finite conjunction}
 $ \onlyIfSingle {(\, A_1 \wedge \neg A_2 \wedge {\calls{\_}{\prg{x}}{\prg{m}}{\prg{ys}}}\, )} {A_2} {A_3}$
 for all methods of that module is logically equivalent with
 $ \onlyIfSingle {A_1 \wedge \neg A_2} { A_2} {A_3}$. }
