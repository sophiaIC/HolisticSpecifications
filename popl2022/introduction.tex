...
While \Scorrect  describes \sd{the \emph{correct use} of the module, but is \emph{not} concerned with its \emph{robustness}.}
...
Specification \SrobustA gives   the guarantee that it is not possible to  take money out of the account through some other method than \prg{transfer} which did not
...
\emph{Chainmail}'s %approach to add
  ...
  temporal operators, we observed that while their
   ...
   number of points in time, the examples found in the literature. \sophiaPonder[here we should add ALL the works, ie
