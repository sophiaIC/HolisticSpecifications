...
\end{figure}
We devote the rest of this section to the \jm[]{proof} of \jm[]{a variation on} our
running example of a Bank Account, along with a proof 
of the bank account specification using our \Nec logic.

\paragraph{The Bank Account Module}
\jm[]{The Bank Account defined in this section differs from that of Sec. \ref{s:outline}.
We modify the Bank Account in order to demonstrate how using \Nec logic 
we are able to deal several challenges that come with more complex data structures and specifications.
These challenges are 
\begin{description}
\item[(1)] Specifications defined using ghost fields.
\item[(2)] Modules with multiple classes and methods that all must be considered when constructing proofs about emergent behavior.
\item[(3)] How ownership systems can be used to construct proofs for \Nec specifications -- even the simple ownership captured by \enclosed classes in \Loo.
\end{description}}
\jm[]{We provide the implementation of \jm[]{\prg{Mod4}} in Fig. \ref{f:ex-bank-short}.
In \prg{Mod4}, we separate the balance 
...
\jm[]{Note, Fig. \ref{f:ex-bank-short} does not provide the classical specifications of \prg{Mod4}, 
which can be found in full in Apdx. \ref{app:BankAccount}. Informally, we introduce classical specifications
that state that 
\begin{description}
...
\jm[]{We rewrite \texttt{NecessityBankSpec} to use the ghost field, \texttt{getBal},
and not simply the balance field as in \texttt{Mod1}, \texttt{Mod2}, and \texttt{Mod3}.}
\begin{lstlisting}[language=Chainmail, mathescape=true, frame=lines]
NecessityBankSpec $\triangleq$ from b:Bank $\wedge$ b.getBal(a)=bal 
                     to b.getBal(a) < bal
                     onlyIf $\neg\wrapped{\prg{a}}$
\end{lstlisting}
...
follows the outline of our reasoning given in Sec. \ref{s:approach}}: \jm[]{(1) we prove encapsulation of 
key assertions, (2) we prove \emph{per-method} \Nec specifications, (3) we prove \emph{per-step} \Nec specifications,
and finally we (4) prove \emph{emergent} \Nec specifications.}
%\begin{description}
%\item[Part 1:]
...
\jm[]{Informally $\intrnl{e}$ indicates that the evaluation of expression $e$ requires references no external objects, and $\encaps{A}$ indicates 
that internal computation is necessary for a change in satisfaction of $A$. The full details of $\intrnl{}$ and $\encaps{}$ are given in 
Apdx. \ref{s:encap-proof}}
We provide the proof for the encapsulation of \prg{b.getBal(a)} below\\
%\begin{figure}[h]
\begin{proofexample}
	...
	{\begin{proofexample}
		\proofsteps{\prg{getBalIntrnl}}
			...
			{$\proves{\prg{Mod4}}{\givenA{\prg{b, b$^\prime$:Bank $\wedge$ a:Account $\wedge$ b.getBal(a)=bal}}{\intrnl{\prg{b.getBal(a)}}}}$}
				{by \prg{aIntrnl}, \prg{bIntrnl}, and \textsc{Intrnl-Ghost}}
		}
		\endproofsteps
	\end{proofexample}
		}
...
\item[\textbf{\prg{AuthPwdLeak}}:] any call to \prg{authenticate} may only ``un-\prg{wrap}'' an object if \prg{false} is first satisfied -- clearly an absurdity.
\end{description}}
...
\paragraph{\emph{\textbf{\prg{AuthPwdLeak}}}} The proof of \prg{AuthPwdLeak} is given below, and is proven by application of classical Hoare logic rules and \textsc{If1-Wrapped}.
...
PasswordLeak $\triangleq$ from a:Account $\wedge$ a.password=p $\wedge$ wrapped<p>
                  ...
                  onlyIf false
\end{lstlisting}
\jm[]{We provide the proofs of these in Apdx. \ref{app:BankAccount}, but describe the construction of the proof of \prg{BalanceChange} here:
by application of the rules/results
 \textbf{AuthBalChange}, \textbf{changePassBalChange}, \textbf{Ledger::TransferBalChange}, \textbf{Bank::TransferBalChange}, \textbf{BalanceEncaps}, and \textsc{If1-Internal}.}
%
