\section{Bank Account Example Proof}
\label{s:examples}
...
for looking up the balance of an account at a bank.\susan[my footnote failed to colour]{\footnote{A Necessity Specification is independent of the implementation details of the code. It would need to hold for an account whose implementation did not use a ledger or ghost variables to hold balances.}} 
A \prg{Ledger} is
...
The specification we would like to prove is given as \prg{NecessityBankSpec}.
...
\item[D]
Finally we use the Logic of Necessity and the results of A, B, and C, prove the emergent behavior specified in \prg{NecessityBankSpec}.
\end{description}
