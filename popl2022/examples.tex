  ...
  class internal Ledger
    ...
    method transfer(amt, from, to)
      (PRE:  from == acc1 && to == acc2
       POST: bal1$_\texttt{old}$ < bal1$_\texttt{new}$)
      (PRE:  from == acc2 && to == acc1
       POST: bal2$_\texttt{old}$ < bal2$_\texttt{new}$)
      (PRE:  a : Account
       POST: res != a.password)
...
\proofsteps{\texttt{BalanceChange}}
	{\onlyIfSingleEx
			...
			{b.getBalance(a) < bal}
			{$\calls{\_}{\texttt{b}}{\texttt{transfer}}{\texttt{a.password}, \_, \texttt{a}, \_}$ $\vee$ $\calls{\_}{\texttt{b.book}}{\texttt{transfer}}{\_, \texttt{a}, \_}$}
			}
		{by \textsc{If1-Encaps} and Classical Specs. on \texttt{BankMdl}}
	{\onlyIfSingleEx
			{a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}
			{b.getBalance(a) < bal}
			{$\calls{\_}{\texttt{b}}{\texttt{transfer}}{\texttt{a.password}, \_, \texttt{a}, \_}$ $\vee$ $\exists$ o.[$\external{\texttt{o}}$ $\wedge$ $\access{\texttt{o}}{\texttt{b.book}}$]}
			}
		{by \textsc{If1-}$\longrightarrow$, \textsc{Caller-Ext}, and \textsc{Calls-Recv}}
	{\onlyIfSingleEx
			{a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}
			{b.getBalance(a) < bal}
			{$\calls{\_}{\texttt{b}}{\texttt{transfer}}{\texttt{a.password}, \_, \texttt{a}, \_}$}
			}
		{by \textsc{If1-}$\longrightarrow$, \textsc{Intrnl-Wrapped}, and \textsc{If1-}$\vee$E}
\endproofsteps
\end{proofexample}
\begin{proofexample}
...
\endproofsteps
\end{proofexample}
\caption{Proofs of the intermediate results \texttt{BalanceChange}, \texttt{PasswordChange}, and \texttt{PasswordLeak}}
\end{figure}
...
\endproofsteps
\end{proofexample}
\caption{Proof of \texttt{BankSpec}}
\end{figure}
