...
for looking up the balance of an account at a bank.\footnote{A \NecessitySpecification is independent of the implementation details of the code. It would need to hold for an account whose implementation did not use a ledger or ghost variables to hold balances.}
A \prg{Ledger} is
...
This further abstracts the implementation of the Bank Account from the \NecessitySpecification \texttt{NecessityBankSpec}. The new 
...
we need to prove three per-step \NecessitySpecifications: \prg{BalanceChange}, \prg{PasswordChange}, and \prg{PasswordLeak}.
\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
...
Finally, we combine our module-wide single-step \NecessitySpecifications to 
