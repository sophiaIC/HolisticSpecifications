...
\sophiaPonder[]{In this section we define \prg{Mod4}, a new Bank Account implementation, which   differs from that of Section \ref{s:outline}.}
...
\jm[]{}\sophiaPonder[said: We rewrite]{} The \texttt{NecessityBankSpec} will use the ghost field, \texttt{getBal},
...
\jm[]{We provide the implementation of \jm[]{\prg{Mod4}} in Figure \ref{f:ex-bank-short}.
In \prg{Mod4}, we move the balance 
of an account into a ledger that is stored within a bank. Thus, 
the module \texttt{Mod4} (Figure \ref{f:ex-bank}) consists of 3 classes: (1) \texttt{Account} that
