...
and provide an elaborated proof for our running example of 
the Bank Account. 
...
examples in this section, we state several 
rules that under any such logic should be provable. }
\jm[]
{\footnotesize
\begin{mathpar}
\infer
...
\prg{BankMdl} has access to the password of that account. An informal account of the construction of 
...
\item[B]
We use classical specifications to prove that only a call to \prg{Bank::transfer} may 
...
necessary precondition for reducing the balance using \emph{any} method in \prg{BankMdl}. Similarly, 
we show that \emph{no} step may leak the password of an account.
\item[D]
Finally we use the Logic of Necessity and the results of A, B, and C, prove the emergent behavior specified in \prg{BankSpec}.
\end{description}
...
\end{minipage}
Below we provide the proofs for each method in \prg{BankMdl} that they cannot
...
\subsubsection{C: Proving Per-Step Necessary Specifications}
...
we need to prove three per-step necessary specifications: \prg{BalanceChange}, \prg{PasswordChange}, and \prg{PasswordLeak}.
\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
