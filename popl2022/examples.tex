...
A \sd{summary} of this proof has already been discussed in \S \ref{s:approach}.
...
Recall that an \prg{Account} includes %a \prg{balance} field, and a \prg{transfer} method, along with any other fields and methods. 
 at least a  field \sd{or ghost field} called \prg{balance}, and a method called \prg{transfer}. 
\sophiaPonder[I thought we had agreed no to repeat these things: In order to confidently 
pass our account to unknown, potentially malicious code, we wish to prove that our 
\prg{Account} and the module that contains \prg{Account} satisfies \SrobustB.]{}
%
 We first rephrase 
...
\end{lstlisting}
\sophiaPonder[I thought we had agreed to get rid of repeating such things: That is, if the balance to my account ever decreases, then some external
object must know my password: 
%Or conversely, 
If only I know my password, then I know my money is safe.]{}

\sophiaPonder[JULIAN, check whether this part is necessary!]{We next revisit the   \funcSpec from \S \ref{s:bank} and derive the following 
...
of \prg{PRE}- and \prg{POST}-conditions is weaker than the one in \S \ref{s:bank}, and is not modular. However, it is sufficient for the purposes
of proving adherence to  \SrobustB.}
...
$\Sclassic$  $\triangleq$
   method transfer(dest:Account, pwd':Password) -> void  
      ...
      (PRE: this.balance$=$bal1 $\wedge$ dest.balance$=$bal2 $\wedge$ (this.pwd$\neq$pwd' $\vee$ this$\neq$dest)
       POST: this.balance=bal1 $\wedge$ dest.balance=bal)
...
\label{s:BA-encap}
The first part of the proof \sophiaPonder[is proving]{demonstrates} that the \prg{balance}, \prg{pwd}, and external accessibility to the password are 
