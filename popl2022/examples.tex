...
\end{figure}
We devote the rest of this section to the proof  
expressed in    \Nec logic
...
\ModC has access to the password of that account. 
...
Note, Figure \ref{f:ex-bank-short} does not provide the classical specifications of \ModD, 
...
While both the implementation and the specification being proven have changed from that of
\ref{s:outline}, the structure of the proofs do retain broad similarities. In particular the proof in this section 
follows the outline of our reasoning given in Sec. \ref{s:approach}: we prove (1) encapsulation of the account balance and password, 
...
These proof steps are quite verbose, and for this reason, we only focus on proofs
of \prg{authenticate} from the \prg{Account} class.

There are two \emph{per-method} \Nec specifications that we need
...
\end{lstlisting}
We provide the proofs of these in Appendix \ref{app:BankAccount}, but describe the construction of the proof of \prg{BalanceChange} here:
