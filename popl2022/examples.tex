...
%       POST: bal1$_\prg{new}$ < bal1$_\prg{old}$)
%      (PRE:  from == acc2 && to == acc1
%       POST: bal2$_\prg{new}$ < bal2$_\prg{old}$)
...
\label{f:ex-bank}
\end{figure}
In this section we provide an elaborated description of our
running example of a Bank Account, along with a proof 
of the bank account specification using our Logic of Necessity.
We start by providing an implementation of \prg{BankMdl} in Fig. \ref{f:ex-bank}.
...
for looking up the balance of an account at a bank. A \prg{Ledger} is
...
The specification we would like to prove is given as \prg{BankSpec}.
\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
BankSpec $\triangleq$ from b : Bank $\wedge$ b.getBalance(a) = bal 
              to b.getbalance(a) < bal
              onlyIf $\neg\wrapped{\prg{a}}$
\end{lstlisting}
That is, if the balance of an account ever decreases, it must be true that some object external to
\prg{BankMdl} has access to the password of that account. We construct this proof by following
the outline of our reasoning given in Section \ref{s:approach}:
\begin{description}
\item[A]
We use our rudimentary encapsulation system to prove that both \prg{b.getBalance(a) = bal} and \prg{a.password = pwd} are an encapsulated assertion.
\item[B]
We prove necessary preconditions for reducing the balance for each method in \prg{BankMdl}.
\item[C]
We combine the per-method necessary preconditions along with the encapsulation of \prg{b.getBalance(a) = bal} to arrive at a necessary precondition 
for reducing the balance using \emph{any} method in \prg{BankMdl}.
\item[D]
We use the Logic of Necessity to prove the emergent behavior specified in \prg{BankSpec}.
\end{description}

\subsubsection{A: The Encapsulation of Account Balance and Password}
We base the soundness of our of encapsulation on the type system of \Loo, and use the proof rules given in Fig.s \ref{f:intrnl} and \ref{f:asrt-encap}.
...
\proofsteps{\prg{BalanceEncaps}}
	{\begin{proofexample}
		\proofsteps{\prg{aIntrnl}}
			{\proofstepwithrule
			{$\proves{\prg{BankMdl}}{\givenA{\prg{b$^\prime$ : Bank $\wedge$ a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}}{\intrnl{\prg{a}}}}$}
				{by \textsc{Intrnl-Obj}}
		}
		\endproofsteps
	\end{proofexample}
	...
	{\begin{proofexample}
		\proofsteps{\prg{bIntrnl}}
			{\proofstepwithrule
			{$\proves{\prg{BankMdl}}{\givenA{\prg{b$^\prime$ : Bank $\wedge$ a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}}{\intrnl{\prg{b}}}}$}
				{by \textsc{Intrnl-Obj}}
		}
		\endproofsteps
	\end{proofexample}
	...
	{\begin{proofexample}
		\proofsteps{\prg{getBalIntrnl}}
			{\proofstepwithrule
			{$\proves{\prg{BankMdl}}{\givenA{\prg{b$^\prime$ : Bank $\wedge$ a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}}{\intrnl{\prg{b.getBalance(a)}}}}$}
	...
	{\begin{proofexample}
		\proofsteps{\prg{balIntrnl}}
			{\proofstepwithrule
			{$\proves{\prg{BankMdl}}{\givenA{\prg{b$^\prime$ : Bank $\wedge$ a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}}{\intrnl{\prg{bal}}}}$}
				{by \textsc{Intrnl-Int}}
		}
		\endproofsteps
	\end{proofexample}
		...
		{\proofstepwithrule
			{
			$\proves{\prg{BankMdl}}{\givenA{\prg{b$^\prime$ : Bank $\wedge$ a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal}}{\encaps{\prg{b.getBalance(a) = bal}}}}$
			}{by \prg{getBalIntrnl}, \prg{balIntrnl}, \textsc{Enc-Intrnl}}}
\endproofsteps
\end{proofexample}\\
We omit the proof of $\encaps{\prg{a.password = pwd}}$, as it's construction is very similar to that of $\encaps{\prg{b.getBalance(a) = bal}}$.
%\caption{Proof of encapsulation of \prg{b.getBalance(a) = bal}}
%\end{figure}

\subsubsection{B: Proving Necessary Preconditions from Classical Specifications}
...
\subsubsection{C: Proving Necessary Module Preconditions}


In order to do this, we need to prove the intermediate results \prg{BalanceChange}, \prg{PasswordChange}, and \prg{PasswordLeak}
\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
...
that we have annotated \prg{BankMdl} with.

%Finally, in order construct the above proofs, we assume that \prg{BankMdl} module 
%has the following traditional specifications.
%%\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
%%AccountIsNotLeakedByBank $\triangleq$ {a : Account $\wedge$ $\forall$ x.[$\internal{\prg{x}}$ $\vee$ $\neg\access{\prg{x}}{\prg{a}}$]} 
%%                              BankMdl 
%%                           {a : Account $\wedge$ $\forall$ x.[$\internal{\prg{x}}$ $\vee$ $\neg\access{\prg{x}}{\prg{a}}$]}
%%\end{lstlisting}
%\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
%PasswordIsNotLeakedByBank $\triangleq$ {a : Account $\wedge$ $\forall$ x.[$\internal{\prg{x}}$ $\vee$ $\neg\access{\prg{x}}{\prg{a.password}}$]} 
%                              BankMdl 
%                           {a : Account $\wedge$ $\forall$ x.[$\internal{\prg{x}}$ $\vee$ $\neg\access{\prg{x}}{\prg{a.password}}$]}
%\end{lstlisting}
%\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
%TransferChangesBalance $\triangleq$ {a : Account $\wedge$ b : Bank $\wedge$ b.getBalance(a) = bal $\wedge$ 
%                                 $\neg$ $\exists$ o.[$\calls{\prg{o}}{\prg{b}}{\prg{transfer}}{\prg{a.password}, \_, \prg{a}, \_}$]}
%                            BankMdl 
%                         {b.getBalance(a) = bal}
%\end{lstlisting}
%\begin{lstlisting}[language = Chainmail, mathescape=true, frame=lines]
...
%                           {b.getBalance(a) = bal}
%\end{lstlisting}
%It is easy enough to see that both \prg{AccountIsNotLeaked}, \prg{PasswordIsNotLeaked}, and \prg{TransferChangesBalance} are provable
...
\endproofsteps
\end{proofexample}
\subsubsection{D: Proving Emergent Behavior}
Finally, we combine our module-wide single-step necessity specifications to 
prove emergent behavior of the entire system.\\
\begin{proofexample}
\proofsteps{\prg{BankSpec}}
