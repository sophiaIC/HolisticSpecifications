...
\sd{Moreover,} \textbf{\prg{balEnc}} states that since bal is an integer, \sd{its} value is constant
...
\prg{a} is encapsulated};  \sd{but remember that their value cannot change, and thus they
vacuously sasfy the definition of encapsulation.}
 \sophiaPonder[was {what we are really saying is that their values may only change 
...
We similarly prove that \prg{a.pwd} \sd{is encapsulated} (\textbf{\prg{PwdEncaps}}), and
that \wrapped{\prg{a.pwd}} \sd{is encapsulated} (\textbf{\prg{PwdInsideEncaps}}). 
That is, if only internal objects have access
to an account's \prg{pwd}, then only internal computation may grant 
 to an external object access to \prg{pwd}.
