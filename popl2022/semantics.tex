...
module. Moreover,    these guarantees  need to be satisfied only at `reachable' states (configurations),
...
\sd{which  characterise configurations at the start of program execution}.
...
\paragraph{Only If}
[$\onlyIf{A_1}{A_2}{A}$]: If an arising program configuration \jm[]{satisfies} $A_1$, and \jm[]{after some execution, a state program state satisfying} $A_2$ \jm[]{is reached}, 
...
[$\onlyIfSingle{A_1}{A_2}{A}$]: If an arising program configuration \jm[]{satisfies} $A_1$, and \jm[]{after a single step of execution, a state satisfying} $A_2$ \jm[]{is reached}, 
then the original program state must have also satisfied $A$.
...
[$\onlyThrough{A_1}{A_2}{A}$]: If an arising program configuration \jm[]{satisfies} $A_1$, and \jm[]{after some execution, a state satisfying} $A_2$ \jm[]{is reached}, then program execution must have passed through some \jm[]{state satisfying} $A$.
