...
%===============
%old
%\jm[]{Given its minimal features, we base \Loo on \LangOO, as defined by 
%\cite{FASE}. While \LangOO was untyped, \Loo includes a simple type
%system to allow for simple type based proofs of encapsulation.}
%================
\jm[modification to description of type system. Not sure if this is clear enough.]{
Given its minimal features, we base \Loo on \LangOO, as defined by 
\cite{FASE}. While \LangOO was untyped, \Loo includes a simple type
system to allow for simple type based restrictions on external access to private data.}
...
Finally, we define define the syntax and semantics of our \Nec specification language  (\S \ref{s:holistic-guarantees}).
...
of an \prg{enclosed} type may not be accessed by module-external
