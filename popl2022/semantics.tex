...
In Definition \ref{def:arising}, \jm[was capitalized]{we} also use the notation $\reductions{M}{M'}{\sigma}{\sigma'}$ to denote
...
\end{description}
The type system is not \jm[]{fundamental to the definition of either \Chainspec or \Chainlogic,
and is included to allow a simple and convenient system for proving encapsulation of \SpecO assertions.}
...
\jm[]{This predicate forms as core aspect of Part 2 mentioned in Section \ref{s:outline},
...
\subsection{\Chainmail -- \NecessitySpecifications}
...
\NecessitySpecifications thus provide us with a rich language for talking about the necessary conditions
under which critical actions within of our software are allowed to occur. 
\jm[]{It is worth discussing the semantics of \NecessitySpecifications and their 
...
and strengthen the right hand side ($A$). This intuition also extends to Only-If and Only-Through \NecessitySpecifications. 
In some places later in this paper, we use the distinction ``\emph{left hand side}'' of a \NecessitySpecifications
