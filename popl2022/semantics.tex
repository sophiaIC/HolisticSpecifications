...
${\sigma' \triangleleft \sigma}$ in more detial in \sophiaPonder[TOADD]{xxx}, but 
...
We can now define our semantics,  $M \vDash H$, 
...
\\
\strut $\hspace{1in}$ \prg{Mod1} $\vDash$ \prg{NecessityBankSpec}
 \\
\strut $\hspace{1in}$ \prg{Mod2} $\not\vDash$ \prg{NecessityBankSpec}
 \\
\strut $\hspace{1in}$ \prg{Mod3} $\vDash$ \prg{NecessityBankSpec}
