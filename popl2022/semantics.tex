...
\label{sec:callbacks}
\sophiaPonder[from rebuttal]{Necessity does not --yet-- support calls of external methods from within internal modules. 
...
\sophiaPonder[from rebuttal, but not all of it]{Note that access is not deep, and only refers to objects that 
an object has direct access to via a field or within the context of the current scope. A transitive definition of access would not be useful in specifying safe and robust software.
 The restricted form of access used in \Nec specifically captures a crucial property of robust programs in the open world: access to an object does not imply access to that object's internal data. For example, an object may have access to an account \prg{a}, but a safe implementation of the account would never allow that object to leverage that access to gain direct access to \prg{a.password.}}
...
%% \kjx{KJX EVIL EVIL NEWPAGE JUST BELOW HERE!!!! EVIL EVIL EVIL EVIL}
%% \newpage
...
Our \Nec specification language extends \SpecO with three novel 
...
state, the \emph{final} state, or any state inbetween.
\end{description}


\label{sec:adapt:motivate}
\noindent Necessity operators can explicitly constrain two or even three states,
and implcitly constrain many states in between. The following 
 specification  \Sadapt 
 says that for an account's balance to go  
 from 150 in one state down to 50 some subsequent state, 
% in \emph{one} step,
 \prg{transfer} must have been be called on that account in between:
%
%
...
\end{lstlisting}
%
\Sadapt  refers to at least two different states: at 
the start, whenever the balance of \prg{a} is 150, and after any number of steps
whenever the balance of \prg{a} is 50. The specification requires
that such a change can only be caused by a call to \prg{transfer}:
that call could be in the current (starting) state, in which case
presumably the balance will be 50 in the immediately following state,
or with any number of intervening states, where for \text{every}
future state where the balance is 50, there must have been a call to
...
%%%%%% SOPHIA's VERSION KEPT IN TOTO BUT COMMENTED OUT
%%%%%% SOPHIA's VERSION KEPT IN TOTO BUT COMMENTED OUT
%%%%%% SOPHIA's VERSION KEPT IN TOTO BUT COMMENTED OUT
...
%% if the assignment \prg{a=a1} is executed? 
...
%% without calling \prg{a.transfer}. It looks as if \Sadapt cannot be satisfied!
%% }
...
%%%%KJX: THIS IS MORALLY IMPORTANT BUT PRACTICALY insignificant
%\begin{figure}[t]
\begin{definition}[\Nec Syntax]
%%%  The syntax of\Nec  Specifications ($S$):
% in   \Chainmail   
%%%%is as follows:
% \footnotesize
...
The defintion of the necessity operators
...
can involve several states.% (\ref{sec:adapt:motivate}). 
%
To see the problem, 
consider a na{\"\i}ve approach to giving semantics to \Sadapt:  if
$..,\sigma_1 \models \prg{a.balance==100}$, 
and  $.., \sigma_1 \leadsto \sigma_2$ and $\sigma_2 \models \prg{a.balance==50}$,
then $\sigma_1$'s continuation starts with a call to \prg{a.transfer}.
If $\sigma_1$ happened to have another account \prg{a1} with balance
50, and the assignment \prg{a=a1} was executed, we would 
reach a $\sigma_2$ where $\sigma_2 \models \prg{a.balance==50}$
without \prg{a.transfer} having been called: indeed, without the
account \prg{a} from $\sigma_1$'s balance having changed at all!
(Haskell programmers will probably feel at home here). 
%%
%% . It looks as if \Sadapt cannot be satisfied!
%% }
%%
%% KJX - don't see why it looks that way, but its' clealr a wrong interpretation.
%%
%% \sd{Obviously, \Sadapt  \emph{is}   satisfied by \ModA, \ModB and
%% \ModC.
%%
...
Under adaptation, the semantics of \Sadapt is:\  if $..,\sigma_1 \models \prg{a.balance==100}$,
and  $.., \sigma_1 \leadsto \sigma_2$ and $..., \boldsymbol{\adapt {\sigma_2}{\sigma_1}} \models \prg{a.balance==50}$,
then $\sigma_1$'s continuation starts with a call to \prg{a.transfer};
...
object with password \prg{pwd1}, and balance \prg{150},  the variable \prg{a1} points to an \prg{Account}
object with password \prg{pwd2}, and balance \prg{50}, and the continuation is $\prg{a.transfer}(\prg{10},...);$ \prg{a=a1;} $\prg{a.transfer}(\prg{20},...)$. 
We reach $\sigma'$ by executing the first two statements from the continuation.
In $\adapt {\sigma'}{\sigma}$ we introduce the fresh variable \prg{y}, and replace \prg{a1} by \prg{y} in the continuation.
...
%%   the continuation in $\sigma$ is $\prg{a=a1;}$ $\prg{a.transfer}(...)$, and   execution of \prg{a=a1} leads
%%   from $\sigma$  to $\sigma'$. 
%%   We then have that $.. \sigma'  \models {\calls{...}{\prg{a}}{\prg{transfer}}{...}}$ and
%%   $... \sigma'  \models {\calls{...}{\prg{a1}}{\prg{transfer}}{...}}$. 
%%   The definition of $\adapt  {\sigma'}{\sigma}$ should  give % be so that 
%%   $... \adapt  {\sigma'}{\sigma}  \not\models {\calls{...}{\prg{a}}{\prg{transfer}}{...}}$ and % so that
%%    $... \adapt  {\sigma'}{\sigma} \models {\calls{...}{\prg{a1}}{\prg{transfer}}{...}}$
%% }

Definition~\ref{d:adapt} gives the full definition of the $\adapt{}{}$
operator
...
\sd{Strictly speaking, $\adapt {}{}$  does not define one  unique state: Because  variables $\overline{y}$ 
