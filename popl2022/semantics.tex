...
Full details in  App. \ref{app:loo}.
\begin{figure}[htb]
%  \vspace*{-2.5mm}
%  \begin{center}
...
The syntanx of \SpecO  % assertions 
is given in
Def. \ref{f:chainmail-syntax}.% the \SpecO specification language.
\scd{\SpecO assertions are a subset of the assertions supported by \emph{Chainmail.}}
...
\scd{\emph{Control} allows tighter  characterisation of the cause of effects --  particularly
...
% \Loo that can be found in App. \ref{app:loo}. Specifically, $\eval{M}{\sigma}{e}{v}$
...
\sophiaPonder[JULIAN: Do we have such a proof indeed?]{We have taken this approach from \cite{FASE}, which also contains a proof that assertions are classical}
%  The full
Find the (unsurprising) semantics of $\hookrightarrow$   in Fig. \ref{f:evaluation}. 

\scd{We also use several shorthands:}  
 $\interpret{\phi}{x} = v$  means that $x$ maps to
value $v$ in the local variable map of frame $\phi$, and $\interpret{\sigma}{x} = v$ means that $x$ 
maps to value $v$ in the top most frame of $\sigma$'s stack. The term  $\interpret{\sigma}{x.f} = v$
has the obvious meaning. \scd{The terms $\sigma.\prg{stack}$,  resp. $\sigma.\prg{contn}$, resp. $\sigma.\prg{heap}$     mean the stack, resp. the continuation at the
top frame of $\sigma$, resp. the heap of $\sigma$.
The term $\alpha\!\in\!\sigma.\prg{heap}$ means that $\alpha$ is in the domain of the heap of $\sigma$,
while $C\in M$ means that class $C$ is in the domain of module $M$. The term \emph{$x$ fresh in $\sigma$} means that $x$ is not in the variable map of the top frame of $\sigma$, while the substitution  $\sigma[x \mapsto \alpha]$ is applied to the top frame of $\sigma$. } 


\begin{definition}[Satisfaction % of \SpecO 
of Assertions
\scd{by a module and a state}] 
...
\scd{In the current work we assume the existence of proof system that judges}
...
\sophiaPonder[simper argument than earlier; keep?]{Wrapping is a very useful concept. For example, the balance of an account whose
  password  is   wrapped,  will not decrease in the next step.
 Quite often,   objects with a lot of  capabilities 
  are kept wrapped so as to adhere to the API's guarantees. 
  For example, in 
  Sect \ref{s:examples} the objects of class \prg{Ledger} would allow withdrawals from an account
  without knowledge of he password; nevertheless \prg{Mod4} adheres to (***) because
...
\sophiaPonder[]{Based on this type system, we define a predicate $Intrnl(e)$, in App TODO-where,
which asserts that any objects read during the evaluation of $e$ are internal.
Thus, any assertion that only involves $Intrnl(\_)$ expressions is encapsulated, more in App TODO-where.}

\sophiaPonder[]{Finally, a further small addition to the type system 
assists the knowledge that an object is wrapped: Classes are 
optionally annotated as \enclosed. An object of \enclosed type 
may not be accessed by  external objects; that is, it is always wrapped. 
