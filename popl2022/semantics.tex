...
\scd{The problem with  the definition (*), is that it does not take into account that the variable bindings in $\sigma'$ might be different to those in $\sigma$. For example, assume  we had a module  which defined accounts with immutable owners. Then, in the for such a module, we would expect that for all $\sigma$:
