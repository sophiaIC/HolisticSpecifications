...
\jm[TODO: make clear that that execution in appendix is small step semantics]{}
 % chopped, as generic 
...
executing object (the \prg{this}) is an \jm[changed from internal]{external} object. 
...
the class of   the \jm[changed from ``address'']{object referred by variable $x$ in the} heap of $\sigma$. 
...
\jm[TODO: change all the ``configurations'' to ``states'']{}
...
\end{definition}
\jm[do we say something about ``$\leadsto*$'']{}
...
\end{itemize}
\jm[]{\emph{Permission} and \emph{Provenance} are inspired by the capabilities literature, while
...
\jm[make $classOf$ consistent, and use it in the definition of $e : C$]{}
...
\item
there exists some $z$, and some frame $\phi$ in the stack of $\sigma$ such that \jm[]{$\interpret{\sigma}{x}=\interpret{\phi}{\prg{this}}$}, \jm[]{and $\interpret{\sigma}{y}=\interpret{\phi}{z}$}
\end{itemize}
\item
...
\item
$\satisfiesA{M}{\sigma}{\calls{x}{y}{m}{z_1, \ldots, z_n}}$ \ \ \ iff \ \ \ \jm[sophia asks if we should replace points 2 - 4 in calls with the encoded version?]{}
\begin{itemize}
\item
...
A proof system for such assertions is indicated by a judgment of the form \jm[]{$\proves{M}{A}$}. 
...
We define a useful shorthand: \jm[fixed some typos]{} the $\wrapped{}$ predicate  states 
...
\jm[removed: reading or writing involving an object ]{}
...
\end{figure}
\jm[TODO: make it clear that \Chainmail includes \SpecO]{}



\paragraph{Only If}
[$\onlyIf{A_1}{A_2}{A}$]: If an arising program state \jm[]{satisfies} $A_1$, and \jm[]{after some execution, a state program state satisfying} $A_2$ \jm[]{is reached}, 
...
[$\onlyIfSingle{A_1}{A_2}{A}$]: If an arising program state \jm[]{satisfies} $A_1$, and \jm[]{after a single step of execution, a state satisfying} $A_2$ \jm[]{is reached}, 
then the original program state must have also satisfied $A$.
...
[$\onlyThrough{A_1}{A_2}{A}$]: If an arising program state \jm[]{satisfies} $A_1$, and \jm[]{after some execution, a state satisfying} $A_2$ \jm[]{is reached}, then program execution must have passed through some \jm[]{state satisfying} $A$.
...
that the intermediate state where $A$ is true might be the initial state \jm[removed: ($\sigma_1$)]{},
or final state \jm[removed: ($\sigma_2$)]{}. 
...
the execution of an entire program\jm[removed:, even those programs in the open world, where
...
bank account example discussed in Section \ref{s:intro}\jm[]{. We} have already shown
how we can \jm[]{specify} knowledge of an account's password using \prg{NecessityBankSpec},
...
$A'$ \jm[typo]{cannot} be invalidated, unless a method from $M$ was called.}
