...
We will now give the semantics of the three necessity operators. To do that, we   need to define the auxiliary renaming operator 
...
$ (Def\_1)$ says that a module $M$ satisfies   $ \onlyIf{A_1}{A_2}{A}$ if for all states $\sigma$ that arise from execution of $M$ linked with any arbitrary further module $M'$, and any further states $\sigma'$ which are reachable from  $\sigma$, if $\sigma$ satisfies $A_1$ and $\sigma'$ satisfies $A_2$, then $\sigma$ must also satisfy $A$.
%
...
identical to that of ${\sigma'}$m and whose tope frame maps variables in the
...
\jm[]{Adaptation may seem complex, however it is esentially a variable renaming operator. Consider the a program state where 
\prg{x.f = 5}.
This simple assertion would be invalidated by simply overwriting \prg{x} in the local variable map, 
even though the field \prg{f} of the object originally indicated by \prg{x} remains \prg{5}. This invalidation does not
reflect a meaningful change in the program state, and in order to preserve the intention of the assertion, the $\triangleleft$ operator
