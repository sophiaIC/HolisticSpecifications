...
(See Defs.\ref{def:initial} and \ref{def:arising}).
...
\SpecO is a subset of the \emph{Chainmail} assertions language, i.e.\
...
An assertion may be an expression,   \scd{query the defining class of
...
\subsubsection{Adapting the future for the present.}  
\jm[I moved adaptation here because it didn't really fit in 3.2]{\Nec specifications may refer to program state at two, or even three different points in time, which introduces the 
potential for variable name clashes between different program states.} 
\jm[]{To deal with this} \sophiaPonder[new]{we introduce the adaptation operation $\adapt {\_} {\_}$ to allow us to
``see a  state   through the lens of another state''.
Assume, for example a state $\sigma_0$ which satisfies $A_0$ where $A_0 \triangleq \prg{a}:\prg{Account} \wedge \prg{a.balance}=100$.
Assume that $\sigma_1$ is the result of executing $\prg{a.transfer(p,10)}$ where \prg{p} is \prg{a}'s password, while $\sigma_2$ is the result of executing $\prg{a=new Account}$. 
We have that $\satisfiesA {...}{\sigma_0} {A_0}$, and 
