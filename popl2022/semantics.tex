...
\paragraph{Relationship with Temporal Logic, and implications between \Nec Operators}
\sd{Two of the three \Nec operators can be encoded in temporal logic.
Namely, ${\onlyIf{A_1}{A_2}{A_3}}$ expressed on temporal logic would be
$A_1\ \wedge\ \Diamond A_2\ \longrightarrow\ A$, while
 $\onlyIfSingle{A_1}{A_2}{A}$ would be $\ A_1\ \wedge\ \bigcirc A_2\ \longrightarrow\ A$ ---
remember that in temporal logic, $\Diamond$ indicates the future,  and  $\bigcirc$ indicates the next step.
 However, $\onlyThrough{A_1}{A_2}{A}$ cannot be encoded, because the state where $A$ 
 holds must be between the state where $A_1$ holds, and the state where$A_2$ holds; and
 this must be so on \emph{any execution} path leading from $A_1$ to $A_2$.
}

The three \Nec \ operators % sd dropped " defined in Def. \ref{f:holistic-syntax}" as obvious
...
\sd{To achieve the adaptations in the variable map and the continuation, in Def. \ref{d:adapt} below,
...
Our Definition is equivalent to the  \emph{adaptation} operator from \cite{FASE}. 
%We
%  write
%  $\adapt {\sigma'} {\sigma}$
...
In Fig. \ref{fig:Adaptation}, we illustrate the viewpoint adaptation operator. In state $\sigma$ the variable \prg{a} points to an \prg{Account}
object with password \prg{pwd1}, and balance \prg{100},  the variable \prg{a1} points to an \prg{Account}
