...
we define two forms of execution for \Loo, one in Figure \ref{f:loo-semantics}
...
have both known (internal) and unknown (external) provenance.
...
\begin{itemize}
\item
$\sigma.(\prg{heap}) = [\alpha\ \mapsto\ \{\prg{class}:=Object;\ \prg{flds}:=\emptyset\}]$ and
\item
...
Finally, we assume that there is some type system defined for \Loo that enforces several 
two encapsulation/ownership properties:
\begin{itemize}
\item
Classes can be annotated as either \prg{inside} or \prg{boundary}, where \prg{inside} objects
may not be returned as method results from \prg{boundary} objects
\item
Ghost fields may be optionally annotated as \prg{intrnl}, meaning only references to objects 
internal to the enclosing may be used in the definition of that ghost field.
\end{itemize}
...
\end{definition}
Chainmail \cite{FASE} 
is also able to express specifications of \emph{permission}, \emph{viewpoint}, and \emph{control}, however we 
...
Much of these gains arise from the exclusion of temporal operators
that \citeauthor{FASE} includes. We model aspects of \emph{time} 
using the novel Necessity Logic presented in this paper. 
We also do not include assertions about \emph{space}, and the shape of 
the heap, as they are not required for either the definition 
of our Necessity Logic, or expressing the examples we are interested in (Seciton \ref{s:examples}).
...
In this Section we define syntactic forms for 
\emph{Necessity Specifications}. Figure \ref{f:holistic-syntax} 
gives the syntax.
...
in some intermediate state. We give the definition of ``only  through'' in Def. \ref{def:ot}. Note 
