...
\sdN{This example has the advantage that it requires several objects, and 
uses two, or even three different classes.}\kjx{do we want to say
that?  can we put the code in?}
...
\sd{We will not show the code  here, but suffice it to say} that class 
\prg{Account} has  methods \prg{deposit(src, amt)} and 
...
One obvious solution would be to \sdN{adopt} %return to 
a closed-world
...
\item % After creation,
% it said
%  the \emph{only} way and account -- SD changed as I thick ti has slightly wrong meaning 
An account's
...
%The contribution of this paper is a specification language and
...
%\sophia{ explain in some later section on that  this assertion implicitly gives   that \prg{o} is external}
...
%back into the main \prg{acm\_acc}. 
%Assertion (3) is crucial, because it guarantees that even 
...
implementation that includes a \prg{steal} \sdN{method.}
% SD chopped the below, as it already appears in prev page
