...
is to make writing holistic specificatons as straightforwad as
possible.

This is why we support bi-directional temporal opreators (``will'',
``was'' etc) rather than just one temporal direction. Some assertions
are easier to express looking forwards (if a vendiug machine takes a
...
Similarly, in an imperative setting, the quesiton of whether a change can
...
various ways: ``access'', ``in'', and ``external''.  On object
referring to another appoears simple enough on the surface, but causes
