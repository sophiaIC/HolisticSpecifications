...
% but we don't! 
 assume \sophia{a dynamically} typed language (\sophia{so that our results are applicable to the statically
 as well as the dynamically typed setting}); 
 \secomment{We do not depend on the additional safety static typing provides, so we only assume a dynamically typed language.}
