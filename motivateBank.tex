...
with access  to two \prg{Account}s of the same  \prg{Bank} on can  transfer any amount of money from
...
The PRE-condition of \prg{deposit} requires that  the receiver and the first argument  (\prg{this}, \prg{scr}) are \prg{Account}s
and belong to the same bank,
that \prg{amt} is a number, and that \prg{src} holds at leat \prg{amt} moneys.
The POST-condition mandates that \prg{amt} has been transferred from \prg{src} to the receiver.
 The function \prg{makeNewAccount} specified below returns a fresh \prg{Account} with the same bank, and transfers \prg{amt}
...
%Notice that the specification means that access to an \prg{Account} allows one to withdraw all the money it holds
...
%for a specification which supports owners.
...
the code below  satisfies the assertion: assuming that 
\prg{acm\_acc} and \prg{auth\_ascc} are \prg{Account}s, and  \prg{acm\_acc} has a balance of 10,000 before an author is
registered, then afterwards it will have a balance of 11,000 while the
\prg{auth\_acc}'s balance  will be f 500 from a starting balance of 1,500
(barely enough to buy a round of drinks at the conference hotel bar).



\begin{lstlisting}
  assume acm_acc,auth_scc: Account $\wedge$ acm_acc.balance=10000 $\wedge$  auth_acc.balance=1500
...
that  during maintenance was given a new method \prg{deposit}
...
\sd{send notifications to customers} but to forbid implementations of \prg{steal}, % that meet fig.~\ref{fig:steal}.  
...
additional specifications that capture those assumptions.  The following
 three requirements   prevent methods like \prg{steal}:

\begin{enumerate}
\item After creation, the \emph{only} way an account's
  balance can be changed is if a client   calls the \prg{deposit} method  with the
...
However, requirement  (1) is not enough to protect our money. We need to (2) avoid that an \prg{Account}'s balance is
modified without access to the particular \prg{Account}, and (3) ensure that such accesses are not leaked. 
...
Below we  express these  informal requirements  %assumptions using 
...
% TODO say later that the object
...
the external object \prg{o} has access to \prg{a} in the \emph{current} configuration.
Notice also, that the object which makes the call to \prg{deposit} described in (1), and the object which 
has access to \prg{a} in the current configuration described in (2) need not be the same:It may well be that the
latter passes (indirectly) to the former access to \prg{a}, which then   makes the call
to \prg{deposit}.
...
some object \prg{o} which is external has access to some account \prg{a}, then in the current configuration some object 
from \SF has access to \prg{a}. Note that \prg{o} and $\prg{o}'$ may but need not be the same object. Note also
that $\prg{o}'$ has to exists and have access to \prg{a} in the current configuration, but 
\prg{o} need not exist in the current configuration -- it may be allocated later.
...
and \prg{notify} methods,  but does not permit an
%specification from fig.~\ref{fig:count},
implementation that also provides the \prg{steal} method.
...
Do we way the story just using (2)?}
...
Thus, if the \prg{ACM} does not trust the \prg{author}, it will not pass to them \prg{acm\_acc}, which
contains all of \prg{ACM}'s money. Instead, it will pass a secondary, empty account, \prg{acm\_aux},
into which the \prg{author}  will pay her fee, and from which the \prg{ACM} will transfer the money
back into the main \prg{acm\_acc}. Assertion (3) is crucial, because it guarantees that even 
if the \prg{author} were malicious, she could not use  \prg{acm\_aux} to obtain access to
 \prg{acm\_acc} or any other account. %, to which she did not have access.
...
in fact, in section XXX\sophia{TO ADD} we show two very different implementations
