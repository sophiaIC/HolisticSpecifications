...
\sd{Consider a simplified banking application. We assume that consists of a \prg{Bank} object at \prg{1},
...
\end{minipage}
\end{tabular}
\sdcomment{After discussion with Susan I think we need such a diagram.}
...
%can 
\sd{Below we} express these two informal \sd{requirments} %assumptions using 
in \Chainmail.  Rather than specifying \prg{functions} to
...
\sdcomment{We have to explain that the two objects above may be different breaking the above into two.}
%\end{definition}
...
that in the past called the \prg{deposit} method: ($\Calls{\prg{deposit}}$).
