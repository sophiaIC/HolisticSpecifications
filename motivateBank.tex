...
The PREcondition of \prg{deposit} requires that  the receiver and the first argument  (\prg{this}, \prg{scr}) are \prg{Account}s
and belong to the same bank,
that \prg{amt} is a number, and that \prg{src} holds at leat \prg{amt} moneys.
The POSTcondition mandates that \prg{amt} has been transferred from \prg{src} to the receiver.
 ...
 \se{As specified in our \prg{BankSpec} the only way to put money into the \prg{Bank} is with a call to \prg{newAccount} and there is no way to remove money from the \prg{Bank}.}
...
\prg{acm\_acc} and \prg{auth\_acc} are \prg{Account}s, and  \prg{acm\_acc} has a balance of 10,000 before an author is
...
To see the problem, what if the bank provided a \prg{steal} method that 
 emptied out every account in the bank into a thief's account \se{(also in the \prg{Bank})}?
...
that $\prg{o}'$ has to exist and have access to \prg{a} in the current configuration, but 
...
Thus, if the \prg{ACM} does not trust \prg{author}s, it will not pass them access to \prg{acm\_acc}, which
contains all of the \prg{ACM}'s money. Instead to receive money, it will pass a secondary, empty account, \prg{acm\_aux},
...
malicious \prg{author}s  could not use  \prg{acm\_aux} to obtain access to
