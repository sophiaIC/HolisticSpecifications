...
\emph{Aside} Notice that the specification means that access to an \prg{Account} allows one to withdraw all the money it holds
...
\susan{ Can I switch assume to pre and assertion to post?}\sophia{No, because ORE and POST is how
methods are specified, while assume and assert appear on code. We could use "requires" and "ensures" instead of PRE and POST}
...
To see the problem, what if the bank provided a \prg{steal} method that 
 emptied out every account in the bank into a thief's account \se{(also in the \prg{Bank})}?\sophia{yes, also in the bank, but does ti matter? I want to avoud having too many facets in the story.}
...
\item An account's balance can be \sd{changed   only  if a client} has
  \sd{access to} that  particular account. % object.
  % I want the structure of (1) and (2) to be similar.
\item \sd{The \prg{Bank}/\prg{Account} module does not leak} access to existing accounts or banks. 
...
\sd{requirements} %assumptions 
capture \emph{necessary} %conditions 
\sd{rather} than
\emph{sufficient} conditions:  \sd{Calling the \prg{deposit}
method access to the account %is called to change an account's balance, and it is necessary
are necessary for any change to an account. to take place.}
...
% we no longer need Past here, as we are ion visible states 
  \exists \prg{o}. [\    \prg{a}:\prg{Account}\Calls{\prg{o}}{\prg{deposit}}{\prg{a}}{\_}{\_} \vee\  \Calls{\prg{o}}{\prg{deposit}}{\_}{\prg{a}}{\_}\  \ ] \ \ \ \ ] \hfill $
...
called the \prg{deposit} method with \prg{a} as a receiver or an an argument 
...
Thus, if the \prg{ACM} does not trust \prg{author}s, it will not give them access to \prg{acm\_acc}, which
contains all of the \prg{ACM}'s money. Instead, in order to receive money, it will pass a secondary, empty account, \prg{acm\_aux},
...
\sd{Thus, we need to be able to argue, that  passing the \prg{acm\_aux} to some unknown object \prg{u},
...
we trandtional specifications we cannot.}
