...
%\sdcomment
\sophia{After discussion with Susan I think we need such a diagram. But the flow with the next para is nit good.}
...
%\sdcomment
\sophia{"Map" should be a mathematical construction, not some more syntax. And why does "at" mean lookup}
%\secomment
\susan{Why not "dst.deposit(src,n)" instead.}
%\secomment
\susan{The name "deposit" is confusing because it depostis into dst, but withdraws from the argument. Please can we have transfer instead}
%\secomment
\susan{Make clear in the spec-s above what is the PRE-condition and what is the POST-condition. We aren't trying to have our own unique sufficient spec stuff are we? Easier for people to get their heads around specs as they know them.}
