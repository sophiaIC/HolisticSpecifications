...
flow operator ``$\Calls{s}{x}{m}{y}$'' allows programmers to make similar
specifications, (e.g. $\Calls{s}{p}{\prg{SetValue}}{v} \rightarrow \Future{\Calls{Notify}{}{}{}}$ --- when a subject receives a \prg{SetValue} method,
...
More recently \citet{ddd} have designed OCPL, a logic
...
and the behaviour of the system described by considering attacks only
on low values.  OCPL is a program logic, and Swasey use that logic to
prove a number of object-capability patterns, including
sealer/unsealer pairs, the caretaker, and a general membrane.

\citet{schaeferCbC} have recently taken a similar approach to Swasey,
...
construction. Although designed to support
...
\susan{It isn't obvious what 'this' refers to.}\kjx{fixed}
\Chainmail\ differs from Swasey and Schaefer's work in a number of ways. 
\citet{ddd} and \citet{schaeferCbc} are primarily concerned about
...
\citet{dd}. have deployed rather more complex
...
\kjx{not sure how to handle our previous non-publications here:
  ignore most of them and if anyone complains, claim double-blind privilege?}
