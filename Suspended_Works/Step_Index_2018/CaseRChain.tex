			...
			\\
                          ...
                          %    \strut \hspace{2.95cm} 
                              Call(\prg{p}',\prg{e}.\prg{transferFrom}(\prg{p},\prg{p}'',\prg{m}''),\_ ) \ \ \ )  \ \ \\ 
                               ...
                               \mbox{ \hlc[yellow]{In the previous step, {\tt p'} was authorized for {\tt m'}, and  transferred  on behalf of {\tt p}}} \\
                                 %  \strut \hspace{1.0cm} 
...
\\
 \strut \hspace{0.2cm} 
 \rightarrow  \mbox{ \hlc[green]{ {\tt c1}, {\tt c2} had sufficient funds \& transfers between them took place, other {\tt ValidClient} not affected}}\ \ ] \  \ \ \wedge \\
\ensuremath{ [ } \ \  \prg{res}=\prg{true}  \  \wedge\  \neg( \prg{c1}\ {\bf obeys}\ \prg{ValidClient}) \   \wedge\ \neg( \prg{c2}\ {\bf obeys}\ \prg{ValidClient} )\\ 
  \strut \hspace{0.2cm} 
 \rightarrow  \mbox{ \hlc[green]{{\tt c1}, {\tt c2} may be affected, but all {\tt ValidClient} not affected}} \ \ ] \  \ \ \wedge
 \\
\ensuremath{ [ } \ \    \prg{res}=\prg{false}  \wedge\, \prg{c1}\ {\bf obeys}\ \prg{ValidClient} \,   \wedge\, \prg{c2}\ {\bf obeys}\ \prg{ValidClient} \\
 \strut \hspace{0.2cm} 
 \rightarrow  \mbox{ \hlc[green]{ {\tt c1} or {\tt c2} had insufficient funds,  {\tt c1} and {\tt c2} and  all  {\tt ValidClient} not affected}}\ \ ] \  \ \ \wedge
 \\
\ensuremath{ [ } \ \     \prg{res}=\prg{false}\  \wedge\  (\  \  \prg{c1}\ {\bf obeys}\ \prg{ValidClient}  \   \veebar\   \prg{c2}\ {\bf obeys}\ \prg{ValidClient}\ \  ) \\
  \strut \hspace{0.2cm} 
 \rightarrow    \mbox{ \hlc[green]{ all  {\tt ValidClient} not affected}}\ \ ]  
\end{array}$ 
\end{minipage}
}
\label{DefEscrow}
\caption{Sketch of Escrow specification  --  \hlc[green]{green part left informal}}                     
 \end{figure*}                         
 % \vspace{.09cm}  
}
...
\author{ 
  \small{S. Drossopoulou \& S. Eisenbach, Imperial College London}  \\
}
...
Traditional spec. %ification 
languages do not adequately address robustness.
Designed for the {\em closed} world,
...
they attach pre- and post- conditions to each function of a contract, and thus
 give {\em sufficient} conditions for some effect to take place. They  are {\em explicit} about 
...
all functions.
 ...
 for effects, and are {\em explicit} about  contracts' overall
behaviour. %   as a {\em whole}.
...
\noindent
In \cite{holistic} we  outline   \RoSpec~and   
specify robustness aspects of  popular  
patterns from object-capabilities~\cite{MillerPhD} and smart contracts: 
membrane~\cite{membranesJavascript}, %~\cite{membranesJavascript,membranes}, 
...
This project aims to develop tools to express and ascertain   robustness of RChain contracts. 
  % In a one year one Research Associate effort, we 
 % We plan the 
% following 
Deliverables:
...
\begin{description}% [topsep=0pt,itemsep=-1ex,partopsep=1ex,parsep=1ex]
 \setlength{\itemsep}{0pt}
  \item {3rd Month}: Complete formal semantics of  \RoSpec~ for $COO_0$, a minimal object-oriented-contract calculus,
 
   \item {6th Month}:  Tool for  \RoSpec~ validation of $COO_0$ contracts through model checking~~\cite{testing}, and corresponding
  test suite.
  \item {9th Month}:   Adaptation  of  previous test suite to  Rholang, and new tests. Extend  \RoSpec~ semantics for  Rholang:  cater for channels, patterns, and    asynchroneity.  
      \item {12th Month}:   Tool for \RoSpec~ validation of $RL_0$ contracts through model checking. 
 \end{description}
 \vspace{-6pt}
 
 \noindent
% We collaborate with S. Eisenbach (Imperial),  J. Noble (VU Wellington) and 
% T. Murray (Univ. Melbourne).
