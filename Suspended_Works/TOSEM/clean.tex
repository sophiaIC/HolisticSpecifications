...
\usepackage{xspace, wrapfig, graphicx, enumerate}
\usepackage{paralist}
...
\definecolor{offblue}{rgb}{0.1,0.3,0.79}

% The following *should* be unneccesary, but does fix the font 
...
\newcommand{\mrr}[2][]{{#2}\xspace}
\newcommand{\mrrz}[1]{{#1}\xspace}
\newcommand{\Mrr}[2][]{{#2}\xspace}
\newcommand{\Mrrz}[1]{{{#1}}\xspace}
...
\keywords{Holistic specifications, open-world settings, robust programs, necessary conditions}

\setcopyright{acmcopyright}

\begin{abstract}
Functional specifications describe what program components \emph{can} do: the \emph{sufficient} conditions to invoke a
component’s operations. They allow us to reason about components in a \emph{closed world} setting,
where known client code must establish the appropriate
pre-conditions before calling into a component.

Sufficient conditions are not enough to reason about the use of components in an \emph{open world} setting, where
components interact with external code, possibly of unknown provenance. In this open world setting, ensuring
that your component is robust even when executing with buggy or malicious external code is critical. We
propose \emph{holistic specifications}, which are concerned with the \emph{overall} behaviour of a
component, in all possible interleavings of calls to the component’s operations with those of external code.
In addition to sufficient conditions, holistic specifications are concerned with \emph{necessary} conditions, \ie conditions 
without which an effect will \emph{not} happen.

In earlier work we presented the \Chainmail specification language. This paper improves on
the design of the formal foundations, contains a more extensive account of \Chainmail’s features, including
both semantics and case studies. It also reports on the mechanisation of the proofs of several properties of
\Chainmail, along with several of the case studies’ adherence to their holistic specifications.
...
%\scd{We propose  \emph{holistic specifications}, which,}
%% We need to say that we propose them, as this is only the 2nd paper on Holistic Specs
...
%\scd{In addition to sufficient conditions} which describe what is enough to \emph{cause} some effect,
% holistic specifications are concerned with
%% with \emph{sufficient} conditions, \ie
%%what is enough to \emph{cause} some effect, as well as with
%\emph{necessary} conditions, \ie
% the conditions without which an effect will \emph{not} happen. 
%%
%
%In earlier work we \scd{presented} % proposed used in the earlier paragraph
% the \Chainmail specification language \cite{FASE}.
% The current paper improves on the design of the formal foundations, contains a more extensive account of \Chainmail's features, 
%and includes  semantics as well as case studies. \scd{It also reports on the mechanisation of the proofs of several properties of
% \Chainmail, as well as several of the case studies' adherence to their holistic specifications.}
% %\scd{This} mechanisation of proofs has led us to simplify some of our original design decisions. \end{abstract}
\end{abstract}
