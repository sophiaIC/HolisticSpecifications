...
It is important to clarify what we mean by ``access''. We use``access'' in the intransitive meaning:  $\CanAccess x y$ means that either \prg{x} has a field which points to \prg{y}, or that \prg{x}  is the receiver in the executing method call, and \prg{y} is one of the arguments. 
Moreover, as we said in the introduction,\footnote{TODO: make sure what we do say it} we 

We now outline how the above 
