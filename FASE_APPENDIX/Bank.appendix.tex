...
In this section we revisit the \prg{Bank}/\prg{Account} example from
 ...
 implementations, derived from Noble et al.\ 
\sophia{ in \textit{Abstract and Concrete Data Types vs Object Capabilities}, in Springer's Festschrift for Arnd Poetzsch-Heffter.}
 %\cite{arnd18} . 
 Both implementations  satisfy the three functional specifications and the holistic assertions
 (1), (2) and (3)  shown in section 3 % HARD \ref{sect:motivate:Bank}.
 The first version gives rise to runtime configurations as $\sigma_1$, 
 shown on the left side of Fig.2 % HARD  \ref{fig:BankAccountDiagrams}, 
 while the
 second version gives rise to runtime configurations as $\sigma_2$,
 shown on the right side of Fig. 2 % HARD  \ref{fig:BankAccountDiagrams}. 
 in the main text.
