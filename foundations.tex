...
Note that field read or write is only allowed if the target object is \prg{this} -- as, \eg, can
...
A basic form of assertion only reflects over the current runtime configuration. These are described in Definition \ref{def:simple_assertion}.
 ...
 ~ \\
  \SA\ &\ \  ::=\ \  & \SE  \ \mid \ \SE > \SE \ \mid \  \SE=\SE  \ \mid \ \SE \equiv \SE \ \mid  \ \SA \rightarrow \SA \ \mid \   \SE:\prg{ClassId}  % \ \mid \       \SE\in\prg{S} 
  \\  & &   
\ \mid \   \exists \prg{x}.\SA  \ \mid \ % \exists \prg{S}:SET.\SA 
...
The validity of basic assertions can be judged in the context of a runtime configuration
...
\item
$\sigma \models  \exists \x.\SA$ \IFF % there exists some  $\alpha\in dom(\sigma)$,  and  \prg{z} free in $\sigma$ and $\SA$, so that\\
%$\strut \hspace{1.1in} $ 
$\sigma[\prg{z}\mapsto \alpha] \models  \SA[\prg{x}/\prg{z}]$, for some $\alpha\in dom(\sigma)$,  and  \prg{z} free in $\sigma$.
\item
...
Basic assertions form a classical logic: TODO: check the term; also to check whether we need any more from the classical logic's properties, since the rest of them probably hold by definition.

Using this small language of basic assertions we can encode all the classical connectives, \ie $\wedge$, and $\vee$ and $\neg$, and we can also encode universal quantifiers.

\begin{lemma}[Basic Assertions are classical]
