...
$\textbf{specification}$  Purse  {
    ...
    $\textbf{with}$  $\forall \prg{p}:\prg{Object}.[\ \ \prg{this.CanTrade(p)} \ \longrightarrow \  \obeys {\prg{p}} {\prg{Purse}}\ \wedge \  \prg{p.CanTrade(this)} \  ]$
          $\wedge$   
          $\forall \prg{p1},\prg{p2}:\prg{Object}[\ \ \prg{this.CanTrade(p1)}\ \wedge\  \prg{p1.CanTrade(p2)}\ \longrightarrow \  \prg{this.CanTrade(p2)}\ ]$  
    ...
    $\textbf{scoped-invr}\  \forall \prg b:\prg{int}.[\ \inside{\prg{this}}  \ \wedge \prg{this}.\prg{balance}=\prg b \ ]$ 
                $\wedge$
                $\forall \prg{o}:\prg{Object}.[\  \prg{this.CanTrade(o)}  \ ]$       
                // QUESTION: Do we also want $\forall \prg{o}:\prg{Object}.[\ \neg\,  \prg{this.CanTrade(o)}  \ ]$
      
    true  
    // implicit $\obeys {\prg{this}} {\prg{Purse}}$
         $\textbf{\{}$ this.transfer(p,amt) $\textbf\}$ : bool
    $\forall$ o:Object. [ $\obeys {\prg{p}} {\prg{Purse}}\, \wedge$ this$\neq$o$\neq$p   $\longrightarrow$  o.balance=o.balance$_{pre}$  ]
    // QUESTION: discuss the above!
    $\wedge\  [$ res $\wedge$  this.CanTrade(p) // implicit $\obeys {\prg{p}} {\prg{Purse}}$
        $\vee$
        $\neg$res $\wedge$ this.balance=this.balance$_{pre}$ $\wedge$ p.balance=p.balance$_{pre}$
             $\wedge$ [ this=p $\vee$ this.balance$_{pre}$ $<$amt $\vee$ $\neg(\, \prg{this.CanTrade(p)}\,)$ ]

    this$\neq$p $\wedge$ this.balance$\geq$amt $\wedge$  this.CanTrade(p)  
    //  implicit  $\obeys {\prg{this}} {\prg{Purse}}\, \wedge\, \obeys {\prg{p}} {\prg{Purse}}$
         $\textbf{\{}$ this.transfer(p,amt) $\textbf\}$ : bool
    res $\wedge$ this.balance=this.balance$_{pre}$-amt $\wedge$ p.balance=p.balance$_{pre}$+amt 
...
A possible implementation of \prg{Purse} appears below. The \prg{Purse}s have a field storing their Mint and their balance
