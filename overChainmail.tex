...
taking $\sigma_4$ to be similar to  $\sigma_2$ and so that the next statement to be executed 
 is  $\prg{a}_2.\prg{deposit}{\prg{(}}\prg{a}_3,\prg{360}{\prg{)}}$ and so that
...
\sd{logical connectors and classical assertions over the contents of the heap and the stack}, 
our holistic assertions draw from some concepts from object capabilities
($\CanAccess{\_}{\_}$  for  permission; \sd{$\Calls {\_} {\_} {\_} {\_}$ and}  $\Changes{\_}$ for
authority) 
...
contains an overview of the formal model; its most important aspect being assertions, whose semantics 
are in section  \ref{sect:assertions}.
} 
