...
\\
If   $\sigma_1$ was currently executing  a call like ${\prg{a}_2}$.\prg{deposit}\prg{(}${\prg{a}_3}$,\prg{360}\prg{)}, then we 
would  have\\
 ...
 Namely, during execution of \prg{deposit}, $\prg{a}_2$ has access to $\prg{a}_3$, and could,
  if the method body chose to,  call methods on$ \prg{a}_3$ , or  store a reference to $\prg{a}_3$ in its own fields.
...
if $\SF_1$ includes object \prg{94}, and  $\SF_2$ does not include  it, then we   have\\ 
 ...
 $\Using{\A}{\SF}$  is therefore {\em not} the footprint of the assertion $\A$;
  it is more like the \emph{fuel}\cite{stepindex}  given to establish that assertion. Note that $\Using {\A} {\SF}$ does not imply  
...
then the balance of $\acc_2$ will eventually change:\\
 ...
 % \sophia{some delicate issues with $\sigma_2$ as used in prev. paragraph, where we already assumed that
