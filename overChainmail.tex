...
%
%
\Chainmail\ incorporates assertions   about \susan{the intro mentions viewpoint as well - the two should be consistent}
%
\textit{access},
...
\susan{I don't see what the code adds - these are completely  straightforward to imagine}
...
%  $\strut$ \hspace{1.1cm} 
 \hspace{1cm}   $...,\sigma_2 \ \models \ \prg{a}_2.\prg{myBank}=\prg{a}_3.\prg{myBank}$.
  ...
  We support ghost fields~\cite{Leavens-etal07}, \sophia{TODO citation}\susan{this is the jml reference manual - if you want something else say}
...
%
%
\sdparagraph{In summary,} \se{in addition to classical logical concepts}, our holistic assertions draw from some concepts from object capabilities
($\CanAccess{\_}{\_}$  for  permission and $\Changes{\_}$ for
authority) 
as well as temporal logic ($\Future \A$, $\Past \A$ and friends), and the relation of
our spatial connective ($\Using{\A}{S}$)  with ownership and effect
