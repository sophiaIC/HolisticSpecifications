...
\\
If   $\sigma_1$ was currently executing  a call like ${\prg{a}_2}$.\prg{deposit}\prg{(}${\prg{a}_2}$,\prg{100.00}\prg{)}, then we 
would  have\\
...
the access assertion" -- that is a nice simile, but is it true?}  holds 
...
if $\SF_1$ includes object \prg{94}, and  $\SF_2$ does not include  it, then we   have\\ 
 ...
 citation to Amal's fuel} given to establish that assertion. Note that $\Using {\A} {\SF}$ does not imply  
...
$\sigma_2(\prg{94},\f_2)=\prg{93}$, and calling   \prg{m2} on \prg{93} returns \prg{4},
 ...
 $\strut$ \hspace{2cm}  $\M_{BA2}\mkpair ..., \sigma_2 \ \models \ \Future{ \Changes {\acc_2.\bal}}$.\sophia{It would
...
then the balance of $\acc_2$ will eventually change:\\
 $\strut$ \hspace{2cm}  $\M_{BA2}\mkpair ..., \sigma_2 \ \models \ {\Calls {..} {\pu_{94}} {\m} {()}}  \longrightarrow \Future{ \Changes {\acc_2.\bal}}$.\sophia{some delicate issues with $\sigma_2$ as used in prev. paragraph, where we already assumed that
