...
ensures interface objects ("low values") are safe to share with untrusted code,
 \sdN{in the sense that untrusted code cannot use them to break any internal invariants of the encapsulated object}. 
...
construction. 
 \sdN{Concerns like \SrobustB are not, we argue, within the scope of these works.}
...
while \Nec users only need  to understand \sdNr[as Alex pointed out]{small extensions to} first order logic.
...
Like \Nec, VerX \cite{VerX} and  \emph{Chainmail} \cite{FASE} also work on problem-specific guarantees.
...
For example, VerX \cite{VerX} work on effectively call-back free contracts, 
