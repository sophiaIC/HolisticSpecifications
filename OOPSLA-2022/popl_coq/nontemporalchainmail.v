...
Module NonTemporalChainmail(L : LanguageDef).

  Import L.
  Module L_Semantics := AbstractOperationalSemantics(L).
  Import L_Semantics.

  Declare Scope non_temporal_chainmail_scope.
  ...
  Notation "'a♢' n" := (av_hole n)(at level 40) : non_temporal_chainmail_scope.
  Open Scope non_temporal_chainmail_scope.
  Notation "'av_' v" := (av_bnd v)(at level 40) : non_temporal_chainmail_scope.
  Notation "'v_' α" := (v_addr α)(at level 39) : non_temporal_chainmail_scope.
  Notation "'a_' α" := (av_bnd (v_ α))(at level 40) : non_temporal_chainmail_scope.
  ...
  Notation "'am_' m" := (am_bnd m)(at level 40) : non_temporal_chainmail_scope.
  Notation "'am♢' m" := (am_hole m)(at level 40) : non_temporal_chainmail_scope.
  ...
  Notation "A1 '⟶' A2" := (a_arr A1 A2)(at level 40) : non_temporal_chainmail_scope.
  Notation "A1 '∧' A2" :=(a_and A1 A2)(at level 40) : non_temporal_chainmail_scope.
  Notation "A1 '∨' A2" :=(a_or A1 A2)(at level 40) : non_temporal_chainmail_scope.
  Notation "'¬' A" :=(a_neg A)(at level 40) : non_temporal_chainmail_scope.
  Notation "'∀[x⦂' T ']∙' A" :=(a_all T A)(at level 40) : non_temporal_chainmail_scope.
  Notation "'∃[x⦂' T ']∙' A" :=(a_ex T A)(at level 40) : non_temporal_chainmail_scope.
  Notation "'⦃x⃒' A '⦄'" := (as_asrt A)(at level 40) : non_temporal_chainmail_scope.
  Notation "e '∈' Σ" := (a_elem e Σ)(at level 40) : non_temporal_chainmail_scope.
  Notation "A 'in_' Σ" := (a_in A Σ)(at level 40) : non_temporal_chainmail_scope.
  Notation "x 'internal'" :=(a_intrn x)(at level 40) : non_temporal_chainmail_scope.
  Notation "x 'external'" :=(a_extrn x)(at level 40) : non_temporal_chainmail_scope.
  Notation "x 'access' y" :=(a_acc x y)(at level 40) : non_temporal_chainmail_scope.
  Notation "x 'calls' y '◌' m '⟨' vMap '⟩'" :=(a_call x y m vMap)(at level 40) : non_temporal_chainmail_scope.
...
Instance substΣA : Subst asrt nat (set a_val) :=
  ...
  Hint Constructors has_type : non_temporal_chainmail_db.
  ...
  Hint Constructors has_access_aval : non_temporal_chainmail_db.
  ...
  Hint Constructors makes_call : non_temporal_chainmail_db.
  ...
  Hint Constructors internal_obj external_obj : non_temporal_chainmail_db.
  ...
  Hint Constructors has_class is_self exp_satisfaction : non_temporal_chainmail_db.
  ...
  Hint Constructors elem_of_set : non_temporal_chainmail_db.
  ...
  Hint Constructors restrict : non_temporal_chainmail_db.
  ...
  Reserved Notation "M1 '⦂' M2 '◎' σ0 '⊨' A"(at level 40).
  Reserved Notation "M1 '⦂' M2 '◎' σ0 '⊭' A"(at level 40).
...
[[[
  ...
  | sat_all : forall M1 M2 σ T A, (forall x, has_type σ x T ->
                                   M1 ⦂ M2 ◎ σ ⊨ (⟦x /s 0⟧A)) ->
                             M1 ⦂ M2 ◎ σ ⊨ (∀[x⦂ T ]∙ A)
  (**
[[[
  ...
  | sat_ex  : forall M1 M2 σ x T A, has_type σ x T ->
                               M1 ⦂ M2 ◎ σ ⊨ (⟦x /s 0⟧ A) ->
                               M1 ⦂ M2 ◎ σ ⊨ (∃[x⦂ T ]∙ A)
  (**
[[[
  ...
  (** Permission: *)
  | sat_access : forall M1 M2 σ a1 a2, has_access_aval M1 M2 σ a1 a2 ->
...
[[[
  ...
  (** Control: *)
  | sat_call : forall M1 M2 σ a1 a2 m β,
...
[[[
   ...
   *)
  (** Viewpoint: *)
  | sat_extrn : forall M1 M2 σ a, external_obj M1 M2 σ a ->
                             M1 ⦂ M2 ◎ σ ⊨ a_extrn a
  (**
[[[
  ...
  | sat_intrn : forall M1 M2 σ a, internal_obj M1 M2 σ a ->
                             M1 ⦂ M2 ◎ σ ⊨ a_intrn a
  (**
[[[
...
#<h3>#Space#</h3>#
   *)
  | sat_elem : forall M1 M2 σ e Σ, elem_of_set M1 M2 σ e Σ ->
                              M1 ⦂ M2 ◎ σ ⊨ (e ∈ Σ)
  ...
  | sat_in : forall M1 M2 χ ψ χ' A Σ,
      restrict Σ χ χ' ->
      M1 ⦂ M2 ◎ (χ', ψ) ⊨ A ->
      M1 ⦂ M2 ◎ (χ, ψ) ⊨ (A in_ Σ)

  where "M1 '⦂' M2 '◎' σ0 '⊨' A" := (sat M1 M2 σ0 A) : non_temporal_chainmail_scope

  with
    nsat : mdl -> mdl -> config -> asrt -> Prop :=

  | nsat_self : forall M1 M2 σ a, ~ is_self σ a ->
                             M1 ⦂ M2 ◎ σ ⊭ a_self a
...
[[[
  ...
  | nsat_class : forall M1 M2 σ a C, ~ has_class σ a C ->
                                M1 ⦂ M2 ◎ σ ⊭ (a_class a C)
  (**
[[[
                        (α ↦ o) ∈ σ     
...
[[[
  ...
  (*quantifiers*)
  | nsat_all : forall M1 M2 σ A x T, has_type σ x T ->
                                M1 ⦂ M2 ◎ σ ⊭ (⟦x /s 0⟧A) ->
                                M1 ⦂ M2 ◎ σ ⊭ (∀[x⦂ T ]∙ A) 
  (**
[[[
  ...
  | nsat_ex : forall M1 M2 σ T A, (forall x, has_type σ x T ->
                                   M1 ⦂ M2 ◎ σ ⊭ (⟦x /s 0⟧A)) ->
                             M1 ⦂ M2 ◎ σ ⊭ (∃[x⦂ T ]∙ A)
  (**
[[[
  ...
  (** Permission: *)
  | nsat_access : forall M1 M2 σ a1 a2, ~ has_access_aval M1 M2 σ a1 a2 ->
                                   M1 ⦂ M2 ◎ σ ⊭ (a1 access a2)
  (**
[[[
...
[[[
  ...
  | nsat_extrn : forall M1 M2 σ a, ~ external_obj M1 M2 σ a ->
                              M1 ⦂ M2 ◎ σ ⊭ a_extrn a
  (**
[[[
                       α ∉ σ.(heap)
  ...
  | nsat_intrn : forall M1 M2 σ a, ~ internal_obj M1 M2 σ a ->
                              M1 ⦂ M2 ◎ σ ⊭ (a internal)
  (**
[[[
                       α ∉ σ.(heap)
...
| nsat_elem_comp3 : forall M1 M2 M σ0 σ e e' A,
  ...
  | nsat_in1 : forall M1 M2 χ ψ A Σ, (forall χ', ~ restrict Σ χ χ') ->
                                M1 ⦂ M2 ◎ (χ, ψ) ⊭ (A in_ Σ)

  | nsat_in2 : forall M1 M2 χ χ' ψ A Σ, restrict Σ χ χ' ->
                                   M1 ⦂ M2 ◎ (χ', ψ) ⊭ A ->
                                   M1 ⦂ M2 ◎ (χ, ψ) ⊭ (A in_ Σ)

  where "M1 '⦂' M2 '◎' σ '⊭' A" := (nsat M1 M2 σ A) : non_temporal_chainmail_scope.
  ...
  Hint Constructors sat nsat : non_temporal_chainmail_db.
  ...
  Notation "M '⊨m' A" := (mdl_sat M A)(at level 40) : non_temporal_chainmail_scope.
  ...
  Hint Constructors entails : non_temporal_chainmail_db.
  ...
  Notation "a '↑' n" := (raise a n)(at level 40) : non_temporal_chainmail_scope.
  ...
  Close Scope non_temporal_chainmail_scope.
  Close Scope reduce_scope.

End NonTemporalChainmail.
