...
Consider the code snippets from fig. \ref{fig:ExampleBank}. Objects of
 ...
 and that addresses are unforgeable, and so there is no way to guess an account.}
...
%The specification from above mandates 
 \prg{deposit} cannot create money from nowhere.\footnote{We take that there must be no effects if (ClassicSpec)'s precoditions aren't satisfied. This is simple to state in a holistic specification, but has been omitted for brevity.}
%
But it cannot preclude that \prg{Account} -- or some other class, for that matter -- contains more methods 
which might make it possible to create "free" money\mrr[; in fact it would be impossible to express such restriction]{Request from Prof. S.D.: Introduction---Mention how ClassicalSpec doesn't preclude \prg{freeMoney}}. For this, we introduce \emph{holistic specifications}, and require that:
...
We relegate various details to appendices. An earlier version of this paper appeared at FASE 2020. 

\mrr[This paper introduces enhancements to assertions, including linear temporal logic and simplified name binding semantics. The main extensions are the problem-driven design, the specification of \LangOO, and the further discussion of the Bank (appendices are not part of the FASE paper), as well as changes to the introduction.]{Is this a good summary, is "enhancements" too strong?} \mrr[The FASE paper focused on a safe example in the introduction before discussing the larger bank example. However, further research found the safe's ability to hold arbitrary objects added complexity that was unnecessary for a simple introduction. Instead this paper builds up the bank example, which has been proven in the Coq model.]{} \mrr[The rest of these changes are discussed where relevant throughout the paper.]{I've left it like this for now, with the changes highlighted throughout the paper. If you want a more centralised discussion, that can be done instead.}
