...
Consider the code snippets from fig. \ref{fig:ExampleBank}. Objects of
 ...
 has an additional method \prg{add}.
 ...
 and that addresses are unforgeable, and so there is no way to guess a secret.}
...
(ClassicSpec)  expresses  that knowledge of the \prg{src} account, and the accounts sharing the same bank, is \emph{sufficient}, %condition 
...
%The specification from above mandates 
 \prg{deposit} cannot create money from nowhere. 
%
...
(HolisticSpec) mandates that for any change in the value of an account, the deposit method must have been called on it, or it must have been the source to another accounts deposit. This prevents the \prg{add} method above, where \prg{a.balance} changes, but the deposit method isn't called. Further holistic propositions are discussed in section~\ref{sect:motivate:Bank} to provide other protections, such as against the leaking of accounts.
