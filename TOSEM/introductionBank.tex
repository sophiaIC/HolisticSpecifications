...
which might make it possible to create "free" money, \Mrr[Is this explicit enough?]{as shown with \prg{freeMoney} on the right in fig.~\ref{fig:ExampleBank}, a situation any sensible bank implementation should avoid.}\mrr[In fact it would be impossible to express such a restriction using a classical specification.]{Request from Prof. S.D.: Introduction---Mention how ClassicalSpec doesn't preclude \prg{freeMoney}} For this, we \Mrr[As per S.D. request]{propose} \emph{holistic specifications}, and require that:
...
An earlier version of this \Mrr[As per S.D.]{work} appeared at FASE 2020. \mrrx[Is this a good summary, is "enhancements" too strong?]{This paper introduces enhancements to assertions, including \Mrr[As per S.D.]{linearising the logic for temporal connectives} and simplifying the name-binding semantics. The main extensions are the problem-driven design, the specification of \LangOO, and the further discussion of the Bank (appendices are not part of the FASE paper), as well as changes to the introduction.}\Mrr{This paper works through the Bank example, whereas previous work began with a safe, which is discussed further in section~??TODO??}. \mrr[The rest of these changes are discussed where relevant throughout the paper.]{I've left it like this for now, with the changes highlighted throughout the paper. If you want a more centralised discussion, that can be done instead.}

\mrr[\sout{The FASE paper focused on a safe example in the introduction before discussing the larger bank example. However, further research found the safe's ability to hold arbitrary objects added complexity that was unnecessary for a simple introduction. Instead this paper builds upon the bank example, which has been proven in the Coq model.}]{}
