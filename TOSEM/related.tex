...
specify preconditions under which specific errors and program states will result. 
Incorrectness Logic provides a sound and compositional way to reason about 
the presence of bugs rather than the absence of bugs. Incorrectness Logic differs in intent to \Chainmail as the exemplars of Section \ref{sect:problemdriven} 
are primarily concerned with correctness rather than incorrectness.
The similarity between the two systems is in the structure of proofs.
Incorrectness Logic and \Chainmail are
both concerned with which program states are necessary preconditions to 
specific results, as opposed to which results might arise from 
those preconditions. As an example, the Bank Account of
Section \ref{sect:motivate:Bank} specifies that if a program state
is ever reached where the balance of an account has been changed, then it came as 
the result of a call to the \prg{deposit} method. Such specifications are
not possible in traditional Hoare Logic. Paraphrasing O'Hearn, the post condition
of a Hoare triple ``\emph{over-approximates}'' the set of reachable states, while
a reverse Hoare triple ``\emph{under-approximates}'' the set of reachable states.
An over-approximation implies that there are other initial states that might
give rise to the same result, while an under-approximation implies that there
are other results that might arise from the same precondition.
}

\jm{
O'Hearn developed a proof system for Incorrectness Logic, and proved
the system sound and complete. This work is heartening from the perspective 
of \Chainmail as it provides a formal background to future extensions
to the ideas in this paper, albeit in a different problem domain.
}
