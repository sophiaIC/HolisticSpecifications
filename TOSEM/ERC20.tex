...
\strut \hspace{0.3cm} $[\ \ \prg{e}.\prg{balance(p)}=\prg{m}+\prg{m}^\prime\ \wedge \ \Next{\prg{e}.\prg{balance(p)}=\prg{m}'}$ \\ %.\forall\prg{m}:\prg{Nat}.$\\
...
That is to say: if next configuration witnesses a decrease of \prg{p}'s balance by
 ...
 \prg{p}, or  a call of \prg{transferFrom} instigated by somebody authorised by \prg{p}.
...
and did not transfer any of \prg{p}'s tokens, nor did \prg{p} issue a fresh authorisation,
or c) \prg{p} was authorised for $\prg{m}+\prg{m}'$ and spent $\prg{m}'$. 
...
\jm[dropped: In the general case, \eg the DAO, the number of   functions which are unrelated
