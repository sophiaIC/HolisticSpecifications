...
We can now answer the question: $\M \models \A$ 
 holds if for all further, {\em potentially adversarial}, modules $\M'$ and in  all runtime configurations $\sigma$ which may be observed as arising from the \mrr{initial configuration $\sigma_0$ and the} execution of the code of $\M$ combined with that of $\M'$, the assertion $\A$ is satisfied. More formally, we define:\\
\sout{$~ \strut  \hspace{1.1in} \M \models \A \ \ \  \ \ \ \ \ \mbox{
if               } \ \ \  \ \ \  \  \forall \M'.\forall \sigma\in\Arising
{\M \mkpair  {\M'}}. [\ \M \mkpair  {\M'},\sigma \models \A\ ]$.} \\
\\
\jm{\\$~ \strut  \hspace{1.1in} \M \models \A \ \ \  \ \ \ \ \ \mbox{
if               } \ \ \  \ \ \  \  \forall \M'.\forall \sigma_0.\ \forall \sigma\in\Arising{\M \mkpair  {\M'}, \sigma_0}. [\ \M \mkpair  {\M'}, \sigma_0 , \sigma \models \A\ ]$. } \\
%\jm{\\$~ \strut  \hspace{1.1in} \M \models \A \ \ \  \ \ \ \ \ \mbox{
...
%Satisfaction is also  judged in the context of modules; t
Modules contain the definitions of ghost fields and method bodies \Mrr[I have kept this here for now, but the discussion today (Friday) made it sound like we want to remove it]{the latter of which is needed to judge the validity of temporal assertions such as $\Future {\_}$.} We distinguish between internal and external modules. This %offers two advantages:
\sophia{has two uses:}
First, 
...
\Mrr[Cleaned this up to keep it clear why we need the initial configuration, and it can seem like "what am I missing", making it less accessible. Then also added "differs from previous papers".]{The initial configuration \mrr{$\sigma_0$} is required to constrain the past execution to one possible path. Since the language \LangOO is deterministic, the initial state is sufficient to determine the exact steps which occurred. For the same reason, the future is also predetermined, maintaining linearity in both directions within the temporal logic. This provides the expected result that \jm[Julian can you please fill this in]{??TODO which property was it Julian that this gave us??}. \Mrr{Explicitly stating initial configurations to linearise the temporal connectives differs from previous work on \Chainmail}\cite{FASE}, where only the current configuration was specified, and every possible path to it was considered.} 
