...
%Satisfaction is also  judged in the context of modules; t
\mrr[Modules contain the definitions of ghost fields and method bodies, ]{} \Mrr[I have kept this here for now, but the discussion today (Friday) made it sound like we want to remove it]{the latter of which is needed to judge the validity of temporal assertions such as $\Future {\_}$.} \Mrr[Cleaned this up to keep it clear why we need the initial configuration, and it can seem like "what am I missing", making it less accessible. Then also added "differs from previous papers".]{The initial configuration is required to constrain the past execution to one possible path. Since the language \LangOO is deterministic, the initial state is sufficient to determine the exact steps which occurred. For the same reason, the future is also predetermined, maintaining linearity in both directions within the temporal logic. This provides the expected result that \jm[Julian can you please fill this in]{??TODO which property was it Julian that this gave us??}. \Mrr{Explicitly stating initial configurations to linearise the temporal connectives differs from previous work on \Chainmail}, where only the current configuration was specified, and every possible path to it was considered.}
