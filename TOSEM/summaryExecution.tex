...
\begin{figure}[htb]
  ...
  \begin{center}
   ...
   \Description{Three rows of circles coloured blue and organge. In the top row are 9 circles labelled 1 through 9. Some are coloured blue, and some are coloured orange. In the next row, only the orange circles from the first are shown. In the final row, only the blue circles. Arrows connect successive circles in each row.}
 \end{figure}
...
\sophia{{\em Initial configurations} are those whose heap  have only one object, of class \prg{Object}, and whose stack} have one frame, with arbitrary continuation.
\sophia{{\em Arising}} configurations are  those that can be reached by two-module execution, starting from \jm{a specific} initial configuration to maintain temporal linearity.
...
\sout{ $\Arising  {\M\mkpair\M'} \ = \ \{ \ \sigma \ \mid \ \exists \sigma_0. \ [\  \Initial{\sigma_0} \  \ \wedge\ \  \M\mkpair\M', \sigma_0 \leadsto^* \sigma \ \ ] \ \ \} $}
 \item
 \jm{$\Arising  {\M\mkpair\M', \sigma_0} \ = \ \{ \ \sigma \ \mid \ \Initial{\sigma_0}\ \wedge\ \M\mkpair\M', \sigma_0 \leadsto^* \sigma \ \ \} $}
 \end{itemize}
