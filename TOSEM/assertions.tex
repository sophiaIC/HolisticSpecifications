...
%when giving necessary conditions for some future effect.
 ...
 In general, $\Using {\Future {\A}} {\SF}$ is different from
  $\Future {\Using {\A} {\SF}}$. In the former assertion, $\SF$ must contain
   the objects involved in reaching the future configuration \mrr[as well as the objects needed to
    then establish validity of $\A$ in that future configuration]{No it doesn't, this is wrong. It need only have the objects that are needed in creating that future. This means that objects in A, plus the objects created by the reduction, are used in A}. In the latter assertion, 
...
Whilst the above shows $\Future { \Using{A}{\SF} } \not\vdash \Using { \Future{A} }{\SF}$\mrr{Not sure which symbol to use for the consequence here, $\models$ doesn't match what is otherwise in the paper, and $\rightarrow$ seems like not a statement, but I don't know}, it is also worth-wile noting that in general the inverse doesn't hold either. This is since the former restricts $A$ in the future, meaning any objects created in the meantime aren't included, whereas in the latter, the restriction occurs in the present, so further objects created by reduction are considered in A. Taking  the continuation of a new configuration $\sigma_6$ to be \prg{x := new Account(100, null) } and otherwise similar to $\sigma_1$, and $\SF = \emptyset$, we cannot prove $\M_{BA1}\mkpair ..., \sigma_6 \ \models \ \Future{ \Using { x.bank = \prg{null} } {\SF}}$  as \prg{x} is not in $S$, however, we can prove $\M_{BA1}\mkpair ..., \sigma_6 \ \models \ \Using { \Future{ x.bank = \prg{null} } } {\SF}$, since the restriction of the configuration to $S$ doesn't prevent the next statement instantiating \prg{x}, and thus the object existing.
