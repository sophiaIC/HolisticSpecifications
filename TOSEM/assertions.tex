...
&    &  \Next \A  \ \mid \   \Future \A \ \mid \  \Prev \A   \ \mid \ \Past \A \ \mid \\  
 &    &        \Using \SF  \A  \ \mid \  \External \alpha \ \mid\ \Name \alpha \x \ \mid \\
 \\
 \x, \f, \m &\BBC&  \prg{Identifier}  ~ \\
\end{array}
\]
...
\mrrx[I have done this for now to discuss that it would need to be translated in general, however limit the scope of discussion given the limited circumstances. Also is there a more fundamental reason we are excluding quantifiers, or is it just for show?]{As they are embedded in program bodies, ghost fields more closely resemble the target language. For \LangOO, this means they are restricted to exclude quantifiers, and retain the ability to refer to local variables, either parameters or \prg{this}, directly, as would be expected in an expression. We thus define a translation function $f_{\mathcal G}$ to decode ghost field syntax into \Chainmail assertions.}


\begin{definition}[\mrrz{Ghost Field Translation}] \label{def:valid:assertion:view}
\mrrz{For any expression \e, we define}
\begin{itemize}
 \item
\mrrz{$f_{\mathcal G}(\e) = \forall {\alpha_{\x_1}}. \dots \forall \alpha_{\x_n}. (\Name{\alpha_{\x_1}}{\x} \wedge \dots \Name{\alpha_{\x_n}}{\x_n} \rightarrow  \e[\x_n/\alpha_{x_n}])
\IFF \\
 ~ \strut  \hfill\x_1$..$x_n$ are the variables in \e.}
\end{itemize}
\end{definition}
...
\phi = (\_, (\this\mapsto\alpha, \p_1\mapsto v_1, .... \p_n\mapsto v_n))\\  
  ...
  \M,\,(\phi, \chi), \SE'    \hookrightarrow_{\SAF}\   v  \hspace{1.2cm} \SE' = f_{\mathcal G}(\SE)
 }
{
...
$\M\mkpair \M',\sigma_0 , \sigma \models \SE\in \prg{S}$ \IFF there exists a value $v$ such that 
 $ \M,\,  \sigma, \SE \ \hookrightarrow\   v$, and $v \in \interp{\prg{S}}{\sigma}$.
\item
$\M\mkpair \M',\sigma_0 ,  \sigma \models\ \Name \alpha \x $ \IFF 
...
\in dom(\sigma)$.
\item
$\M\mkpair \M',\sigma_0 , \sigma \models \forall \prg{m}.\A$ \IFF
$\M\mkpair \M',\sigma_0 , \sigma \models  \A[\prg{m}/\prg{m}']$ \ for all method names  $\prg{m}'$.
\item
$\M\mkpair \M',\sigma_0 ,\sigma \models \exists \prg{m}.\A$ \IFF
$\M\mkpair \M', \sigma_0 , \sigma \models  \A[\prg{m}/\prg{m}']$\ for some  method name $\prg{m}'$.
\item
...
\noindent 
\mrr[Thus, $\Calls {\alpha}  {\prg{m}} {\alpha'}  {{\alpha_1,...\alpha_n}}$ expresses the there will be a call from $\alpha_1$ to the method \m\ on object $\alpha'$ , given objects $\alpha_1,...\alpha_n$. If these are named \y, and $\z_1,...\z_n$ respectively, then $\y.\m(\z_1,...\z_n)$ will be executed next, with the caller \this\ being $\alpha$.]{Updated to use addresses, further update to rename $v_k$ to $a_k$ as they can only be addresses in a call.}
