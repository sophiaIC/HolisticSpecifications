...
&
\inferenceruleN {\mrr[Addr\_Val]{Had to switch old \textit{Var_Val} rule for \textit{Addr_Val}}} {
\alpha : \text{Address}
}
{
...
all the previous configurations.} \mrr[Taking addresses and not names, and requiring them to be bound from a name in a given time, by keeping them inside or outside of a temporal assertion, allows us to avoid issues involving adapting the configuration to judge the validity of assertions in the future or past configurations, using the bindings from the current one.]{This read "validity of assertions in the future or the past needs to be judged in the 
