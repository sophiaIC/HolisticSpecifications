...
Continuing our earlier example, given \mrr["An" not general enough]{any} initial configuration $\sigma_0$,  
...
\hfill for all sets of addresses $R\subseteq dom(\sigma)$.
\item
$\M\mkpair \M',\sigma_0 ,\sigma \models \exists \prg{S}:\prg{SET}\!.\,\A$ \IFF  $\M\mkpair \M', \sigma_0  ,  \sigma \models  \A[\prg{S}/\prg{R}]$ \
 \hfill  for some set of addresses $R\subseteq dom(\sigma)$.
\item
...
\Mrr[New discussion moved further down and updated to follow changes. Once again I'm unsure what you want "previous work" things to look like?]{\Chainmail quantifies over addresses instead of local variables, thereby referring directly to objects. Since \Chainmail assertions are invariants over modules, local variable names have no consistent meaning. Previous work on \Chainmail exclusively used variables in assertions, including for quantification \mrr[Cited]{\cite{FASE}}. Aside from being less clear, as assertions generally refer to objects meeting some property, rather than the value at a certain name, using local variables also complicated the handling of temporal assertions, as discussed in section~\ref{sect:time}}.
...
\noindent 
\Mrr{Thus, $\Calls {\alpha}  {\prg{m}} {\alpha'}  {{\alpha_1,...\alpha_n}}$ expresses that there \mrr{is about to be} a call from $\alpha_1$ to the method \m\ on object $\alpha'$ , given objects $\alpha_1,...\alpha_n$. If these are named \y, and $\z_1,...\z_n$ respectively, then $\y.\m(\z_1,...\z_n)$ will be executed next, and the current caller \this\ is $\alpha$.}
...
\mrr[I cannot remember exactly what was desired here, so I have changed it to this for now. "all" of previous was seen as ...?]{each of} the previous configurations.} 
...
Finally, we address challenge (c) \mrr[To make it explicit this is why we introduced it]{by introducing an explicit initial configuration, and only considering configurations which arise from it and lead to the current configuration.}
}
