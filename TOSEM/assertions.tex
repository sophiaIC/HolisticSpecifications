...
\label{sect:time} 
\sd{To deal with time, we are faced with three challenges: (a) the current configuration needs to store the code being executed, so
...
We address challenge (b) by defining constrained reduction, where we only take the top of the frame when considering future executions.
...
\end{itemize}
\end{definition}
\jm{Constrained reduction (defined in Definition \ref{def:constrained_reduction}) is a form of reduction
that is restricted only to reduction steps that arise from the top frame of the stack, and does not consider 
reduction steps that result as a method return from the top frame. As stated above, this addresses challenge (b).
Constrained reduction is a strict subset of reduction, and for all modules $\M$ and $\M'$, and configurations $\sigma_1$ and $\sigma_2$, 
if $\M\mkpair \M',  \sigma_1\lceil\leadsto\rceil \sigma_2$ (or $\M\mkpair \M',  \sigma_1\lceil\leadsto*\rceil \sigma_2$), then 
it follows that $\M\mkpair \M',  \sigma_1 \leadsto \sigma_2$ (or $\M\mkpair \M',  \sigma_1\leadsto* \sigma_2$). 
The reverse however is not true. Reduction does not imply constrained reduction.
Since constrained reduction does not consider any frames in the stack preceding the top frame of $\sigma_1$, 
...
\jm{Constrained reduction is used in the satisfaction of the temporal operators $\Future{A}$ and $\Next{A}$, defined in Definition \ref{def:valid:assertion:time}.
This restriction is necessary for the definition the future temporal operators, as satisfaction of Chainmail assertions are generally 
concerned with the current state of the program, and are not able to peel back the stack to inspect underlying frames. As an example, 
...
If the reduction implied by future assertions were not constrained, this would not necessarily be true unless we inspected every frame in the stack to check access.
Further, the use of constrained reduction is necessary in maintaining some expected properties of the temporal operators. 
...
%when giving necessary conditions for some future effect.
 ...
 In general, $\Using {\Future {\A}} {\SF}$ is different from
  ...
  and $\SF_2$ to consist of objects \prg{1}, \prg{2},   \prg{4}.  Assume that 
   $\sigma_5$ is like $\sigma_1$, except that the next call in $\sigma_5$ is a method on $\prg{u}_{94}$, whose  body obtains the
...
Whilst the above shows that from $\Future { \Using{A}{\SF} }$ we cannot conclude $\Using { \Future{A} }{\SF}$, it is also worth noting that in general the inverse does not hold either. $\Future { \Using{A}{\SF} }$ restricts $A$ in the future, meaning any objects instantiated in the between the present and then are excluded, whereas $\Using { \Future{A} }{\SF}$ restricts the space in the present, so the objects are considered in A. Taking a new configuration $\sigma_6$  with continuation \prg{x := new Account(100, null)} and otherwise similar to $\sigma_1$, and $\SF = \emptyset$, the assertion $\M_{BA1}\mkpair ..., \sigma_0 , \sigma_6 \ \models \ \Future{ \Using { \prg{x.bank} = \prg{null} } {\SF}}$  does not hold, as \prg{x} is not in $S$. However, we can prove $\M_{BA1}\mkpair ..., \sigma_0 , \sigma_6 \ \models \ \Using { \Future{ \prg{x.bank} = \prg{null} } } {\SF}$, since the restriction of the configuration to $\SF$ in the present doesn't exclude the new \prg{Account} with field $\prg{bank} = \prg{null}$, created in the next statement, being considered by \Future{\_}.

Using local variables in a past or future context allows specifications to use their past or future values. For example, that at some point in the future the receiver will have access to an object at $\alpha$ can be asserted with $\forall r . \Future {\prg{this} = r \wedge \CanAccess{r}{\alpha}}$. However, to state some future or past property of the current value of a variable, it must be connected to its address in the present, $\forall \alpha_x. (\alpha_x = \x \rightarrow \dots)$. Local variables referring to their past or future values in temporal connectives differers from previous formalisations of \Chainmail, where past and future configurations were adapted to use the current configuration's bindings \cite{FASE}. This allowed past and future assertions to directly refer to current variable bindings, however, it prevented using the past or future value of a name, and required more a complicated formalisation for minimal benefit.
...
\subsection{\mrrz{Connectivity}}
\mrr[Not sure if this is where we want it, but just trying it for now]{
Many programs are only allowed to pass on objects under certain conditions. This is to keep these objects safe from code that shouldn't have access to them, such as malicious external modules, or even other internal objects that were not expecting them, and whose actions may have unintended consequences. In \Chainmail restricting access is expressible: assertions can ensure that objects which will need access have a means to gain it. Alternatively, they can ensure that when an object's state changes, some other object could have had access to cause that change.
}

\mrr{However, to ensure these assertions are sound, we need to ensure that code only has access to objects it is directly passed, thereby ensuring that checking direct connections is sufficient for checking these assertions. To check code only has access to what it is passed, we can ensure any method call---the barrier between \Mrr{the private state of different objects}---only \emph{connects} to what its parameters \Mrr{can obtain} access to. This can be expressed formally as:
}


\mrr[We don't need to create a separate ``connected'' predicate since we're not dealing the return step in the main lemma]{
...
\mrr{This states that for any method call, if two objects are connected in the configuration when the call has completed, then either they were originally connected, or an object explicitly passed to the call could \Mrr{obtain access} to both.
}

\mrr{To evaluate the entire call, we define a new reduction operator, $\leadsto^\text{call}$, which executes function calls in one step. It does this by skipping any intermediate reduction steps with the additional frames for the method call and its child calls in the stack.
}

\begin{definition}[\mrrz{Call-as-step execution}]
\label{def:call_execution} 
\mrr{Given runtime configurations $\sigma$,  $\sigma'$,  and a module $\M$ we define call-as-step execution as follows:}
...
\mrr{
We then prove that if two objects are connected by the final configuration, either the inital \prg{this} had access to both, or they were initially connected. This follows intuitively from lemma~\ref{lemma:local_variable_connectivity}, since any connections made must occur through local variables by the grammar of \LangOO.
}
...
\mrr{From here we can prove the original lemma~\ref{lemma:method_call_connectivity} by noticing in the next step, the receiver will have access to the parameters, and so lemma~\ref{lemma:receiver_connectivity} applies.
}
...
\end{itemize}
\end{definition}
\jm{Satisfaction of assertions by modules differs to prior work on Chainmail in the use of initial configurations.
As noted in Section \ref{sect:PL}, the definition of arising configurations has changed from any configuration arising 
from any initial configuration to any configuration arising from a specific configuration. This means that the definition
of satisfaction by modules must be quantified over initial configurations. This introduces linearity to \Chainmail by 
restriction questions of satisfaction to single execution paths.}
