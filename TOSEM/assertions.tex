...
\Mrr[New discussion moved further down and updated to follow changes. Once again I'm unsure what you want "previous work" things to look like?]{\Chainmail quantifies over addresses instead of local variables, thereby referring directly to instances. Since \Chainmail assertions are invariants over modules, local variable names have no consistent meaning. Previous work on \Chainmail exclusively used variables in assertions, including for quantification. Aside from being less clear, as assertions generally refer to objects meeting some property, rather than the value at a certain name, using local variables also complicated the handling of temporal assertions, as discussed in section~\ref{sect:time}}.
...
\Mrr[This again replaces the "adaption" section]{Through quantifying over addresses instead of variables, as had been the case in previous formalisations of \Chainmail, we avoid the need to adapt the future or past configuration to use the current configuration's bindings in $\Future{\_}$ and $\Past{\_}$. This would otherwise be necessary to allow specifications to reference consistent values. However, not adapting the future configuration means references to an explicit variable, such as \prg{this}, will refer to the value of the variable at the time specified, given the context of the surrounding temporal connectives. Thus, to use the present value of a local variable in a future or past context, as could be done directly if the future or past configuration were adapted as above, universally-quantified addresses should be bound to names in an implication, $\forall \alpha_x. (\alpha_x = \x \rightarrow \dots)$. This is more flexible as names can be bound at any time, rather than restricted to their current value. Explicitly binding the name to an instance once renders it irrelevant if that binding changes over time, as the address will not, providing a time-independent handle to the same instance.}
