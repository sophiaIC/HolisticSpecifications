...
\begin{definition}[Assertions]  \sd{\st{Assertions consist of (pure) expressions \e, classical assertions about the contents of heap/stack, the usual logical  connectives, as well as our holistic concepts.}}
\label{def:assertions}


\sout{\mbox{$\begin{array}{lcl}
  ~  \\
...
\label{def:assertions}
 ...
 \begin{array}{lcl}
  ~  \\
 \prg{v}  &\BBC&    \kwN{true}   ~\SOR~  \kwN{false}   ~\SOR~  \kwN{null}  ~\SOR~  \alpha ~\SOR~ \Mrrz{\prg{x}} \\
  ~  \\
...
We will discuss evaluation of expressions in section \ref{sect:expressions}, standard assertions about heap/stack and logical
 ...
 \sophia{We \Mrr[As per S.D.]{will discuss} the treatment of  permission, control, space, and viewpoint  in 
...
\mrr{In Fig.~\ref{fig:ValueSimpleExpressions}, I rewrote ${\sf {Field\_Ghost\_Val}}$, but what does $_W$ mean? I've left it for now but I don't understand it. (or have I missed something?)}
...
\begin{array}{l}
\begin{array}{ll}
\inferenceruleExact{\SP\SP\SP}{\Mrrz{Var\_val}} {
 \interp {\x}{\sigma} = v
}
{
...
\end{array}
& &
\inferenceruleNM{\mrrz{Field\_Ghost\_Val}}
{
%~ \\
...
\begin{definition}[\st{Basic Assertions}] \st{For modules $\M$, $\M'$,  configuration $\sigma$,  we define$:$}
...
Continuing our earlier example, \mrr[given an initial configuration $\sigma_0$]{Added missing $\sigma_0,$ throughout this section. Obvious, but I just want to make sure I haven't misunderstood, and this is the right way to introduce $\sigma_0$?},  
...
\begin{definition}[\st{Assertions with logical connectives and quantifiers}]  
%We now consider 
\label{def:valid:assertion:logical}
\sout{For modules $\M$, $\M'$, assertions $\A$, $\A'$, variables \prg{x}, \prg{y}, \prg{S},  and configuration $\sigma$, we define$:$}
\begin{itemize}
\item\sout{
...
\mrrz{\sout{\Chainmail as described here uses addresses rather than variable names to refer to objects. The $\Name \alpha x$ assertion allows addresses to be bound to current local variables. Previous papers have used variable names, both for referring to local variables, and in quantification. Using addresses allows for simpler formal semantics, without superficially affecting predicates for quantified variables. To specify local variables, as could be done in previous papers, universally-quantified addresses should be bound to names in an implication, $\forall \alpha_x. (\Name {\alpha_x} {\x} \rightarrow \dots)$. This is more flexible as names can be bound at any time with temporal assertions, rather than restricted to their current value.}}

\mrrz{\sout{The cost of complicating specifying local variables is considered acceptable, as in most use cases, only \prg{this} is needed. As \Chainmail works with invariants, not assertions or pre/post-conditions, there is no consistent scope for local names to have a persistent meaning, so they are rarely useful.}}\\

\Mrr[New discussion moved further down and updated to follow changes. Once again I'm unsure what you want "previous work" things to look like?]{\Chainmail quantifies over addresses, as referring to instances rather than local variables is more relevant. Since \Chainmail assertions are invariants over modules, local variable names no consistent meaning. Previous work on \Chainmail exclusively used variables in assertions, including for quantification. Aside from being less clear, as in assertions we generally want to refer to objects rather than the value at a certain name, it also complicated handling temporal assertions, as discussed in section~\ref{sect:time}}.
...
\st{Permission}]  \label{def:valid:assertion:access}
\sout{For any modules $\M$, $\M'$,  variables \prg{x} and \prg{y}, we define}
\begin{itemize}
\item\sout{
...
\end{itemize}
\end{definition}
\Mrr[Added $\sigma(\alpha,\f)$ for now, since this is what is used above, and I don't think we define interpretation for fields on addresses, no?]{}
\jm{
\begin{definition}[%Satisfaction of  Assertions about 
...
$\alpha_1$=$\alpha_{2}$, \ \ or
\item
$\Mrrz{\sigma(\alpha_1,\f)} $=${\alpha_2}$, \  \ for some field \prg{f},  \ \ or
\item
...
\mrr[In the last disjunct the interpretation of \x\ in the configuration ensures it is a parameter or local variable.]{Kept and reworded, might as well?}
...
\sout{For any modules $\M$, $\M'$,  variables \prg{x} , \y, $\prg{z}_1,...\prg{z}_n$, we define$:$}
\begin{itemize}
   \item \sout{
...
\noindent 
\Mrr{Thus, $\Calls {\alpha}  {\prg{m}} {\alpha'}  {{\alpha_1,...\alpha_n}}$ expresses that there will be a call from $\alpha_1$ to the method \m\ on object $\alpha'$ , given objects $\alpha_1,...\alpha_n$. If these are named \y, and $\z_1,...\z_n$ respectively, then $\y.\m(\z_1,...\z_n)$ will be executed next, and the current caller \this\ is $\alpha$.}
...
\textit{Viewpoint} concerns whether an object is viewed as belonging to the internal module, determined by the class of the object.
 ...
 \begin{definition}[%Satisfaction of  Assertions about 
\st{Viewpoint}]  \label{def:valid:assertion:view}
\sout{For any modules $\M$, $\M'$, and variable\x, we define}
\begin{itemize}
 \item
\sout{$\M\mkpair \M', \sigma \models \External {\x}$ 
  \IFF
 \sd{$\interp {\x} {\sigma}$ is defined} and $\ClassOf {\interp{\x}{\sigma}} {\sigma} \notin dom(\M)$}
\item
\sout{$\M\mkpair \M', \sigma \models \Internal {\x}$ 
  \IFF
...
\begin{definition}[\sout{Space}]  \label{def:valid:assertion:space} 
\sout{For any modules $\M$, $\M'$, assertions $\A$ and variable \prg{S}, we define$:$}
\begin{itemize}
\item\sout{
...
}\Mrr{where \prg{S}$_4$ is as defined previously.}
...
\begin{definition}[\sout{Time Assertions}]  \label{def:valid:assertion:time}
...
%when giving necessary conditions for some future effect.
 ...
 In general, $\Using {\Future {\A}} {\SF}$ is different from
  ...
  and $\SF_2$ to consist of objects \prg{1}, \prg{2},   \prg{4}.  Assume that 
   $\sigma_5$ is like $\sigma_1$, \Mrr{except that} the next call in $\sigma_5$ is a method on $\prg{u}_{94}$, whose  body obtains the
...
Whilst the above shows that from $\Future { \Using{A}{\SF} }$ we cannot conclude $\Using { \Future{A} }{\SF}$, it is also worth noting that in general the inverse does not hold either. $\Future { \Using{A}{\SF} }$ restricts $A$ in the future, meaning any objects instantiated in the between the present and then are excluded, whereas $\Using { \Future{A} }{\SF}$ restricts the space in the present, so the objects are considered in A. Taking a new configuration $\sigma_6$  with continuation \prg{x := new Account(100, null) } and otherwise similar to $\sigma_1$, and $\SF = \emptyset$, \Mrr{the assertion $\M_{BA1}\mkpair ..., \sigma_0 , \sigma_6 \ \models \ \Future{ \Using { \prg{x.bank} = \prg{null} } {\SF}}$  does not hold, as \prg{x} is not in $S$. However, we can prove $\M_{BA1}\mkpair ..., \sigma_0 , \sigma_6 \ \models \ \Using { \Future{ \prg{x.bank} = \prg{null} } } {\SF}$, since the restriction of the configuration to $\SF$ in the present doesn't exclude new \prg{Account} with field $\prg{bank} = \prg{null}$, created in the next statement, being considered in the \Future{\_}}.

\Mrr[This again replaces the "adaption" section]{Through quantifying over addresses instead of variables, as had been the case in previous formalisations of \Chainmail, we avoid the need to adapt the future or past configuration to use the current configuration's bindings in $\Future{\_}$ and $\Past{\_}$. This would otherwise be necessary to allow specifications to reference consistent values. However, not adapting the future configuration means references to an explicit variable, such as \prg{this}, will refer to the value of the variable at the time specified, given the context of the surrounding temporal connectives. Thus, to use the current object at a local variable in a future or past context, as can be done directly if the future or past configuration were adapted as above, universally-quantified addresses should be bound to names in an implication, $\forall \alpha_x. (\alpha_x = \x \rightarrow \dots)$. This is more flexible as names can be bound at any time, rather than restricted to their current value. Explicitly binding the name to an instance once, renders it irrelevant if that binding changes over time, as the address will not, providing a time-independent handle to the same instance. It also allows for further flexibility over when the binding occurs, allowing for assertions to reference the past or future values of a certain binding.}
...
the context of the same configurations and module pairs -- \ie\\
 \strut\mrr{Is adding $\sigma_0$ okay like this?} \hspace{1.1cm} $\A \equiv \A'\  \IFF\    \forall \sigma_0, \sigma.\, \forall \M, \M'. \ [\ \ \M\mkpair \M', \sigma_0,\sigma \models \A\ \mbox{ if and only if }\ \M\mkpair \M', \sigma_0,\sigma \models \A'\ \ ].$\\
...
\Mrr[Moved up properties as requested, however I'm not sure >>exactly how<< it should be moved up?]{}
...
\begin{itemize}
\item
$\M \models \A$ \IFF  $\forall \M'.\, \forall \sigma_0, \Initial{\sigma_0}, \ \sigma\!\in\!\Arising{\M\mkpair\M', \sigma_0}.\   \M\mkpair\M', \sigma_0 , \sigma \models \A$
\end{itemize}
\end{definition}
}
