...
\jm{The  assertion $\Calls {\alpha_1} {\m} {\alpha_2} {\prg{vs}}$ holds 
in configurations where a method on object 
${\alpha_1}$ calls method ${\m}$ on receiver ${\alpha_2}$ with arguments $\prg{vs}$ --- that is, 
there exists $\x$, $\y$, and $\zs$ in the local variable map that map to $\alpha_1$, $\alpha_2$, and $\prg{vs}$ 
