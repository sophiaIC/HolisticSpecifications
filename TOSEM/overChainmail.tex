...
we would 
 ...
 \mrr{I switched $\CanAccessTr{\_}{\_}$ for a more explicit version, since the former, whilst obvious, is less obvious, and whilst is pretty standard, is never explicitly defined or explained}
...
{\Future {\A}} {\SF}$  requires two things: i) \mrr[that $\A$ will hold in
the future]{Wait, didn't we say that \textit{"Note that  $..., \sigma \ \models \Using {\A} {\SF}$ does not imply  
  $..., \sigma \ \models \A$  nor does it imply $..., \sigma \ \models \Using {\A} {\SF\cup\SF'}$."}. I get what it's getting at, but at face-value it doesn't seem valid}, and ii)  that all the objects which cause the effect which will make $\A$ valid are   included in \SF.
