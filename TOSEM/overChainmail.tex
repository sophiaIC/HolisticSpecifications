...
shown in the left and right diagrams in Figure \ref{fig:BankAccountDiagrams}\mrr[, with $\sigma_0$ as the initial configuration]{Added this, and further modified by Prof. Eisenbach, and added $\sigma_0,$ to below assertions}. 
...
\noindent 
\mrr[For the rest, we refer to objects as $\prg{b}_1$, and $\prg{a}_2$--$\prg{a}_4$, and  $\prg{u}_{91}$--$\prg{u}_{94}$, denoting objects \prg{1}, \ \prg{2}--\prg{4},
and   \prg{91}--\prg{94} respectively for both $\sigma_1$ and $\sigma_2$. This clarifies in the assertions below the role of the object.]{I think this is what we want, not so sure this is how we want it?}
