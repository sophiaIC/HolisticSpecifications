...
\sd{Each of the novel, holistic, features of \Chainmail (permission, control, time, space, and viewpoint) introduced its
own challenges, as well as our wish to support some form of recursion while keeping the logic of assertions classical.}
\sd{We will discuss our treatment of these challenges in this section.}
...
%% instead\kjx{can we just cut this please?}}
 ...
 \sd{Notice that expressions support calls with parameters  ($\e.\f(\e^*)$); these are calls to ghostfield
...
Assertions of the form or $\e$=$\e'$ are satisfied only if both $\e$ and $\e'$ are defined. Because we do not support 
recursion at the level of assertions, assertions from a classical logic (\ie $\A \vee \neg\A$ is a tautology). }
...
Notice that the relation $ \hookrightarrow$ is partial. 
...
\sd{\textit{Permission} expresses that an object has the potential to call
methods on another object, and to do so directly, without  help from any
intermediary object. This is the case when the two objects are aliaes, 
...
In the last disjunct, where \z is a parameter of local variable,
...
from $\SF$.
 ...
 \begin{definition}[
Restriction of Runtime Configurations]  \label{def:restrict}
...
\sd{For example, if we take $\sigma_2$ from Fig. \ref{fig:BakAccountDiagrams} in Section \ref{sect:motivate:Bank},
...
separation is provided through the assertions, where $\A * \A'$ holds in a heap $\chi$ which can be split into
disjoint $\chi$ and $\chi'$ where $\chi$ satisfies $\A$ and $\chi'$ satisfies $\A'$. That is, the split of the 
heap is determined by the assertions   $\A$ and $\A'$, while the operation $\restrct {\sigma}{\prg{S}}$ is
explicitly taking a set as an argument. 
}
...
\begin{definition}[
Space ]  \label{def:valid:assertion:space} \label{def:restrict}
...
\sd{
The set \SF\ in the assertion $\Using {\A} {\prg{S}}$ is related to  framing  from implicit dynamic frames \cite{IDF,MattAlex}:
...
The difference is that frames are sets of locations (pairs of address and field), while our \prg{S}s are sets of addresses.
 More  importantly, in implicit dynamic frames, assertions whose frame is not large enough are badly formed, 
...
or not   (\eg   $\M_{BA2}\mkpair \M', \sigma   \models \exists n.\acc_2.\prg{balance}=n$, while $\M_{BA2}\mkpair \M', \sigma \not \models \Using {(\exists n.\acc_2.\prg{balance}=n)} {\prg{S}_4}$)
} 
...
\sd{We discuss challenge  b)  in some more detail: take an  assertion
...
To address challenge b), we define   an  auxiliary concept:
$\adapt$, the adaptation of one runtime configuration to the bindings 
of another.
}
\sd{
% To address a),    we store the continuation, \prg{cntn}, in each stack frame.
To address b), we dene  an  auxiliary concept:
$\adapt$, the adaptation of one runtime configuration to the scope  
of another.
}
 $\sigma \adapt \sigma'$ adapts the second configuration to the top frame's view of the former: it returns a new configuration whose stack has  the top frame as taken from $\sigma$ and where the \prg{contn} has been consistently renamed, while the heap is taken from $\sigma'$. This allows us to interpret expressions  in the newer (or older) configuration $\sigma'$ but with the variables bound according to the top frame from $\sigma$; \eg we can obtain that value of \prg{x} in configuration  $\sigma'$ even if \prg{x} was out of scope. The consistent renaming of the code allows the correct modelling of execution (as needed,   for the semantics of  nested time assertions, as \eg in $\Future {\x.\f=\prg{3} \wedge \Future {\x.\f=\prg{5}}}$
...
$\M\mkpair \M', \sigma  \models \A '$. 
 Full definitions and lemmas appear in Appendix \ref{app:assertions}.
% However, note that this desirable property comes at the cost of 
% the loss of some expected correspondences between assertions and expressions. For 
% example $\not \e$ is not equivalent with $\e \rightarrow 
