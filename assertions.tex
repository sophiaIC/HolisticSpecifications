...
Through ghost fields we support recursive definitions.}
...
${\sf {Field\_Heap\_Val}}$), and the module, \M, to find the corresponding ghost filed declaration. 
...
%
The lookup function $\mathcal G$  (defined in the obvious way in the Appendix, Def.\refdef:lookup})
...
$\sigma_0$ where
\prg{acyc} points to a \prg{Node} whose field \prg{next} with value \prg{null}, and   
...
Notice that for an expression of the form  
...
For modules $\M$, $\M'$, assertions $\A$, $\A'$, variables \prg{x}, \prg{y} and \prg{S},  configuration $\sigma$, we define$:$
\begin{itemize}
\item
