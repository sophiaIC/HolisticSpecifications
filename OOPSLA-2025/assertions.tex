...
\subsection{Semantics of Assertions  -- First Part}
...
\ \ \ \wedge\ \  \alpha\neq \alpha_0$,
 \item
$\forall \alpha' \in {\Relevant {\alpha_o} {\sigma}}.  \forall f.[\ \ \satisfiesA{M}{\sigma}
{\external {\alpha'}  \ \ \Longrightarrow \ \    \interpret {\sigma} {\alpha'.f} \neq \alpha  }   \ ] $  $  \strut \hspace{.25cm} ] $.
\end{enumerate}
...
$ \eval{M}{\sigma}{{\re}}{\alpha}$,
 ...
 \item
$\forall \alpha'\in {\LRelevantO {\sigma}}.\forall f.[\  
 \satisfiesA{M}{\sigma}{ \external {\alpha'}  \ \ \Longrightarrow \ \  
...
% that appendex commented out from all version of main.tex
   and    Fig.    \aref{12 in App. C}{\ref{fig:ProtectedFrom} in App. \ref{appendix:assertions}}.
%
...
\footnoteSD{JAMES' comment: If is possible that ``we'' do not know the complete heap (eg we only know about the green stuff.) how do we know whether an object is protected. The answer is that we do not know that it is protected, but we do know that our code guaranrtees preservation of protectedness.
}  
