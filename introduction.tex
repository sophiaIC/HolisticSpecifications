...
Necessary conditions are guarantees that are upheld throughout program execution.
A  prevalent form of necessary condition used in programming languages is type systems. 
They guarantee that well-formed programs will always produce well-formed runtime
configurations.  
Another form of necessary conditions is information flow control \cite{infflow}, which guarantees that values 
classified as high  will not be passed into contexts classified as low. 
The  guarantees made by types or information flow control
 are  practical to check, but  too coarse grained
for the purpose of fine-grained, ADT specific specifications. 

Necessary conditions are  akin to  monitor or object invariants\cite{Hoare74,Meyer97}. The difference between
these invariants and our holistic specifications is that object/monitor invariants only talk about 
the current state (\ie the contents of the
stack frame and the heap), while our holistic specifications also talk about the execution 
history, the future, xxxx.
