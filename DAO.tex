...
a repayment; it is \prg{m} if  the previous call was \prg{p} joining \prg{d} and paying in \prg{m}. More cases are needed to reflect the financing and repayments of proposals, but they can be expressed with the concepts described so far.\footnote{We do not represent al of the
...
% This is the original, longer one: 
%\vspace{.1cm}
%
%\noindent
% \strut \hspace{0.5cm} $\forall \prg{d}:\prg{DAO}.\forall \prg{p}:\prg{Any}.\forall\prg{m}:\prg{Nat}.$\\
...
We can now define  what it means for \prg{p} to have a  \prg{Balance} at  \prg{d}. 