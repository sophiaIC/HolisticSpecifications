...
might do \se{with \prg{this.accnt}}. 
}
...
$\strut \ \ \ (B)\ \ $  if \prg{buyer} cannot eventually get access to the account's \password,   \ \ \    {\emph{ then}}  \\
...
\vspace{.2cm}
\noindent
 \textbf{NOTE}  \sdN{Wrapping} % just so as to shorten sentence
  the account so as to allow payments but forbid withdrawals,   would be ineffectual in \taming the possible effects on line 9. 
...
\sdN{SD: The point is that if before the call to \prg{pay}, the \prg{buyer} had access to the account and its keyword, and when we call \prg{pay} instead of 
