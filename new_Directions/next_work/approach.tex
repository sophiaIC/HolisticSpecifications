...
\item   funds cannot be withdrawn unless the external entity responsible for the withdrawal has indirect access to the account's \password,
\end{enumerate}
\item[--- then]
%\ \  \ \ \ \emph{---} then
...
Fig.\ \ref{f:CurrentPoint} illustrates the current point of execution.  The left pane, $\sigma_1$, shows a state with the same heap as earlier, but where the top frame is $\phi_1$ -- it could be the state before a call to \prg{buy}.
...
if no external object indirectly accessible from the current frame\footnote{An object is indirectly accessible from a frame if 
there exists a sequence of field accesses (direct references) leading from one of the variables in the frame  to that object.} has direct access to $o$,
...
from}. Object $o_6$, it is not protected in states $\sigma_1$ and
$\sigma_2$, but $o_6$ is protected in state $\sigma_3$.
...
Perhaps counterintuitively, in this formal model
calling a method (pushing a frame) can only ever \emph{decrease} the set of indirectly accessible (pre{\"e}xisting) objects,
so while object $o_5$ is indirectly accessible from the top frame in $\sigma_2$, it is not indirectly accessible from the top frame in $\sigma_3$.
...
This is the case even if $o$
was not protected before the execution of the current method,
during the remainder of the execution of the method calling the current method, nor after termination of the current method.
We express these call-stack bounds with \emph{scoped invariants}.
% need not be protected during execution of the method calling the current method, nor after termination of the current method.   
...
Fig.\ \ref{fig:illusrPreserve} shows the  states of an unspecified execution starting at internal state $\sigma_4$ and terminating at internal state $\sigma_{23}$.
Fig.\ \ref{fig:illusrPreserve} distinguishes between steps within the same method ($\rightarrow$),
method calls ($\uparrow$), and method returns ($\downarrow$). %  method; up-arrows represent ; down-arrows represent method returns.
The scoped future of $\sigma_6$ consists of $\sigma_6$-$\sigma_{21}$. The scoped future of $\sigma_9$ consists of $\sigma_9$, $\sigma_{10}$,  $\sigma_{11}$,  
...
This example illustrates three crucial properties of our invariants:
 ...
 \vspace{.05cm}
\noindent
\textbf{\emph{Conditional}}: Invariants are \emph{preserved}, but unlike object invariants, they do not % guarantee that they
always hold.
...
%We show some  rules below:
%{any newly created object is protected}, the ass 
%
% More in Figs. \ref{f:underly} -  \ref{f:calls}.
For example, any newly created object is protected. Any %the underlying Hoare logic guarantees that if $A$ holds    
...
receiver (\prg{buyer}) or  arguments (\prg{this.\myAccount} and \prg{price}) when \prg{pay} was called --- calls cannot increase the set of indirectly accessible objects (\S \ref{sect:approach:protection}).

In general, for a call $\prg{y}_0.m(\prg{y}_1, ..., \prg{y}_n)$, if at the call $\protectedFrom{\prg{x}}{\prg{y}_i}$  for all %arguments 
...
In %our threat model
an open world, external objects can execute arbitrary code, invoke any public internal methods, access any other external objects, and even collude with each another.
...
The conditional and scoped nature of our invariants are critical to their applicability.
...
precisely because these conditions are local, they can be enforced locally with a module,
