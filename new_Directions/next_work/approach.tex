...
Indeed, if \Mshop  makes appropriate guarantees, it can \tame  the effects at line 9. In particular,  
 \\
$\strut \ \ \ (A)\ \ $  if \prg{buyer} cannot eventually get access to the account's \password,   \ \ \    {\emph{ then}}  \\
...
%\vspace{.3cm}
\noindent
{The remit of our work is to provide the specification and verification tools to support arguments such as the one given above. 
In \sdN{that argument}, we used the not yet defined concepts of  (A) ``can  eventually get access to'', and (B) assumed  \tamed effects (no money withdrawn ... unless ..). 
...
%
We build on the concept of two-state invariants \se{ref???}, and define
