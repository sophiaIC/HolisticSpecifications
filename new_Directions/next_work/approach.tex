...
Take a  small, object oriented, class-based  language similar to Joe-E \cite{JoeE} with modules,   module-private fields
...
{Methods are module-private, unless defined as \prg{public} -- in this case they may be called by objects which do not belong to the same module, 
...
\item[{Scoped invariants}] to describe emergent behaviour:  {$\TwoStatesN  {\overline{x:C}}  {A}$} expresses that if a reachable state $\sigma$ 
  satisfies ${\overline{x:C}}  \wedge A$, then all $\sigma$'s \emph{scoped} future  states will  {also} satisfy  {$A$}. The scoped future contains future states, including further method calls and returns, but {does not return} from the call active in $\sigma$ -- more in Def 5.3. \sdN{and 3.1}.
I% In other words, the current call \emph{scopes} where \se{$A$} must hold -- more in Def 3.1. 
