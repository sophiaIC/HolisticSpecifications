...
For our example we assume a very small, object oriented language with modules, where fields are private 
...
Also, we need that methods are dynamically type-checked before the external call -- I think -- ot even the call from the external. }
  
We build on, and expand the same example as in OOSPLA-22. Module \ModA consists of an empty 
...
%\end{lstlisting}
 ...
 From then on, anybody who has access to \prg{a} may transfer money into it; but in order to transfer money out of the account, one needs to
...
With these concept, we no in


Note that OOPLSA instead uses    necessity operators, while mentioned in  \S \ref{intro:this:work}. 
