...
We illustrate the situation through an example of a heap snippet. Red and green squares are external and internal objects, respectively. 
Each object has a unique number, followed by an abbreviation of its class, \eg $o_1$ has class \prg{Shop}, $o_1$ has class \prg{Inventory}, while 
$o_5$ and $o_7$ are external. 

Curved arrows between objects indicate field values. For example, $o_1$ has three fields, pointing to 
$o_4$, $o_5$ and $o_2$.

Fields denote direct access. The transitive closure of direct access gives indirect (or transitive) access. Thus $o_1$ has direct access to $o_4$, and indirect access to $o_6$.

Object $o_6$ is the key to the account $o_4$, and represents the capability that allows withdrawal. We highlight this through a dark outline to the object.
...
&
$\sigma_2$, top frame $\phi_1$
&
$\sigma_3$, top frame $\phi_3$
\\
\hline \hline
\end{tabular}
\caption{\textit{Protected from and Protected}. 
...
Horizontal arrows represent execution within the same method; up-arrows represent method calls; down-arrows represent method returns.
 \\
Thus, the scoped future of $\sigma_6$ includes all states in the diagram, and the scoped future of $\sigma_8$ consists of $\sigma_9$, $\sigma_{10}$,  $\sigma_{11}$,  
