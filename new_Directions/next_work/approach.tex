...
In this Section we introduce our problem and outline our approach. 
 ...
 \susan{one that can deal with challenges provided by reasoning about external modules accessing our code.}
...
\susan{modules pluaral is clumsy and I think we only deal with our single internal module's emergent behaviour?}
%
...
\item[Bounded, external, two-state invariants] to describe emergent behaviour:  $\TwoStatesQ {\overline{x:C}}  {A}{A'}$ expresses that if a reachable state $\sigma$ 
  satisfies ${\overline{x:C}}  \wedge A$, then all $\sigma$'s \emph{bounded future} states will satisfy $A'$ -- more in Def. 5.3. The concept of a \emph{bounded} future includes further method calls and returns, but excludes returning from the call active in $\sigma$ -- more in Def 3.1. 
