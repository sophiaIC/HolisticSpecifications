...
\footnote{{TODO: elaborate here on ``can eventually get acces'', and on ``tamed effect''}}
...
Here we have that 
 ...
 {Below we show a  Hoare logic rule %\ruleExtCallB\ from
...
% }}
\\
and similar for {$S_2$ and $S_4$}. Here we used   $stmt\_b$  as short for the method body of \prg{buy}, and $A_1 \triangleq \prg{this}:\prg{Shop}, \prg{buyer}:\prg{external}, \prg{anItem}:\prg{Item}{,}\prg{a}:\prg{Account}$
and $A_2 \triangleq A_1{,} \prg{b}:\prg{int}$.
