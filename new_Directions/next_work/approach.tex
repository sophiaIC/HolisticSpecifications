...
To prove \textbf{(1)}, we aim to utilize $S_3$; but this is not straightforward: %Applying
 $S_3$   {\color{red} {requires $\inside{\prg{this.\myAccount.key}}$}}, but the precondition of \textbf{(1)}  only provides $\protectedFrom{\prg{this.\myAccount.key}}{\prg{buyer}}$. And more alarmingly,  
...
To formulate this, we introduce the adaptation operator $\FIXSymbolA$, which translates assertions from the caller's perspective to that of the callee. Specifically, $\PushASLong  {(\prg{y}_0, ..., \prg{y}_n)}   A$ at a call, ensures   $A$  when the variables  $\prg{y}_0, ..., \prg{y}_n$ are pushed onto a new frame.

Here, \small $\PushASLong {(\prg{buyer},\prg{this.\myAccount},\prg{price})} {\inside{\prg{this.\myAccount.key}}}$
= $\protectedFrom{\prg{this.\myAccount.key}}{\prg{buyer}}$  \normalsize -- more in Def   \ref{def:push} and  Lemma \ref{lemma:push:ass:state}. This enables the application of $S_3$ in \textbf{(1)}. The corresponding Hoare logic rule is shown in Fig. \ref{f:external:calls}.
