\newcommand{\balance}{blnce}
\newcommand{\password}{pwd}
\newcommand{\myAccount}{accnt}

\section{The problem and our approach}
\label{s:outline}
 
In this Section we introduce our problem and outline our approach. 
 We build on, and expand the  example as in OOSPLA-22 and FASE,  but \sdN{note that our approach is different, and more importantly, it can reason about external calls -- something that OOSPLA-22 and FASE did not do.}%and we solve one more challenge than they,
 \susan{one that can be used to reason about external modules accessing our code. \red{SD: there is also the difference that we have two-state invariants and OOPSLA had the only-if and only-through specs.}}
 \susan{above paragraph - 2 state is how and isn't novel to us, I think it falls under the we propose a different approach,
 below paragrah - it would read better if the word modules' is just omitted, I think that is the right level of detail for here
 \red{SD: is it better now? not that clear whether you wanted to drop the 2-state, or the different approach}}
...
%\susan{modules pluaral is clumsy and I think we only deal with our single internal module's emergent behaviour? \red{SD: note sure' I always thought that the emergent behaviour is the behaviour of calls to our as well the external modules}}
%
...
$\ModA$ consists of an empty class  \prg{Password} whose instances model  unique passwords}, and an \prg{Account} class with fields  \prg{\password} and  \prg{\balance}, a \prg{set} method to initialize the password, and  a \prg{transfer} method. 
...
An \prg{Account} is created with its \prg{\balance} set to \prg{0}, and its \prg{\password} set to \prg{null}, \eg by executing \prg{acc = new Account}.
...
\item[Bounded, external, two-state invariants] to describe emergent behaviour:  $\TwoStatesQ {\overline{x:C}}  {A}{A'}$ expresses that if a reachable state $\sigma$ 
  satisfies ${\overline{x:C}}  \wedge A$, then all $\sigma$'s \emph{bounded future} states will satisfy $A'$ -- more in Def. 5.3. The concept of a \emph{bounded} future includes further method calls and returns, but \sdN{does not return} from the call active in $\sigma$ -- more in Def 3.1. 
%  \susan{Excludes sounds like you could jump over that frame and continue, perhaps 'stops before' or if you like included/excludes then maybe (e.g. stops before) after excludes }
...
\label{s:bankSpecEx}
 ...
 \\
 $S_2$   & $\triangleq$   &  $\TwoStatesQ {\prg{a}:\prg{Account}}  {\inside{\prg{a.pwd}}} {\inside{\prg{a.pwd}}}$
 \\
 $S_3$ & $\triangleq$   &
...
$S_4$ & $\triangleq$   &
  ...
  &  &  $\FirstState{\inside{\prg{a.pwd}} \wedge \prg{a.\balance}=\prg{b}} 
...
module with bank accounts which have a % {\textit{implementation}} of a bank account with a 
 \prg{\balance} and a \prg{\password} -- even as ghost fields --}, irrespective of the API offered, services  exported, or  dependencies on other parts of the system.\footnoteSD{does this come from OOPSLA? if so we need to rephrase}
\notesep
...
Consider that {our internal module included the following class} \prg{Shop}. It exports a public method \prg{buy} whose formal parameter \prg{buyer} is an \prg{external}   object. 
...
It is safe to apply $S_4$,  because from the point of view of the callee at line 7 the  password is indeed protected -- \cf Fig, 5,   Lemma 4.7, and the 
 ...
 \notesep 
 We could specify the method \prg{buy} through pre-condition $\protectedFrom {\prg{a.pwd}} {\prg{buyer}} \wedge {\prg{a.\myAccount.\balance}}=\prg{b}$ which
 guarantees that $\prg{a..\myAccount.\balance}\geq \prg{b}$.  Note that this is a  direct conclusion of $S_4$; 
  as \prg{Shop} is part of our internal module, this specification superfluous -- more in Example \ref{example:mprepostl}.
