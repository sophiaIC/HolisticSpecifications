...
If the  internal module  never passes $o$ to any external object (\ie never leaks $o$,) then  if $o$ is protected from $o_e$ now, it will remain so.
...
Modules $\ModA$ and $\ModC$ satisfy $S_2$ and $S_3$, while $\ModB$ satisfies neither $S_2$ nor $S_3$. No module satisfies $S_1$. 
\end{example}
...
Our specifications are conditional: they do not guarantee that certain effects will never occur, but they ensure that specific effects will only happen if certain conditions were met prior to the execution of the internal \susan{external} code.
