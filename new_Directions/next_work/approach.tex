...
\label{s:bank}
Our example  demonstrates the need to consider object capabilities as \emph{guards}, and the importance 
of the \emph{emergent behaviour} of modules. 
%
in Sect.  \ref{s:approach:necopers} we have three versions of a banking module, $\ModA$, $\ModB$, and $\ModC$.
...
\sdN{Thus, in all three modules, the password is the object capability which \emph{enables} the withdrawal of the money. 
...
Hoare logics can describe the enabling (through per-method PRE/POST condition pairs), but cannot describe guarding.
The description of capabilities as guards requires per-module assertions, as proposed in in OOPSLA-22, and in the current work.
...
\begin{description}
\item[1st Challenge -- Specification] How to specify that object capabilities are guards over effects while taking the emergent behaviour into account?
\item[2nd Challenge -- Verification of Open Calls] How to verify that calls to external, unknown code, will not cause effects it is   ``not entitled'' to?
\item[3rd Challenge -- Verification of Modules] How to verify that modules adhere to their specification?
\end{description}

We give a high level overview to each of these in Sections \ref{s:approach:necopers}, \ref{sec:how}, and \ref{sec:howThird}, but before doing that we make the example more concrete, and discuss external modules.
