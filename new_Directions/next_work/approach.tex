...
%%In this sense, and, similar to OOPSLA'22, our  specifications capture a module's \emph{emergent behaviour}. 
 ...
 When reasoning about external calls, even though we do not have a specification for the \se{called method}, we know that the effects of the call are tamed according to the module's scoped invariants. 
...
{In general, more objects are protected from the viewpoint of the \se{called} than from the viewpoint of the caller. -- cf Fig.  \ref{fig:ProtectedBoth}.
This is so, because the   accessible objects in the execution of \prg{pay} are those accessible from \prg{buyer}.
To express this,} we define the operator $\pushSymbolAA$ , which   translates an assertion from the viewpoint of the \se{called}, to that of the caller:
...
To develop our logic, we     assume a  Hoare logic of  triples, whose assertions do not \se{have} the concept of protection.
...
%can only be {not satisfied} by internal code %from that module 
-- \cf Def. \ref{d:encaps}. 
...
\vspace{.1cm}
\se{!!!WE HAVE REMOVED S_3!!! I CAN REMOVE THE MENTION HERE}
\Eg to prove  that method \prg{Shop::buy} satisfies  {$S_3$}  we  need to prove:
\\
% {\footnotesize{
...
% }}
\\
and similar for {$S_4$}. Here we used   $stmt\_b$  as short for the method body of \prg{buy}, and $A_1 \triangleq \prg{this}:\prg{Shop}, \prg{buyer}:\prg{external}, \prg{anItem}:\prg{Item}{,}\prg{a}:\prg{Account}$
$, \prg{b}:\prg{int}$.
