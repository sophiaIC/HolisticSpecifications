...
\sdN{Thus, the password if the capability protects the money.}\footnote{We use protects in a different sense later...}
...
\sdN{Therefore, in $\ModA$ and $\ModC$  the password capability indeed affords the intended protection}, \footnote{what word to use here?}  while in $\ModB$ it does not.
% \noindent
...
\item[Emergent Behaviour] What effects may become possible through the interplay of several methods from a module (here in ${\ModB}$  an external object which has no access to the password may modify the password, and then withdraw the money).
\end{description}}
