...
\subsection{Internal and external modules, objects, and states}
 \label{s:concepts}

\sdN{We are concerned with guarantees made in an \emph{open} setting; that is, a given module
...
Therefore, $\ModA$ and $\ModC$  are robust modules, while  $\ModB$ is not. \susan{Do we need the concept of robust in this paper?}
...
and who is guaranteed not to obtain access to the capability.
\item[Emergent Behaviour] What effects may become possible through the interplay of several methods from a module (here in ${\ModC}$ \susan{$\ModB$}, an external object which has no access to the password may modify the password, and then withdraw the money).
\end{description}}
...
\sdN{Methods are module-private, unless defined as \prg{public} -- in this case they may be called by objects which do not belong to the same package \susan{module? I assume we don't have packages?}, after dynamically checking that the types of the arguments correspond to the formal parameter types.}
...
\subsection{First Challenge: Specifications for  Capabilities, and Emergent Behaviour}
\label{s:approach:necopers}

\susan{it sounds like capabilities and emergent behaviour are different things, perhaps Specifications for Capabilities including Emergent behaviour - or something similar}

\sdN{We want to be able to write specifications that reflect the spirit of OCAP programs: namely capabilities and the effects they enable, and emergent behaviour.
In particular, lack of access to capabilities guarantees that certain effects will not happen. 
We propose a specification language for modules, which has two main novel ingredients:}

\sdN{
\begin{description}
\item[Protection] to describe a lack of access to capabilities: An object $o$ is \emph{protected} from $o'$ if $o$ cannot obtain direct access to $o'$ unless an internal object affords access to $o$. And $o$ is \emph{protected}, formally ${\inside{\prg{\it{o}}}}$, if $o$ is protected from all external objects transitively accessible from the currently executing method call. More in Def. 3.5, 4.5 and 4.6.
\item[Bounded, external, two-state invariants] to describe emergent behaviour:  $\TwoStatesQ {\overline{x:C}}  {A}{A'}$ expresses that if any reachable state $\sigma$ 
state satisfies ${\overline{x:C}}  \wedge A$, then all $\sigma$'s \emph{bounded future} states will satisfy $A'$ -- more in Def. 5.3. The concept of \emph{bounded} future includes states that are reachable from further method calls and returns, but excludes returning from the call active in $\sigma$ -- more in Def 3.1. 
...
With these concepts we can write specifications which  rule  out \ModB while permitting \ModA and
\ModC, namely:
...
\sdN{This % gives our specifications the
% brings the vital advantage that our specifications can % be used to constrain
...
\footnote{\red{to discuss} 
I think the below no longer holds. 
It held for OOPSLA.
\notesep
% Our example also demonstrates that 
\sdN{Adherence to   specifications}  is not monotonic:
...
% {For example, \ModA satisfies $S_2$, while \ModB does not.}
...
\sdN{We now consider how we can reason about external method calls. 
...
\susan{Is Shop in an external or an internal module?}
\sdN{
Consider the following class \prg{Shop}. It exports a public method \prg{buy} whose formal parameter \prg{buyer} is an \prg{Object} and thus may be an external object. 
...
\vspace{.1cm}
  ...
  \noindent
\sdN{\textbf{NOTES}
\susan{You've never said how a holistic spec is used.}
 \notesep We   applied $S_4$ at the point of call in line 7, even % though the
 the pre-state in $S_4$ is  in terms a \emph{stronger} property than the one we have.
...
It is safe to apply $S_4$,  because from the point of view of the callee at line 7 the password is indeed protected -- \cf Fig, 5,   Lemma 4.7, and the 
 ...
 \notesep 
 We could specify the method \prg{buy} through the PRE/POST pair: $PRE: \protectedFrom {\prg{a.password}} {\prg{buyer}} \wedge {\prg{a.balance}}=\prg{b} -- POST: \prg{a.balance}\geq \prg{b}$. However this PRE/POST pair, is a direct conclusion of $S_4$; thus is \prg{Shop} were part of our internal module, the PRE/POST pair would be superfluous.
}
 
 \subsection{3rd Challenge: Proving that a module adheres to its specification}
...
We therefore assume a sound Hoare logic for our underlying language, and extend it with rules about protection, external calls, and the module's adherence to its 
...
These guarantees must be upheld while 
 $\overline M$, the  \emph{\externalM} modules, are executing; in contrast, the internal module may 
temporarily break them,
so long they are it externally visible.}
...
\item[Capabilities] What are the capabilities (here the password), what effects do the capabilities enable (here reduction of the money in an account), 
and who is guaranteed not to obtain access to the capability.
\item[Emergent Behaviour] What effects may become possible through the interplay of several methods from a module (here in $\ModC$, \susan{bad rather than better} an external object which has no access to the password may modify the password, and then withdraw the money).
\end{description}}
...
\sdN{
\begin{description}
\item[Protection] to describe lack of access to capabilities: An object $o$ is \emph{protected} from $o'$ if $o$ cannot obtain direct access to $o'$ unless an internal object affords access to $o$. And t $o$ is \emph{protected}, formally ${\inside{\prg{o}}}$, if $o$ protected from all external objects transitively accessible from the currently executing method call. More in Def. 3.5, 4.5 and 4.6.
\item[Bounded, external, two-state invariants] to describe emergent behaviour: t $\TwoStatesQ {\overline{x:C}}  {A}{A'}$ expresses that if any reachable state $\sigma$ 
state satisfies ${\overline{x:C}}  \wedge A$, then all $\sigma$'s \emph{bounded future} states will satisfy $A'$ -- more in Def. 5.3. The concept of \emph{bounded} future includes states that are reachable from further method calls and returns, but excludes returning from the call active in $\sigma$ -- more in Def 3.1. 
We consider executions of the internal module linked with any number of unknown, external modules -- cf Sect. 2.2.
 For $\sigma$ and its bounded future states  we only consider states whose receiver is \emph{external}. 
\end{description}
...
In this sense, and, similar to OOPSLA'22, our  specifications capture a module's \emph{emergent behaviour}. 
 ...
 \subsection{2nd Challenge: Reasoning about external calls}
\label{sec:how}
...
\sdN{\textbf{NOTES}
 ...
 Namely,   $S_4$ requires   the password to be protected, while we only know 
  that the password is protected from   \prg{buyer}.
It is safe to applly $S_4$,  because from the point of view of the callee at line 7 the password is indeed protected -- \cf Fig, 5,   Lemma 4.7, and the 
  Hoare logic rule in Fig.10.
 %,  and in the callee's \emph{bounded} future states,  the password is protected.
  \notesep  If at the point of call of \prg{buy} the password was not protected from the \prg{buyer}, then, of course, no guarantee can be made about the account's balance.
 \notesep Requiring that the account's balance was protected from all external objects would be far too strong; if that were the case, then the account would not be useful for any payments.
