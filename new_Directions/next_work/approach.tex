...
%\vspace{.05cm}
 ...
 Now  revisit the external call from line 9 in Section \ref{sec:shop}. Our argument is that if the module satisfies $S_4$ (\ie $(A)$), and if before calling \prg{pay}, the \password\ of the shop's account is protected from \prg{buyer} (\ie $(B)$), then, after the call, the balance of that account will not have decreased (\ie $(C)$). 
...
\\
%and similar for {$S_4$}. Here we used   
