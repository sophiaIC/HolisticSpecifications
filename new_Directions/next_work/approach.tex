...
Hoare logics can specify enabling (through per-method PRE/POST-condition pairs), but cannot specidy guarding.
The specification of capabilities as guards requires per-module guarantees, as proposed, \eg,  in OOPSLA-22, and in the current work.
...
\sdN{We want to be able to write specifications that \sdN{whioch capabilities guard which effects while taking emergent behaviour into consideration.}
...
For this, we propose a specification language for modules, which has two main novel ingredients.
...
%It held for OOPSLA.
% SOphia now thiks it holds -- hence added}
\notesep
