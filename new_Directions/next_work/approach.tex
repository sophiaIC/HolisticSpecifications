...
If the \prg{Account} comes from a module that satisfies $S_4$, and if at the point of the call to \prg{buy} -- \ie at line 4 -- the \password~was protected from the \prg{buyer}, then it is guaranteed that after the external call -- \ie after line 7 --  the shop's account's balance will not have decreased.
 \ruleExtCallB is out Hoare logic rule from Fig. \ref{f:external:calls}, which when verifies line 4 through application of $S_4$.
...
{In order to prove that a module adheres to its specification, we need to prove that all  private methods satisfy their  PRE/POST post-conditions, and that all public methods satisfy the module's scoped invariants combined with their PRE/POST conditions -- if any.
