...
which does not {have} the concept of protection 
 and does not deal with external calls, \  \emph{i.e.,} the assertions $A$ in the $\vdash_{ul}$-triples do not mention protection, and the $stmt$s do not contain external calls.
We then extend this logic through  
substructural rules,  an embedding into our quadruples,   rules about protection, and rules about external calls
...
\Eg as shown below, any valid triple in the underlying Hoare logic is a valid quadruple in our logic, and  any newly created object is protected. 
...
%\vspace{.1cm}
\paragraph{Well-formed modules} A module is well-formed, if  its specifications are well-formed,   its public methods preserve   its invariants, and  all  its methods satisfy their specifications - \cf  Fig.  \ref{f:wf}.
A specifications is well-formed if it  does not mention protection in negative positions (this is needed for the proofs of external calls). 
A method satisfies a specification or an invariant, if its body satisfies the relevant pre- and post-conditions.
...
To prove that a method body satisfies pre- and post-conditions we will be using the Hoare logic discussed in this section.
The treatment of external calls is if special interest. 
...
In general, if   $\protectedFrom{\prg{x}}{\prg{y}_i}$  for all %arguments 
