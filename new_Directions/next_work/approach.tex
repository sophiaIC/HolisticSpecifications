...
\caption{\textit{Protected from and Protected}. -- continuing from Fig, \ref{f:CurrentPoint}.
 ...
 \end{figure}
 \sue{AFTER 'new top frame' THE SUBSCRIPT SHOULD BE 4 NOT 5}
...
\sue{So if the internal module is required to satisfy either $S_2$ or $S_3$ and the programmer wrote version $\ModB$, it should  be rejected as not safe.}
None \sue{of the versions} satisfy $S_1$, because  \prg{pay} \sue{could leak} %leaks 
an \prg{Account}.
