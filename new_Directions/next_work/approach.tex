...
\noindent 
The first task is to describe that effect  $E$ took place: if we  find  some assertion $A$ \sue{(\eg \sdred{ balance is $\geq$ some value $b$})}
...
\caption{\textit{Protected from and Protected}. -- continuing from Fig, \ref{f:CurrentPoint}.
 ...
 \end{figure}
 \sue{SUSAN: after 'new top frame' THE SUBSCRIPT SHOULD BE 4 NOT 5}
 \sdred{SOPHIA: Do you mean it should read " $o_4$ is transitively accessible from the  top frame in $\sigma_2$, but not transitively accessible from the   top frame in $\sigma_3$." I think that $o_4$ is accessible from tip frame in $\sigma_3$}
...
\sue{So if the internal module is required to satisfy either $S_2$ or $S_3$ and the programmer wrote version $\ModB$, it should  be rejected as not safe.}\sdred{SOPHIA: What does the previous sentence add?}
None \sue{of the \sdred{three} versions} satisfy $S_1$, because  \prg{pay}  could leak %leaks 
an \prg{Account}.
...
{ \noindent
\paragraph{Hoare Quadruples} Our specifications are  in terms of the complete module, rather than per-method.
...
\paragraph{Hoare Quadruples} Our scoped invariants require quadruples, rather than the classical triples.
Specifically, \\
...
%\sdred{any newly created object is protected}, the ass 
%
% More in Figs. \ref{f:underly} -  \ref{f:calls}.
\sdred{For example, any newly created object is protected. And any %the underlying Hoare logic guarantees that if $A$ holds    
...
\\
\noindent
which says, that  if the shop's account's key is protected from \prg{buyer}, then after the call, the account's balance will not decrease.

 \vspace{.1cm}
 
To prove \textbf{(1)}, we aim to utilize $S_3$; but this is not straightforward: %Applying
 $S_3$   {\color{red} {requires $\inside{\prg{a.key}}$}}, but the precondition of \textbf{(1)}  only provides $\protectedFrom{\prg{this.\myAccount.key}}{\prg{buyer}}$, while 
$\inside{\prg{a.key}}$ may not hold at the time of the call.
%
...
we find that $\sigma_3 \models \inside{o_4\prg{.key}}$. More generally, if $\protectedFrom{\prg{this.\myAccount.key}}{\prg{buyer}}$ holds before the call to \prg{pay}, then $\inside{\prg{this.\myAccount.key}}$ holds upon entering the call.
 This is because any objects   accessible during \prg{pay} are accessible from the call's arguments % \prg{pay}'s arguments 
 (\ie \prg{buyer}, \prg{this.\myAccount}, and \prg{price}).

In general, for a call $\prg{y}_0.m(\prg{y}_1, ..., \prg{y}_n)$, if $\protectedFrom{\prg{x}}{\prg{y}_i}$  for all %arguments 
 $\prg{y}_i$, at the call then $\inside{\prg{x}}$   upon entering the call. 
To formulate this, we introduce the adaptation operator $\FIXSymbolA$, which translates assertions from the caller's perspective to that of the callee. Specifically, $\PushASLong  {\prg{y}_0, ..., \prg{y}_n} y A$ ensures that $A$ holds when the variables  $\prg{y}_0, ..., \prg{y}_n$ are pushed onto a new frame.

Here, \small $\PushASLong {(\prg{buyer},\prg{this.\myAccount},\prg{price})} {\inside{\prg{this.\myAccount.key}}}$
= $\protectedFrom{\prg{this.\myAccount.key}}{\prg{buyer}}$. \normalsize More in Def \ref Def. \ref{def:push} and  Lemma \ref{lemma:push:ass:state}. This enables the application of $S_3$ in \textbf{(1)}. The corresponding Hoare logic rule is shown in Fig. \ref{f:external:calls}.
