...
\ \Eg   \prg{buy} cannot assume $\inside{a.\prg{key}}$ holds on entry, but   guarantees that if it holds on entry, then  it will still hold on exit.
 ...
 We  can also use the features from the previous section to specify methods. 
  \Eg, $S_6$  promises that \prg{buy} will not decrease the %shop's account 
 balance, if the account's key was protected from \prg{buyer} before the call.
...
$S_6$  does \emph{not} guarantee \prg{buy} will only be called when $\protectedFrom{\prg{this}.\prg{\myAccount}.\prg{key}} {\prg{buyer}}$ holds. 
