...
The critical point is the external call on line 9,   {where the \prg{Shop} asks the \prg{buyer} to pay the price of that item,
...
{The \prg{Shop} hopes, but cannot be sure, that at line 10  it  will have received money; but 
...
Indeed, if \Mshop  makes appropriate guarantees, it can \tame  the effects at line 9. In particular,  
 \\
$\strut \ \ \ (A)\ \ $  if \prg{buyer} cannot eventually get access to the account's \password,   \ \ \    {\emph{ and}}  \\
$\strut \ \ \ (B)\ \ $ If \Mshop guarantees that no money can be withdrawn from an account unless the external\\  
...
%\vspace{.3cm}
\noindent
The remit of this paper is to provide specification and verification tools that support arguments like the one above. In that example, we relied on two yet-to-be-defined concepts: (A) "can eventually gain access to" and (B) \tamed effects (e.g., no money withdrawn unless certain conditions are met).
...
%An object $o$ is considered \emph{protected} if external objects cannot directly access $o$ unless this access is granted by an internal object.
An object   is % considered
 \emph{protected} if it remains inaccessible to external objects unless an internal object explicitly grants access.
...
How can we express the guarantee that effects are \tamed?  
 ...
 Traditional, \sdN{per-method} PRE/POST conditions \sdN{cannot guarantee} %specifications cannot express guarantees
...
\noindent
{\textbf{%\Taming of  effects is
\sdN{Scoped invariants are} \emph{conditional}}:}  They ensure that assertions are \emph{preserved}, but unlike object invariants, they do not guarantee that they %assertions 
always hold.\ \Eg   \prg{buy} cannot assume $\inside{a.\prg{key}}$ holds at the start, but it can guarantee that if it holds on entry, it will still hold on exit.
...
%%In this sense, and, similar to OOPSLA'22, our  specifications capture a module's \emph{emergent behaviour}. 
 ...
 When reasoning about external calls, even though we do not have a specification for the  {called method}, we know that the effects of the call are tamed according to the module's scoped invariants. 
...
%\vspace{.05cm}
 ...
 Now  revisit the external call from line 9 in Section \ref{sec:shop}. Our argument is that if the module satisfies $S_3$ (\ie $(A)$), and if before calling \prg{pay}, the \password\ of the shop's account is protected from \prg{buyer} (\ie $(B)$), then, after the call, the balance of that account will not have decreased (\ie $(C)$). 
 That is, we want to use $S_3$, and   establish   a Hoare triple of the form
 \\
...
% We cannot directly apply the Hoare rule as given above. Namely, to apply $S_3$, we need to establish  
However, applying $S_3$ is not that straightforward: $S_3$ preserves   
${\inside {\prg{a.key}}}$\ $\wedge$\ $\prg{a.\balance}\geq\prg{b}$.
%
%${\inside {\prg{this.\myAccount.key}}}$\ $\wedge$\ $\prg{this.\myAccount.\balance}\geq\prg{b}$. 
The problem is the first conjunct: it seems that to apply $S_3$ we need to know that ${\inside {\prg{this.\myAccount.key}}}$, while
we only know that $\protectedFrom {\prg{this.\myAccount.key}} {\prg{buyer}}$.%, while we  need the stronger property  ${\inside {\prg{this.\myAccount.key}}}$. 

 {While we cannot be certain that \prg{this.\myAccount.key} 
 {is  protected during execution of \prg{buy}\footnote{{What if one of the \prg{clients} had access to it?}}, we are certain that it  \emph{is} protected during execution} of \prg{pay}: 
{In general, more objects are protected from the viewpoint of the  {called} than from the viewpoint of the caller. -- cf Fig.  \ref{fig:ProtectedBoth}.
This is so, because the   accessible objects in the execution of \prg{pay} are those accessible from \prg{buyer}.
To express this,} we define the operator $\pushSymbolAA$ , which   translates an assertion from the viewpoint of the  {called}, to that of the caller:
...
Here we have that 
 ...
 {Below % the essential part of 
 a  Hoare logic rule %\ruleExtCallB\ from
...
To develop our logic, we     assume a  Hoare logic of  triples, whose assertions do not {have} the concept of protection.
...
%\subsection{Our Contributions}\footnote{{Perhaps drop this section?  }}
%
