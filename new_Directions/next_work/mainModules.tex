...
In this document it is all intuition -- no proofs.
...
\item
$M$ calls into $M'$\ \ \ means that $M$ calls methods from $M'$ \footnote{Sophia thinks that if $M$ does not call into $M'$ then it also has no access into the fields of $M'$}
\end{itemize}


\section{Checking combined modules}

How much do we need to check when combining modules?  How modular can the checks be?

\subsection{Combining modules with ``classical'', non-holistic, specifications, and which make no external calls}

Here, implicitly, there are no external calls.
...
Assuming that ... & The checks below ensure $M_1 \circ M_2 \models S_1 \wedge S_2 $ \\
 ...
 $M_1$ does not call outside $M_1$    & $\MCheck {M_1} {S_1} {\epsilon}$ \\
 $M_2$ calls into $M_1$   &  $\MCheck {M_2} {S_2} {S_1}$ \\
  \hline    
  
 $M_1$ calls into $M_2$    & $\MCheck {M_1} {S_1} {S_2}$ \\
 $M_2$ calls into $M_1$  &  $\MCheck {M_2} {S_2} {S_1}$ \\
  \hline \hline 
...
The above would follow from our current OOPSLA submission, and other papers on modules checking.

\subsection{Combining modules with  holistic  specifications}

 Sophia thinks that in general, if we establish $\MCheck {M_1} {S_1 \wedge S_2} {S_2}$ and $\MCheck {M_2} {S_1 \wedge S_2} {S_1}$, then we obtain that $M_1 \circ M_2 \models S_1 \wedge S_2 $. However, in some cases we can be more modular. Namely:
