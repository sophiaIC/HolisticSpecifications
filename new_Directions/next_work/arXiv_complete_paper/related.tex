...
Interestingly, even though in earlier  works% in the space
  \cite{FASE,OOPSLA22} {we} employed \emph{necessary} conditions for effects (\ie under-approximate pre-conditions), we can, instead, employ \emph{sufficient} conditions for the lack of effects (over-approximate postconditions).
...
\cite{BirkedalL:caps-mmio-conf}  {supports several different wrappers on the same object.}
%via problem-specific proofs in Iris's logic.
{Besides the distinction between the object-based and class-based approach}, our work differs from
{\cite{iris-wasm-pldi2023,vmsl-pldi2023,dd,ddd,cerise-jacm2024,BirkedalL:caps-mmio-conf,RustHornBelt-pldi2022}}
%  {these works} 
in that {they follow a ``static'' capability model whereby authority is conferred though an object's interface, whereas we follow a ``dynamic'' model whereby authority may be stored in the callee's context.}
...
\paragraphSD{Future work}  includes 
 ...
 languages that rely on lexical nesting for access control, 
{\eg}   Javascript \cite{ooToSecurity},
% rather than public/private annotations,
to languages that support ownership types,  
{\eg} Rust, %that can be
...
{features} such as %ownership and 
uniqueness\cite{funk-ownership-oopsla2024,ocaml-ownership-icfp2024}. 
{These} different language paradigms may lead to a 
{refined view of the meaning
of protection}. %, and faming operations.
{We also want to investigate tighter integrations with the underlying Hoare logic and   its framing operators.}
%
