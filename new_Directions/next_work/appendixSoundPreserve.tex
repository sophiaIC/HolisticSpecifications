...
\label{l:sfs}
For any sequences of variables $\overline x$, $\overline y$, $\overline z$, $\overline v$, $\overline w$, and assertions $A$, we have
\begin{enumerate}
\item
\label{l:sfs:zero}
...
\label{l:sfs:three}
 ...
 \ \ \ \Longrightarrow\ \ \ 
 \exists \overline {y_0}.[\  \overline {y_0}\subseteq \overline {y} \ \wedge \ \overline v \, = \, \overline {y_0}; \overline w \ ]$
\end{enumerate}
...
$
\\
The above gives that $\overline {v_2} = \overline {w}$. Thus, from (c) and the above we obtain  $ \overline v =  \overline {y_0}; \overline {w}$; and (a) gives $\overline {y_0}\subseteq \overline {y}$
\end{enumerate}
...
\item 
\label{l:calls:r:two}
$\fv(\Ao) \subseteq  \overline{v_1}; \overline{v_2} $\footnote{As we said earlier. this gives  also that the variable sequences  are pairwise disjoint, \ie $\overline{v_1}\#\overline{v_2}$.}
%\item 
...
$\satDAssertFrom M  \sigma k   \Ao[\overline {v_3/v_1}] [\overline {\alpha_2/v_2}] \ \    \wedge\ \ M, \sigma' \models \intThis  $
 \\ 
$\strut \hspace{2cm}  \hfill \Longrightarrow  \ \ \  \   \ \satDAssertFrom M  {\sigma} k  {\Ao[\overline { \alpha_2/ v_3}]}$.
