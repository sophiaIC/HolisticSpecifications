...
\begin{lemma}
 \label{l:sfs:eight}
For   assertion  $A$, variables $\overline {x}$, $\overline v$, $\overline y$, $\overline w$, $\overline {v_1}$, and $\overline {v_2}$,  addresses 
$\overline {\alpha_x}$, $\overline {\alpha_u}$, $\overline {\alpha_y}$, $\overline {\alpha_v}$, and $\overline {\alpha_{v_1}}$
...
$\overline v \txteq  \overline {v_1}; \overline v_2$, \ \ \ $\overline {v_1}\txteq  \overline y\cap \overline v$, \ \ \ $ \overline {\alpha_{v,1} } \txteq  \overline {v_1}[\overline{\alpha_v/v} ]$
\end{enumerate}
...
We first prove that  \ \  (**) \ \ $\forall z\in \fv(A)[\ z [\overline{y/x}] [\overline{\alpha_y/y}]   \txteq z [\overline{\alpha_x/x}] [\overline{\alpha_{v,1}/v_1}] $.
 
Take any arbitrary $z\in \fv(A)$. Then, by part \ref{l:sfs:eight:one} and (*) we have that either $z\in \overline x$, or $z\in \overline {y,1}$, or 
$z\in \overline {w}$.
 ...
 [  {1st Case}]  
 $z\in \overline x$. 
 Then, there exists some $y_z\in \overline y$, and some $\alpha_z \in \overline {\alpha_y}$, and    $z[\overline{y/x}]=y_z$  and
  $y_z [\overline{\alpha_y/y}] = \alpha_z$. 
  On the other hand,   by part \ref{l:sfs:eight:two} we obtain, that  $z[\overline{\alpha_x/x}] =\alpha_z$.
  And we have that $\alpha_z[\overline{\alpha_{v,1}/v_1}] $=$\alpha_z$. 
  This concludes the case.
  \item
   [  {2nd Case}]  
$ z\in \overline {y,1}$, which means that $ z\in \overline {y}\cap\overline{v}$. Then, because $\overline x \# \overline  v$, we have that $z[\overline{y/x}]=z$. 
...
And because $\overline {y,1} \subseteq \overline y$, we also obtain that $z[\overline {\alpha_{y,1}/y_1}]$=$z[\overline {\alpha_{y}/y}]$.
This concludes the case.
  \item
     [   {3rd Case}]  
  $z\in \overline {w}$. Because $\# \overline {y} \overline {w}\# \overline {x}$, we obtain that $z [\overline{y/x}] [\overline{\alpha_y/y}]$=$z$,
  and $z[\overline{\alpha_x/x}] [\overline{\alpha_{v,1}/v_1}] $=$z$.
  This concludes the case
\end{description}
...
\item
Assume  \\
  $\satDAssertFrom M  \sigma k   \Ain[\overline {v_3/v_1}] $.  \ \ \ \ \ By Lemma ... this  implies that \\
$\satDAssertFrom M  \sigma k   \Ain[\overline {v_3/v_1}][\overline { {\interpret \sigma {v_3} }/v_3 }] $   \ \ \ \ \ By Lemma ... we obtain \\
$\satDAssertFrom M  \sigma k   \Ain[\overline {\alpha_{1,3}/v_1}]  [\overline { \alpha_{6,1}/v_{6,1}}] $ \\ 
$\strut \hspace{2cm}  
      $ where $\overline {\alpha_{1,3}} \triangleq (\overline {v_1} [\overline {v_3/v_1}])[\overline { {\interpret \sigma {v_3} }/v_3 }]$, \ \   
       $\overline{v_{6,1}} \triangleq \overline{v_2} \cap \overline{v_3}$, \ \ 
   $\overline {\alpha_{6,1}} \triangleq  \overline {v_{6_1}} [\overline {\interpret {\sigma} {v_6}/v_6 }]$ \\
$\strut \hspace{2cm}  $ The above, and Lemma ... give that\\
$\satDAssertFrom M  \sigma k   \Ain[\overline {\alpha_{1,3}/v_1}]  [\overline { \alpha_{6,1}/v_{6,1}}]  [\overline { \alpha_{6,2}/v_{6,2}}]$ \\ 
$\strut \hspace{2cm}  
      $ where     $\overline{v_{6,2}} \triangleq \overline{v_2} \setminus \overline{v_3}$, \ \ 
       $\overline {\alpha_{6,2}} \triangleq  \overline {v_{6_2}} [\overline {\interpret {\sigma} {v_6}/v_6 }]$ \\
 The assertion above is variable-free.Therefore,  by Lemma xxxx we also obtain\\ 
$\satDAssertFrom M   {\sigma'} k   {\Ain[\overline {\alpha_{1,3}/v_1}]  [\overline { \alpha_{6,1}/v_{6,1}}]  [\overline { \alpha_{6,2}/v_{6,2}}]}$ \\
The above gives \\
$\satDAssertFrom M  {\sigma'}  k  { \Ain[\overline{  \interpret {\sigma'} {v_1}/v_1}]  [\overline {  \interpret {\sigma} {v_6}/v_6 }]}$\\
Again by Lemma yyy, we obtain \\
