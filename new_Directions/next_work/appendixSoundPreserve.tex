...
In this section we  prove   lemma \ref{l:calls}, which is about the preservation of assertions when pushing and popping of frames.
This lemma  will be used when we argue soundness of the rules about method calls, as given in Fig. \ref{f:calls}. 
. We will first state the lemma, then discuss it, and and then will state and prove a sequence of auxiliary lemmas which we will use in the proof of lemma \ref{l:calls}.
...
The guarantees from
 (\ref{l:calls:callee:one})  and (\ref{l:calls:caller:one})  are  used to prove soundness of rule {[\sc{Call\_Int}]}.
  ...
  (one can see an example demonstrating the need for a local callee in \ref{???}),
...
as well as rule {[\sc{Call\_Ext\_Adapt}]}.
  ...
  The guarantees from
(\ref{l:calls:callee:three}) and (\ref{l:calls:caller:tthree}) are  used to prove soundness of rule {[\sc{Call\_Int\_Adapt\_Strong}]}.

The lemma's validity hinges on the following facts: The heap does not change. The locally visible objects decrease upon entry to a method body. 
