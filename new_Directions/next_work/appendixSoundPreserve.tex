...
In this subsection we introduce some notation and some lemmas which deal with these concepts.

\begin{definition}[Sequences, Disjointness, and Disjoint Union]
...
\item 
\label{l:calls:r:two}
$\fv(\Ao) =  \overline{v_1}; \overline{v_2} $\footnote{As we said earlier. this gives  also that the variable sequences  are pairwise disjoint, \ie $\overline{v_1}\#\overline{v_2}$.},
\ \ \ 
$\fv(\Ao[\overline {v_3/v_1}]) =  \overline{v_3}; \overline{v_4} $, \ \ \ \ 
...
$\strut \hspace{2cm}  \hfill \Longrightarrow  \ \ \  \   \ \satDAssertFrom M  {\sigma'} k 
 {\Ao[\overline { {\interpret {\sigma} {v_6}} / {v_6} } ] }$
...
$\satDAssertFrom M  \sigma k   \Ao  \ \  \wedge\ \ \DepthSt {\sigma'} \geq k  \ 
$ \\ 
$\strut \hspace{2cm}  \hfill \Longrightarrow  \ \ \  \   \satDAssertFrom M  {\sigma'} k   {\Ao[\overline {v_3 / v_1}]  }$ 
