...
variables  $\overline{v_1}$,    $\overline{v_2}$,  $\overline{v_3}$,   $\overline{v_4}$,    $\overline{v_5}$,    $\overline{v_6}$, and  $\overline{v_7}$,
addresses $\overline \alpha$ and $\overline \alpha_3$, %and $\overline \alpha$, 
and statement $stmt$, and frame $\phi$.
...
\item 
\label{l:calls:r:two}
$\fv(A) =  \overline{v_1}, \overline{v_2}, \overline {v_3}, \overline {v_4} $\footnote{This fives us, implicitly, also that $\overline{v_1}\#\overline{v_2}$, $\overline{v_1}\#\overline{v_3}$, ... etc.}
\item 
\label{l:calls:r:three}
 $\overline{v_7} \#\ fv(A) $, \ \  and 
\item 
\label{l:calls:r:four}
 $\overline {\interpret {\sigma'} {v_1} } = \overline {\interpret {\sigma} {v_1} }$, \ \ and\ \  $\overline {\interpret {\sigma'}  {v_2} } = \overline {\interpret {\sigma} {v_6} }$, \ \ and \ \  $\overline {\interpret {\sigma}  {v_3} } = \overline {\alpha_3}$,   
\end{enumerate}
 then 
\begin{enumerate}
...
$\sigma'=\sigma  \pushSymbol \phi  \ \ \ \  \wedge\ \ \  dom(\phi)=\overline{v_1},\overline{v_2} \Longrightarrow$

\begin{enumerate}
\item
\label{l:calls:callee:one}
$\satDAssertFrom M  \sigma k   A[\overline {v_6/v_2}] [\overline {\alpha_3/v_3}] \ \    \wedge\ \ M, \sigma' \models \intThis  
% $\\
 \hfill  \Longrightarrow\ \ \ \ \ \ \  \ \satDAssertFrom M  {\sigma} k  {A[\overline { \alpha_3/v_3}][\overline { {\interpret \sigma {v_4}} / v_4}]}$.
...
$\sigma'= (\sigma \popSymbol)[\overline{ v_7\! \mapsto\! \alpha}][\prg{cont}\!\mapsto\! stmt]
 \ \ \ \ \wedge \ \ \ \overline v_4 = \emptyset\ \ \ \  \Longrightarrow$ 

\begin{enumerate}
\item
\label{l:calls:caller:one}
$\satDAssertFrom M  \sigma k   A[\overline {\alpha_3/v_3}] \ \  \wedge\ \ \DepthSt {\sigma'} \geq k  \wedge\ \ M, \sigma' \models \intThis
\hfill \Longrightarrow   \ \ \ \ \ \ \  \   \satDAssertFrom M  {\sigma'} k   {A[\overline {v_6 / v_2}][\overline { \alpha_3/v_3}]}$ 
