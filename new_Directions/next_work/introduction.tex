...
\forget{Reasoning about effects limited  through capabilities is the remit of this paper.}
\sue{Reasoning about precluded effects is a special case of reasoning about effects limited through capabilities, which is the remit of this paper.}
...
\paragraph{The Object Capability Model}\footnote{SD says: check whether these two paragraph caliry the "necessary" nature}
...
\forget{As described by} Mark Miller's \cite{MillerPhD}
...
as capabilities. 
\forget{Object references cannot be forged; each refers to a
...
encapsulation is encoded in the relationships between the objects,
and the interactions between them.\forget{-- interactions which can
...
\textit{``Object capabilities make it possible to write secure
  programs''} \cite{capeFTfJP}\forget{ in ways other security models simply do
not, }although they cannot guarantee that any particular program uses the provided mechanism to be correct
or even secure.
%
