...
\subsection{Deep adherence}%\footnoteSD\red{better term than deep? 

% MOVED

text text text
MOTIVATE: (*) Method rerunn establishes post-condtion at the end of the method body (at the callee's side), but not necessarily also at the caller's side. 
GIVE EXAMPLE.
This means, that it is possible for an assertion $A$ which has no free variables to be valid at the $k^{thi}$  frame, but invalid at the $k-1^{st}$ frame.
