...
\end{lemma}
 ...
 \red{TODO: define $\externalexec$.    NOTE I deleted  Arising. And also deleted  GloballyReachable -- only needed for $\forall$ in ssertions}
...
\begin{itemize}
\item
 $\measure {A, \sigma,A',A''} {M,\Mtwo}  $ = $(m, n)$, \ \ \  $\triangleq$ 
\begin{itemize}
\item
$m$ is the minimal number so that $\exists \sigma'.[\ \leadstoBoundedStarFin {M\cdot \Mtwo} {\sigma}    {\sigma'}\ ]$ in $m$ execution steps,  
 \item
  $n$ is minimal depth of all proofs of $M \vdash \quadruple {A} {\sigma.\prg{cont}} {A'} {A''} $ in $P$,
\end{itemize}
 \item
...
\newcommand{\SPS}{\strut \ \ \hspace{1cm} \ \ }
...
$\leadstoBoundedStar  {\Mtwo\cdot M}  {\sigma_2}  {\sigma''}$. In the first case, $\leadstoBoundedStar  {\Mtwo\cdot M}  {\sigma_1}  {\sigma''}$. 
Then, because $(A_1,\sigma_1,A_2, A'') \ll_{M,\Mtwo,P} (A_1,\sigma,A_3, A'')$, we can apply the inductive hypothesis, and obtain $(\gamma)$, ie that \ \ $ M, \sigma'' \models A''$. The second case is analogous.
%
