...
\noindent
And for module  $M$, state $\sigma$, and $k\in \mathbb{N}$ with $k\leq \DepthSt {\sigma}$, and assertions $A$:
\begin{itemize}
\item
$  \satDAssertFrom M  \sigma k   A  \ \  \ \triangleq \  \ \ fv(A)=\emptyset \ \wedge\  k\leq  \DepthSt {\sigma} \ \wedge \  \forall i\geq k.[\ M,{\RestictTo {\sigma}{i}} \models A \ ]$ 
\end{itemize}
\end{definition}
}
...
\sdN{
\begin{lemma}
For a state $\sigma$, numbers $k,k'\in \mathbb{N}$, assertion  $A$:
\begin{enumerate}
\item
$ \satDAssertFrom M  \sigma { \DepthSt \sigma}   A \ \Longrightarrow\ M,\sigma \models A\ $
\item
$fv(A)=\emptyset \wedge A \mbox{does. not mention $\inside{...}$} \wedge M,\sigma \models A \ \Longrightarrow \
 \forall k  \leq  \DepthSt \sigma.[ \ \satDAssertFrom M  \sigma k   A \ ]$
\item
$k\leq k'\  \wedge\ \satDAssertFrom M  \sigma {k}  A \ \   \Longrightarrow \ \ \satDAssertFrom M  \sigma {k'} A$ 
\end{enumerate}
\end{lemma}
}

\sdN{
\begin{definition}%[???call what??]
\label{def:restrict}
For modules $\Mtwo$, $M$, state $\sigma$, and $k\in \mathbb{N}$ with $k\leq \DepthSt {\sigma}$, and assertions $A$, $A'$ and  $A''$, we define   $ {\satDAssertQuadrupleFrom \Mtwo  M  \sigma   {A} {A'} {A''} }$: 
\begin{itemize}
\item
$ {\satDAssertQuadrupleFrom \Mtwo  M  \sigma   {A} {A'} {A''} } \ \ \triangleq \ \ $  \\
$\strut \hspace{.5cm} \forall k,\overline{\alpha}, \overline{x}, \sigma',\sigma''.[
\   k \leq  \DepthSt {\sigma} \, \wedge\,  \ fv(A)=\overline x \, \wedge\,    {\GRelevant {\overline \alpha}  \sigma}\, \wedge\, \satDAssertFrom M  \sigma k   {A[\overline{\alpha/x}]} \  \ \Longrightarrow$\\
...
We now move to the semantics of \SpecLang Specifications, and   define what it means for  a module  $M$ to satisfy specification  $S$, written as $M \vDash S$.  
 ...
 \subsection{Deep Semantics}
\label{sect:HLmeans}

We  define the deep {\emph {meaning}} of  our Hoare triples, $\triple {A} {stmt} {A'}$,  in the usual way, \ie that execution of $stmt$ in a state that satisfies $A$ leads to a state which satisfies $A'$.  
...
\sdN{$\strut  \ \ \ \forall    \Mtwo. \forall  \sigma.[ \ \   \arising{\sigma}{M\madd \Mtwo }\   \wedge\  \sigma.\prg{cont}\txteq stmt\   \Longrightarrow \ 
%$\\ $\strut \hspace{2.7cm} 
...
\forall stmt.[\  \satisfiesD {M} {\quadruple {\externalexec \wedge A} {stmt} {A} {A} } \ ] $
}
% \ \forall   \Mtwo. \forall  \sigma.
%$    \\ $\strut   \ \ \ \  \  \ \ \ \ 
% [ \ \   \arising{\sigma}{M\madd \Mtwo }\   \Longrightarrow \  {\satDAssertQuadrupleFrom \Mtwo  M  \sigma   {\externalexec \wedge A} {A} {A} }\ \ ]$
%}
  \item
 \sdN{$\satisfiesD{M} { \mprepostN {A_1}{p\ C}{m}{y}{C}{A_2} {A_3'}    }\  \ \ \   \triangleq  \ \ \ $}\\ % , \overline y.$ \\ % \Mtwo. \forall  \sigma$.    \\
\sdN{
$\strut  \ \ \   \ \ \ \ \ \ \ \ \   \ \ \     \forall   y_0.[ \ \  \satisfiesD {M} {\quadruple  {\ y_0:C,{\overline {y:C}}   \wedge  A_1'\ }   
    {u:=y_0.m(y_1,..y_n)} 
    {A_2' }
     {A_3' }
  \  \  ]$ \\
$\strut  \ \ \  \mbox{where} \ A_1' \triangleq A[y_0/\prg{this}],\  \  A_2' \triangleq A'[u/result,y_0/\prg{this}],\ \ A_3' \triangleq A_3[y_0/\prg{this}]} $
  }
%$\strut   \ \ \ \  \  \ \ \ \ \ \ \ \ \   [ \ \   \arising{\sigma}{M\madd \Mtwo }\   \wedge\  \sigma.\prg{cont}\txteq {u:=y_0.m(y_1,..y_n)}\   \Longrightarrow $\\
%$\strut \hspace{2.7cm} 
%{\satDAssertQuadrupleFrom \Mtwo  M  \sigma   {\ y_0:C\, \wedge\, \overline {y:C} \, \wedge\,  A\ } { A'[u/result] } {A''} } \ \ ]$%  \ \ \  ]  $
 \item
...
\subsection{Deep Semantics of Hoare triples and quadruples}
\label{sect:HLmeans}
