...
\newcommand{\modelsD}{\models\!\!\!\!_{_{\circ}}\ } %\downarrow \ } %\!\!\!\!_{_\bullet}\ }
 \newcommand{\satAssertFrom}[4]{\ensuremath{#1,#2 ,{#3} \models {#4} }}
 \newcommand{\satAssertQuadrupleFrom}[6]{\ensuremath{#1\madd#2,#3  \modelsD \, \{\, #4\,\}\,  \{\, #5\,\} \,  \{\, #6\,\} }}
...
\begin{itemize}
\item
$  \satAssertFrom M  \sigma k   A  \ \  \ \triangleq \  \ \ fv(A)=\emptyset \ \wedge\  k\leq  \DepthSt {\sigma} \ \wedge \  \forall i\geq k.[\ M,{\RestictTo {\sigma}{i}} \models A \ ]$ 
\end{itemize}
\end{definition}
}
...
\begin{enumerate}
\item
$ \satAssertFrom M  \sigma { \DepthSt \sigma}   A \ \Longrightarrow\ M,\sigma \models A\ $
\item
$fv(A)=\emptyset \wedge A \mbox{does. not mention $\inside{...}$} \wedge M,\sigma \models A \ \Longrightarrow \
 \forall k  \leq  \DepthSt \sigma.[ \ \satAssertFrom M  \sigma k   A \ ]$
\item
...
\label{def:restrict}
For modules $\Mtwo$, $M$, state $\sigma$, and $k\in \mathbb{N}$ with $k\leq \DepthSt {\sigma}$, and assertions $A$, $A'$ and  $A''$, we define   $ {\satAssertQuadrupleFrom \Mtwo  M  \sigma   {A} {A'} {A''} }$: 
\begin{itemize}
\item
...
%$\\ $\strut \hspace{2.7cm} 
{\satAssertQuadrupleFrom \Mtwo  M  \sigma   {\  A\ } { A'  } {true} } \ \ ]$%  \ \ \  ]  $
}
...
\sdN{$\strut  \ \ \  \forall    \Mtwo. \forall  \sigma.[ \ \   \arising{\sigma}{M\madd \Mtwo }\   \wedge\  \sigma.\prg{cont}\txteq stmt\   \Longrightarrow \ 
{\satAssertQuadrupleFrom \Mtwo  M  \sigma   {\  A\ } { A'  } {A''} } \ \ ]$%  \ \ \  ]  $
}
