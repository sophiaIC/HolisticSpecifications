...
\begin{itemize}
\item
$  \satDAssertFrom M  \sigma k   A  \ \  \ \triangleq \  \ \ %\fv(A)=\emptyset \ \wedge\ 
...
$\fv(A)=\emptyset\  \wedge  \  \Stable A \ \wedge\  M,\sigma \models A \ \Longrightarrow \
...
\end{lemma}
 ...
 \red{TODO: define $\externalexec$.    NOTE I deleted  Arising as well as Gobally Reachable}
...
$\strut \hspace{2.3cm}\ \ \  \ \ \ \mbox{where } \overline z=\fv(A)\setminus\fv(stmt.\prg{cont})\ \ \ ]$
\end{itemize}
\end{definition}
}
...
$\strut \ \ \hspace{2.3cm} \ \ (3) \ \ \overline z = \fv(A)\setminus \fv(\sigma.\prg{cont})$\\
...
$\strut \ \ \hspace{2.3cm} \ \ (5)\ \ M\ \vdash\  \quadruple {A_1} {s_1} {A_2} {A''}$,\ \ \ \  and\\
$\strut \ \ \hspace{2.3cm} \ \ (6)\ \  M\ \vdash\  \quadruple {A_2} {s_2} {A_3} {A''}$.

Define $\sigma_1$ as  $\sigma_1 \triangleq \sigma[\prg{cont} \mapsto s_1]$.
Therefore,  $(A_1,\sigma_1,A_2, A'') \ll_{M,\Mtwo} (A_1,\sigma,A_3, A'')$. 

Assume that $\leadstoBoundedStarFin {\Mtwo\cdot M}  {\sigma}  {\sigma'}$. Then, by lemma \ref{lemma:subexp} we obtain that there exists a $\sigma_1'$ such that $\leadstoBoundedStarFin {\Mtwo\cdot M}  {\sigma_1}  {\sigma_1'}$,
and $\leadstoBoundedStarFin {\Mtwo\cdot M}  {\sigma_2}  {\sigma'}$, where $\sigma_2 \triangleq \sigma_1'[\prg{cont} \mapsto s_2]$. Moreover,  $(A_2,\sigma_2,A_3,A'') \ll_{M,\Mtwo} (A_1,\sigma,A_3, A'')$.
