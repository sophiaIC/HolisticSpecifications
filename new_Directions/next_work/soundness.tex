...
For this, we give a stronger, deeper meaning to our Hoare tuples  (Sect \ref{s:deep:valid}), which requires that validity of assertions is preserved for all frames 
...
\subsection{Deep Validity}%\footnoteSD\red{better term than deep? 
\label{s:deep:valid}
% MOVED

As we saw in section \ref{ }, method return establish a post-condtion at the end of the method body (at the callee's side), but not necessarily also at the caller's side. 
For example, consider a state $\sigma$, such that $\interpret {\sigma} {\prg{this}}=o_1$, and $\interpret {\sigma} {x}=o_2$,  and where 
$\interpret {\sigma} {\prg{this}.f}=o_3$, and $\interpret {\sigma} {\prg{this}.f.g}=o_4$, and $\interpret {\sigma} {x.f}=o_3$, where there are no other fields or objects,
 and where all objects except for $o_3$, are internal.
We call a method $x.m()$, whose body is $x.f:=x$. 
At the end of the method body  we have that $\inside {o_4}$. 

 $\interpret {\sigma} {\prg{this.f}}=o_3$, and $\interpret {\sigma} {x}=o_2$
 
 n which we call a method $\prg{x}.m()$, whose body 
This means, that it is possible for an assertion $A$ which has no free variables to be valid at the $k^{th}$  frame, but invalid at the $k-1^{st}$ frame.

To addrresss (*) we introduce "deep satisfaction" of assertions $ \satDAssertFrom M  \sigma k   A$ which says that $A$ is satisfied in $\sigma$  in all frames of $\sigma$ form $k$ onwards,
\cf Def. \ref{def:restrict}.
And introduce "deep" quadruples,   $\satisfiesD {M} {\quadruple  {A} }   {stmt}   {A'} {A''}$ , which promise that if $stmt$ executes in a state which satisfies $A$ from $k$ onwards, then, if ir terminates, the final state will satisfy $A'$ from $k$ onwards, and also, all intermediate external states will satisfy $A''$ from $k$ onwards - \cf Def \ref{def:restrict}.
