...
Nevertheless, we want to be able to deduce that an assertion which holds at the end of a method body, also holds upon return. For this, we introduce a  
"deeper" notion of assertion satisfaction, which requires that an assertion   not only is satisfied from the viewpoint of the top frame, but also from the viewpoint of all fames from $k$-th frame onwards: % \emph{\strong satisfaction}: % of assertions: 
