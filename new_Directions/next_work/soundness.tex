...
%\\ \\
%  \Basic {\protectedFrom \re {\overline {u}}} &  \triangleq& true 
%& & 
%  \Basic  {\internal \re} &  \triangleq & true  %   \PushAS y  {(\external \re)} & \triangleq &   {\external \re}
%& &
%    \Basic {\re} &  \triangleq&   true %    \PushAS y  {(\internal \re)} &  \triangleq & {\internal \re}
%\\  \\
%   \Basic {(\re:C)} &  \triangleq&   true
 \end{array}
 \\ 
   \Basic {\protectedFrom \re {\overline {u}}} \ = \   
  \Basic  {\internal \re}  \ = \   %   \PushAS y  {(\external \re)} & \triangleq &   {\external \re}
    \Basic {\re} \ = \     
     \Basic {\re:C}\   \triangleq \    true
%\end{array}
\end{array}
...
$fv(A)=\emptyset\  \wedge  \  \Basic A \ \wedge\  M,\sigma \models A \ \Longrightarrow \
...
\forall \sigma.[\  \satisfiesD {M} {\quadruple {\externalexec \wedge A} {\sigma} {A} {A} } \ ] $
 ...
 \sdN{
$\strut  \ \ \   \ \ \ \ \ \ \ \ \   \    \forall   y_0,\overline y, \sigma[ \ \ \ \sigma\txteq {u:=y_0.m(y_1,..y_n)} \ \ \Longrightarrow \ \ \satisfiesD {M} {\quadruple  {A_1'} }  
  {\sigma}  
    {A_2' }
     {A_3' }
  \  \ \  ]$ } \\
