...
$\sigma=(\phi\cdot\_, \_)$ and $\Relevant \alpha \phi \sigma$. % for some $\phi$
\item
$\GRelevant \alpha \sigma$  \ \ iff\ \  
$\sigma=(\_\cdot\_\phi\cdot\_, \_)$ and $\Relevant \alpha \phi \sigma$. % for some $\phi$
\end{itemize}
\end{definition}
...
\sdOct{\textbf{NOTE}  This allows for the ugliness that we can write assertions like $\iota_330.bal > 700$ which obviously makes no sense without a $\sigma$. On the other hand, $a.bal > 700$ does not make sense without a $\sigma$ either.
}
...
$\satisfiesA{M}{\sigma}{\all{x:C}{A}}$ \ \ \ iff \ \ \  
 \sdOct{$\forall \alpha.[\ \GRelevant \alpha \sigma \wedge  \satisfiesA {M}{\sigma} {\alpha:C}  \ \Longrightarrow   \ \satisfiesA{M}{\sigma} {A[x/o]}\ ]$.} 
...
$\satisfiesA{M}{\sigma}{\ex{x:C}{A}}$ \ \ \ iff \ \ \  
 \sdOct{$\exists \alpha.[\ \GRelevant \alpha \sigma \wedge  \satisfiesA {M}{\sigma} {\alpha:C}  \ \wedge \ \satisfiesA{M}{\sigma} {A[x/o]}\ ]$.} 
...
\item
\label{cProtected}
$\satisfiesA{M}{\sigma}{\protectedFrom{\sdOct{\re}} {\sdOct{\re_{orig}}}}$  \ \ \ iff \\
  $\exists \alpha, \alpha_{orig}. [\ $
\begin{enumerate}
\item $\eval{M}{\sigma}{\sdOct{\re}}{\alpha} \ \ \ \wedge\ \ \  \eval{M}{\sigma}{\sdOct{\re_{orig}}}{\alpha_{orig}}$, \ \ \ \ \ \ and  
\item
\sdOct{$\alpha \neq \alpha_{orig}$}, % \footnote{!!!! \  NOT THAT SURE HERE, but think so !}
\ \  \ \ \  and 
\item 
\sdOct{$\forall n\in\mathbb{N}. \forall f_1,...f_n.[\ \interpret{\sigma}{\alpha_{orig}.f_1...f_n}=\alpha \ \wedge\ \ \forall k<n.\interpret{\sigma}{\alpha_{orig}.f_1...f_k}\neq \alpha$\\
\strut \hspace{2.5cm} \  $ \Longrightarrow \ \ \satisfiesA{M} {\sigma} {\internal{{\alpha_{orig}.f_1...f_{n-1}}}}$}
\end{enumerate}
\strut \hspace{.4cm} $]$
