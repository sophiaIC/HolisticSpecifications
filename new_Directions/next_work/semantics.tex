...
\strut \hspace{1.3in}  \sdN{for all  $o$ such that $\sigma \models \GRelevant o$, where $x$ is free in $\sigma$ } 
\item
\label{quant2}
$\satisfiesA{M}{\sigma}{\ex{x}{A}}$ \ \ \ iff \ \ \  
$\satisfiesA{M}{\sigma[x \mapsto o]}{x:C} \ \ \wedge \ \ \ \satisfiesA{M}{\sigma[x \mapsto o]}{A}$, \\
\strut \hspace{1.3in}  \sdN{for some $o$ such that $\sigma \models \GRelevant o$, where $x$ is free in $\sigma$. }
\item
\label{cInternal}
...
\sdN{Both existential and universal quantification (defined in \ref{quant1} and \ref{quant2}) is done over all objects which are transitively 
...
\footnote{\sdN{. TODO: Do we prove the implications as in TACAS, or just rely on TACAS? -- perhaps the former, since we have some new primitives? hmhh}}
...
\footnote{\sdN{TODO Notice that we assume that $\overline x$ are not free in $A$ -- cf Barendregt convention.}}
