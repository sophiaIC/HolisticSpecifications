...
NOTE: In the introduction show a method where
\begin{lstlisting}
...
We assume that any untrusted object may point to any other untrusted object.
...
NOTE: Do we need to worry about well-fomedness? Eg what if we had an assertion $3 \wedge 5 \rightarrow 66$?
...
\subsection{Semantics of Assertions - second part: Protection}

An object $o$ is protected from another, external, object $o'$, if $o'$ can obtain access to $o$ only if 
some internal object is involved in giving access to that object. In other words, $o$ cannot become reachable to $o'$ unless some internal 
...
JAMES' comment: If is possible that "we" do not know the complete heap (eg we only know about the green stuff.) Nevertheless, if the objecta are "riobust" then we can prove that some properties will be preserved. 

\footnote{
OLD COMMENTS: ${\inside {\_}}$  is central to thining about capabilities. For example, the balance of an account whose
...
Figure \ref{fig:ProtectedFrom} illustrates the concepts  ....
% protected. On the first line, the   three diagrams illustrate $\sigma_1$, a state with one frame on the stack ($\phi_1$), and   a heap with 7 objects. 
...
We continue the definitions in \ref{def:chainmail-semantics}, \ref{def:chainmail-protection} and  define   
\begin{enumerate}
\item
...
accessible any frame in the stack (as in OOPSLA). But note that $\satisfiesA{M}{\sigma} {\inside {y}}$ only considers objects that are locally reachable -- see later on 
...
TODO: use the diagram from Fig. 10 to explian deep and shallow execution
...
\subsubsection{Arising and \sd{Arising} External States}
