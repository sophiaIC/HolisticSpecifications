...
\footnote{TODO: Compare with Dafny (or others). Does univ. quantifier mean quantify over all allocated objects (Dafny), or over reachable objetcs?}
...
$o$ is ``introduced'' to some external object (not necessarily $o'$, but external) by some internal object $o''$.  Here we use ``introduce'' in the Mark Miller sense, whereby $o$ is is ``introduced'' to $o'$ by $o''$, iff either $o''$ sends to $o'$ a message containing $o$, 
or $o'$ made a call to $o''$, and $o''$ eventually returns  $o$ as the result of the call.
} 

\sdN{Even though the motivation for protection comes from considering eventual executions, we can obtain sufficient conditions just by observing the heap. In the definition below, we say that $o$ is protected from $o'$ if the last object on any path from $o'$ to $o$ is internal. 
...
\sdN{TODO: discuss the alternative, weaker definition, and explkain that lemma \ref{lemma:internalProtects} would not hold}
...
We highlight the protected objects in each pane with a golden hallo.
 ...
 \textbf{TODO: praise the lemma below}
 ...
 [ \  \satisfiesA{M}{\sigma}{\external {z_o}} \ \Longrightarrow\   \satisfiesA{M}{\sigma} {z_i} {\alpha}\ ]\ \  ] $ \\
