...
We  use the term \emph{\tamed  effect} for   an effect which may be caused by an external object, but only if the external objects satisfies certain conditions specific to that effect -- 
while the literature has studied open calls \cite{Lars},  to our knowledge, it does not have a term for that concept. % of \tamed effect.   
%
...
Can we follow a sytematic discipline to develop these proofs,
e.g.\ embodied a Hoare logic, rather constructing proofs that are 
