...
\paragraph{\Tamed effects} Nevertheless, not all open calls have unlimited effects:
...
\sdN{If the programming language supports some form of encapsulation (\eg private fields, no forging of addresses, \etc), then the internal module can be written so as to ensure that external objects can cause effects only if some conditions specific to that effect are met.
For example, the DAO can be implemented so as to ensure that an external object cannot cause
 the reduction of the balance of the DAO unless the causing object is one of the DAO's account holders; moreover, an external object should not cause the balance of the DAO to become less than the sum of the valances of the accounts it is holding\cite{DAO}.\footnote{\red{AUTHORS: I prefer that example}}
}
...
\sdN{\Taming of effects %is important, because it 
allows to reduce the uncertainty at the point of the open call.}
...
of a bank account (whether positive or negative)  only if the causing object  can obtain access to that account} moreover
an external object may cause a change   in the balance 
...
\sdN{In general,  the guarantees given by \tamed effects  are  hypothetical: In general there is no guarantees that the untrusted object (line 6 above) has no access to the capability. In the case where the untrusted object has no access to the corresponding capability, the effect can be excluded. Moreover, leaking if the capability can also be excluded.}
...
invariants, they propose interesting problems for open calls,
but can only prove these properties through problem-specific proofs
rather than a systematic Hoare logic. Similarly, 
Birkedahl-PLDI'23  proves a property of an open call, but again with
...
1) a specification language to to describe  \taming of effects for  programs which employ the OCAP model, 
 ...
 \paragraph{Stucture of this paper}
