...
\begin{minipage}{.45\textwidth}
  ...
  While method \prg{m1} contains only trusted code,
  method \prg{m2} takes an untrusted parameter \prg{untrst},
  and then at line 6
  calls an unknown external method \prg{unkn} on that parameter,
...
\citet{CassezFQ24}  handle external calls by replacing them through an unbounded number of calls to the module's public methods.
 ...
 are not concerned with indirect, eventual access to capabilities.
...
the Hoare logic. 
\sdN{Sect.\ \ref{sect:example:proof:short} summarises  the proof of  our running example; also mechanised at \red{XXXX}}. 
%Sect.\ \ref{sect:related} discusses related
%work and
 Sect.\ \ref{sect:conclusion} concludes. 
\sdN{More details in the appendices in the accompanying material.}
