...
% But  under some circumstances we know that the external call will not include certain effects.
%
%%{\textbf
%{Our remit
% is not to rule out effects caused by external objects, % at the external call,  
% but to {\tame} them:}
% %}
...
%For example, the 
%  DAO can be implemented so as to ensure
...
\sdN{We say that a module has \emph{\tamed effects}, when external calls can be guaranteed not to cause  \emph{some}  effects. % are not possible at external calls. 
}\footnote{
...
\sdN{
 Some \tamed effects are independent  on the state of the external objects. \eg 
  the DAO's balance never becoming lass than the sum of balances  of  it accounts does not depend on the external object,
Other \tamed effects  depend of the state of the external objects, \eg the DAO also guarantees that its  balance will not be reduced  unless the causing, external, object is one of tits account holders.
...
1) a specification language to to describe  \taming of effects for  programs which employ the OCAP model, 
 2)  a Hoare logic of quadruples which can reason about external calls and prove that module's emergent behaviour indeed \tames the effects as specified.
