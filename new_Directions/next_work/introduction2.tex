...
It looks as if the possible effects are limitless. % We should use "limit" or "unlimited" because later we say "not all clls will have unlimited effects"
...
programmers    rely on various kinds of  encapsulation -- \eg if address forging were possible, the range of potential effects from external calls would be limitless.
...
Sect.\ \ref{sect:underlying} outlines a simple object-oriented language used for our work, which in Sect. \ref{s:auxiliary} is augmented derived concepts essential to   our study.
