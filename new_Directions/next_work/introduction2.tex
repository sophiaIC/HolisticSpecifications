...
\paragraph{\Tamed effects}  In practice, not all external calls will have unlimited effects.
 ...
 calls \se{have limited effects}  on internal components.} %  can cause effects only if some conditions specific to that effect are met.
...
\forget{Relying on techniques from separation logic, they intend their proofs
e.g.\ to support library components that can be reused in different
contexts.}
\forget{--- which is implicitly support in our distinction between
external and internal code.
 \footnote{\red{TO CHECK}\kjx{I have looked at it - good enough for
 me.}}}
  \citet{dd} \forget{work with Kripke logical relations describing
...
object capabilities.  %\citeauthor{dd} 
They propose and solve several
problems involving external calls \forget{, 
but require proofs specific to each individual problem,
rather than a systematic Hoare logic.} Similarly,
\citet{vmsl-pldi2023} \forget{deploy seperation logics, the IRIS framework
\cite{iris-jfp2018}, and mechanistic metamathematics
to} prove properties of programs involving external calls.\forget{, but again
...
\se{\paragraph{This paper's contributions} % In this paper we propose 
{1) a specification language to describe \taming of effects,
with new assertions to describe access to capabilities, 
2) a Hoare logic to reason about external calls and to prove that modules satisfy their \tamed effects specifications,
3)  proofs of soundness,
4) a worked illustrative example.
}}
