...
In this section  we use our Hoare logic from Section \ref{sect:proofSystem} to \sdN{outline the proof} that the \verb|buy| method
...
\\
 we \sdN{outline the proof} that $M_{good} \vdash S_2 \wedge S_3$.
\sdN{We  also show why $M_{bad} \not\vdash S_2$.}

\sdN{We first extend the semantics and the logic to deal with scalars (\S \ref{s:app:scalars}). 
...
\sdN{We extend the notion of protection to also allow apply to scalars. }
...
\sdN{The definition from above gives rise to further cases of  protection; we supplement the triples from 
Fig. \ref{f:protection} with some further inference rules, given in in Fig. \ref{f:protection:conseq}.}
...
We also introduce a rule for conditionals in Fig. \ref{f:if}, \sdN{where we expect the obvious syntax and semantics for $Cond$}
...
For this, we  add a return type to each of the methods; We add an explicit assignment to the variable \prg{res}: and We   add a temporary variable \prg{tmp} to which we assign the result of our \prg{void} methods.
For simplicity, we allow sequences of field reads, \eg \prg{this.accnt.blnce} rather than using   temporaty variables, as 
in \prg{tmp1} := \prg{this.accn};  \prg{tmp2} := \prg{tmp1.blnce}. We also allow the shorthands \prg{+=} and \prg{-=}.
...
\sdN{ To write our proofs more succinctly, we will use \prg{ClassId}::\prg{Methodid}.\prg{body} as a shorthand for the method body of \prg{Methodid} defined in \prg{ClassId}.}

 \subsubsection{Proof outline for $M_{good} \models S_2$}
...
\end{align*}
\normalsize
\sdN{Thus, we need to prove the following three Hoare quadruples:}
...
\sdN{We will only prove (1), as the proofs of (2) and (3) are simpler.} 
\sdN{We prove (1) in Lemma \ref{l:sub:one}}
...
\begin{proofO}
\newcommand{\Aparams}{\prg{A}_{decls}}

\sdN{We will be using the shorthand\\
 $ \SPTG  \Aparams\ \triangleq \  \prg{this}:\prg{Shop}, \prg{buyer} : \prg{external}, \prg{anItem} : \prg{Item},\, \prg{a}:\prg{Account},\, \prg{price} : \prg{int}$}.\\
That is, want to prove
...
\sdN{We prove (1)} using the quadruple rules defined in Figure \ref{sect:proofSystem}. 
By our Hoare triple extension and the \textsc{Mid} rule that raises triples to quaruples, the first two lines
of \prg{buy} preserve the precondition in (1) above\footnote{\red{NOT THE CASE}}, and further that 
\small
$$
\begin{array}{ll}
...
\small
\begin{align*}
M_{good} \vdash & \{ \ \Aparams \   \wedge\    {\inside{\prg{a.key}}}\  \} \\
...
\red{Do we prove the above, or do we prove instead the below?}
\red{\begin{align*}
M_{good} \vdash & \{ \ \Aparams \   \wedge\    {\inside{\prg{a.key}}}\  \} \\
		  		& \SPTG\prg{ tmpAcc:= this.accnt;}\\
				& \SPTG\prg{  tmp := buyer.pay(tmpAcc, price)}\ \\  
		  		& \{ \  \ \Aparams \  \wedge\ {\inside{\prg{a.key}}} \ \} \ \ \  || \ \ \ 
		  		   \{ \  {\inside{\prg{a.key}}}\  \}
\end{align*}
}
...
$\SPTG \SPTG (9a)\ M_{good}   \vdash_{ul}   \{\ \prg{a.key}=z\wedge z\txtneq \prg{res} \ \} \ {\prg{res:=0}}\ \{ \ {\prg{a.key}=z}\ \}$.\\
 Then, we apply   {\sc{embed\_ul}}, and  {\sc{Prot-1}}, and  {\sc{Mid}}, and obtain \\  
$\SPTG \SPTG (9b) \  \hprovesN {M_{good}}   {\inside{\prg{a.key}} \ }  {  \prg{res:=0} } { \  {\inside{\prg{a.key}} \ } } {\ \inside{\prg{a.key}}\ } $.\\
Moreover, we apply that \prg{res} has the scalar type \prg{int}, and this fact, together with 
   {\sc{Prot-Int}}$_1$, gives\\  
$\SPTG \SPTG (9c)\ \hprovesN {M_{good}}    { \  true \ }  { \  \prg{res:=0}\  } { \   \protectedFrom {\prg{a.key}} {\prg{res}} \ } {\ \inside{\prg{a.key}}\ } $.\\
We combine (9b) and (9c), apply   {\sc{Combine}}, and obtain (9)
}

\sdN{We combine the earlier steps\footnote{unfortunately they do not all have names, and some are nested, \ie we are not meant to apply them}, use the fact that $\protectedFrom {\prg{a.key}} {\prg{res}}$=${\PushASLong {\prg{res}} {\inside{\prg{a.key}}}}$, apply rules {\sc{Sequ}} and {\sc{Consequ}}, and have the proof of (1)}
...
%				   \protectedFrom{\prg{a.key}}{\prg{this.accnt}} \\
%				& \wedge \prg{this.accnt.blnce} \neq \prg{oldBlnce+price}\} \\
%		  		&\SPTG  \prg{buyer.tell("you have not paid me")}\ \\  
%		  		& \{  \prg{this}:\prg{Shop}, \prg{buyer} : \prg{external}, \prg{anItem} : \prg{Item},\, \prg{a}:\prg{Account},\, \prg{this.accnt}:\prg{Account}\\
%				& \prg{price} : \prg{int},\,
%				  \wedge\, 
%				  {\inside{\prg{a.key}}}\ \wedge\ 
...
\subsubsection{Demonstrating that $M_{good} \vdash S_3$}
...
\sdN{This proof is similar to the one in the previous section}
