...
{\prg{Private} methods  {may be} called by objects of the same module,  while \prg{public}  methods  may be called by \emph{any} objects {provided the dynamic types of the arguments are the same as} the formal parameter types.} Our model of execution is entirely unsurprising with a stack, a heap, and states.
...
\subsection{Challenge: Reason about external calls (using protection)}
\label{sec:how}
...
Assume that {our internal module includes the classes} \prg{Shop}, \prg{Account}, and \prg{Inventory}.  \prg{Shop} exports a public method \prg{buy} whose formal parameter \prg{buyer} is an \prg{external}   object  with a method { \prg{payMe}}.
