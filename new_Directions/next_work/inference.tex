...
For {\sc{Call\_Int\_Adapt}} we require an adapted pre-condition ($  \PushAS {y}{A_1}$  rather than $A_1$) and also ensure the adapted post-condition ($\PushAS {y}{A_2}$, rather than $A_2$).
...
% \footnote{Appendix~\ref{app:proof} shows stronger alternative rules.}

%%%%%%%%%%%%%
{For external methods,  {\sc{Call\_Ext\_Adapt}}, we consider the module's invariants. If the module promises to preserve $A$, \ie if  $\promises M   {\TwoStatesN {\overline {x:D}} {A}}$, and   $  \PushAS {y}{A}$  holds before the call, then it also holds after and during the call. }
...
\\
\inferruleSD 
{~ \strut \hspace{6.5cm} {\sc{invariant-old}}}
{  
\begin{array}{l}
