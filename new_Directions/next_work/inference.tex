...
\subsection{Soundness of a Hoare Logic}

We define 

\sd{
\begin{definition}[Sound Hoare triples]

$M_{int}\ \models\  \{\, A \,  \}\ e\  \{\, A' \, \}$ \\
iff\\
 for a all $M_{ext}$, for all $\sigma$ such that $\sigma \in Arising ...$ \\
If $M_{int},\sigma \ \models \ A$, and 
$\sigma.cont$=$e$, and $M_{int}\circ M_{ext}, \sigma \leadstoUp \sigma'$, and $\sigma'.cont$ is a value, 
\\
then \\
$M_{int},\sigma' \ \models \ A'$
\end{definition}
}
