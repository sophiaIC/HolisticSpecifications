...
\inferrule[\sc{ExtCall}]
	{ 
		M \vdash A\ \rightarrow \ (\  \external  z \ \wedge \external  {\overline {u}}  \ \wedge\ \internal {\overline y} \ ) \\
		\overline v \mbox{ is a permutation of } \overline {u} , \overline {y}  \\
...
$\sigma=(\psi,h)$, and $\sigma'=(((\overline {x \mapsto \sigma(x)}, {x \mapsto \sigma(x)}), cont)\cdot \psi, h)$,  
we have

\begin{itemize}
\item
$M,\sigma\ \models\  A$  \ \ implies \ \  $M,\sigma' \ \models\  \lift {A} {\overline x}  {\overline z} $
\item
$M,\sigma'\ \models\  A$  \ \ implies \ \  $M,\sigma  \ \models\  \llower  {A}  $\footnote{\sd{TODO should lower have more arguments?}}
\end{itemize}
\end{lemma}
