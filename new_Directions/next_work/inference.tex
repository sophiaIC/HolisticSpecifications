...
% it is possible to know that a certain object will be protected ($\inside \re$) after pushing a frame, if we know enough about the arguments of that frame. 
...
%%% WRONG: \ \ (\ref{lemma:push:ass:state:three}): \ When calling from external states we obtain the adapted version. % (Lemma \ref{lemma:internal:adapted}).}
}
...
frame $\phi$,  variables $\overline y$: % with $Range (\phi)=\overline {\interpret \sigma y}$:
...
%$%\fv(A)=\emptyset \ \ \wedge \ \ 
%M, \sigma  \models  A  \wedge \extThis  \ \wedge \ M\cdot\Mtwo \models \PushSLong {\phi} {\sigma}   \ \  \ \ \  \  \Longrightarrow  \ \ \ \ M, \sigma \models \PushASLong  {Rng(\phi)} {A}$
\end{enumerate}
\end{lemma}
}
...
% 
%  shows that   the \prg{extl}  and \prg{intl}
%  requirement is necessary for (\ref{lemma:push:ass:state:two}) and (\ref{lemma:internal:adapted}) above, and that  original and adapted versions of assertions are not always comparable.
...
%(\ref{lemma:internal:adapted})   
 $\sigma_2= \PushSLong {\phi_2} {\sigma_1}$, and   
$\sigma_2 \models \inside{o_1}$,  and 
$o_1\in Rng(\phi_2)$.
But, since $o_1=o_1$, we also have 
...
% above they 
$\Pos{\_}$ are not necessarily preserved against popping of such frames \cf Ex. \ref{ex:pop:does:not:preserve}.
