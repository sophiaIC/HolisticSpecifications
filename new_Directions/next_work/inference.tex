...
Before describing the three phases, we introduce some preliminaries.
The judgement    $\promises M S$ expresses that $S$ is guaranteed by the module. 
...
Def. \ref{d:promises}, and Ex. \ref{e:rename}. We also require an underlying Hoare logic.
...
These promise, as expected, that any execution of $stmt$ in a state   satisfying $A$ leads to a state sartisfying $A'$.
...
\small{
\begin{array}{c}
\inferruleSD{\hspace{0.5cm} [\sc{extend}]}
	{ M \vdash_{ul} \{ \ A\ \} {\ stmt\ }\{\ A'\ \} \ \ \ \ \ \ \ \ \ \ 
...
\footnote{The restriction to statements which do not contain method calls in rule {\sc{types-1}} was required to make this lemma valid}, and therefore its execution will not reach any external states.
