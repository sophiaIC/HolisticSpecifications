...
\begin{itemize}
\item
 \sdN{For statement $s_1$ and $s_2$, we write $s_1 \txtin   s_2$  when $s_1 \txteq  s_2$ or  $s_2 \txteq  s_1; s_3$ for some $s_3$.}
 \item
 \sdN{ For specifications $S_1$ and $S_2$, we write $S_1 \txtin   S_2$  when $S_2 \txteq  S_1$, or  $S_2 \txteq  S_1 \wedge S_3$, or $S_2\txteq S_3 \wedge S_1$,  or   $S_2 \txteq S_3 \wedge  S_1 \wedge S_4$ for some $S_3$, $S_4$.}
\end{itemize}
...
Encapsulation of an assertion does not imply encapsulation of its negation; 
 for example,  ${\inside{x}}$ is encapsulated, % (as per below), 
 but $\neg  {\inside{x}}$ is not.
...
is described in Fig. \ref{f:internal:calls}. {\sc{Call\_Int}} for internal methods, whether public or private;  \  {\sc{Call\_Pub}} for public methods;   {\sc{Call\_Ext}} 
...
$\begin{array}{c}
 \inferruleSD{\hspace{4.7cm} [\sc{Call\_Int}]}
	...
	{  \hprovesN {M} 
						{ \  y_0:C  \wedge\ \overline {y:C} \wedge {\PushAS {y}{A_1[y_0/\prg{this}]}}  \ } %\wedge A_3)}}}
						{ \ u:=y_0.m(y_1,.. y_n)\    }
...
\sdN{For internal methods, {\sc{Call\_Int}},  we  look  up (one of) the method's specifications. 
...
We require a weaker pre-condition ($  \PushAS {y}{A_1}$  rather than $A_1$) and also ensure a weaker post-condition ($\PushAS {y}{A_2}$, rather than $A_2$).
...
For external methods,  {\sc{Call\_Ext}}, we apply similar treatment as for  {\sc{Call\_Pub}}.
...
Finally, the variables  $\overline x$ from E{\TwoStatesN {\overline {x:D}} {A}}$ can be renamed, so as to allow the specification to be applied to any object.
}
...
{  
 \prg{mBody}(m,D,M)=p \ (\overline{y:D})\{\  stmt \ \}  %\ \ \vee \ \   \prg{mBody}(m,D,M)=\prg{public} \ \overline{x:C}\{\  stmt \ \}       
    \\
  {\hprovesN{M} { \ \prg{this}:\prg{D}\, \wedge\, \overline{y:D}\, \wedge\, A_1\  } {\ stmt\ } {\ A_2 \ }   {A_3} } 
}
{
...
{  
\forall  D\in M,  m:\ \ \  \ \  \prg{mBody}(m,D,M)=\prg{public} \ (\overline{y:C})\{\  stmt \ \}      \ \ \Longrightarrow  
    \\
    \begin{array}{l}
   ~ \strut \hspace{1.7cm}[ \ \ \ \  \  %SD chop: do not know whay I had added \ A_{inv} \triangleq \overline{x:C}\, \wedge\, A
%  \\
%  ~ \strut \hspace{1.7cm} \ \ \ \ \  \ \ \ 
 {\hprovesN{M} { \ \prg{this}:\prg{D}\, \wedge\, \overline{y:D}\, \wedge\, \overline{x:C}\, \wedge\,  A\  } {\ stmt\ } {\ A\ } {\ A \ }  } \ \ \ \ \  \ \ ]
\end{array}
}
{
...
\sdN{{\sc{WellFrm\_Mod}} and 
 ...
 \red{Here we apply implicitly a form of Barendregt convention, and assume, wlog, that no free variable  in $A_1$   clashes with a variable  in $stmt$.}
 %Any free variable  in $A_1$ which does not come from the formal parameters ($\overline{y}$), and clashes with a variable  in $stmt$,  gets   consistently renamed   in $A_1$, $A_2$ and $A_3$.}
 }
 ...
 \red{We apply again,   a similar form of Barendregt convention, and assume, wlog, that none of
