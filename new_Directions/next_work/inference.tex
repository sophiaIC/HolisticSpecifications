\newcommand{\extract}[1]{\ensuremath{{\mathcal E}xtr(#1)}}
...
For this, we define in Def. \ref{def:extract}, the function $\extract{\_}$, which turns the holistic specification of a module into an assertion. 
This assertion has to be satisfied before any external call.}
...
$\begin{array}{c}
%\begin{array}{ll}
\inferruleSD{ [{\sc{IntCall1}]}}
	{ 
	{  u\not\equiv x \ \ \ \ \ \   u\not\equiv x'\  }
	}
	{   \hproves{M}  { {\internal{y_0}}\  \wedge\ \ x=x'\   }	{ \ u:=y_0.m(y_1,.. y_n)\  } { \  x=x'\ }	 }

\\ \\ 
\inferruleSD{ [\sc{IntCall2}]}
	{ 
		{ \ u\not\equiv x\ \ \ \ \  \ u\not\equiv x'\   }
	}
	{   \hproves{M}  {  \internal{y_0}\ \wedge  x\neq x' }  { \ u:=y_0.m(y_1,.. y_n))\  } { \  x\neq x'\ }	 }
\\
\\
...
\\
\\
\inferruleSD{ [{\sc{ExtCall1}]}}
	{ 
	{  u\not\equiv x \ \ \ \ \ \   u\not\equiv x'\  }
	}
	{   \hproves{M}  {  \external{y_0}  \wedge \  \PushAS {y}{\extract{M}}\  \wedge\ \ x=x'\     } 	
	{ \ u:=y_0.m(y_1,.. y_n)\  } { \   \PushAS {y}{\extract{M}} \  \wedge\ x=x'\ }   
	}	
	\\ \\ 
\inferruleSD{ [\sc{ExtCall2}]}
	{ 
		{ \ u\not\equiv x\ \ \ \ \  \ u\not\equiv x'\   }
	}
	{   \hproves{M}  {  \external{y_0}  \wedge \  \PushAS {y}{\extract{M}}\  \wedge\ \ x\neq x'\     } 	
	{ \ u:=y_0.m(y_1,.. y_n)\  } { \   \PushAS {y}{\extract{M}} \  \wedge\ x\neq x'\ }   
	}
%\end{array}
\\
\\
...
\inferruleSD{\hspace{4.7cm} [\sc{ExtCall\_WithSpec\_Weak}]}
	...
	{   \hproves{M} 
						...
						{ \ u:=y_0.m(y_1,.. y_n)\    }
						{ \ \PushAS  {y} { A_2} \,  \wedge\,    \PushAS {y}{\extract{M}}   \ }	
}
}
\\
\\
...
\inferruleSD{\hspace{4.7cm}[\sc{ExtCall\_WithSpec\_Strong}]}
	...
	{   \hproves{M} 
						...
						{ \ u:=y_0.m(y_1,.. y_n)\    }
						{ \  \ \PushAS  {y} { A_2}\,  \wedge\, A_2 \,  \wedge\,   \PushAS {y}{\extract{M}}\   }	
}}
\\
\\
\end{array}
...
$ M, C, m  :  (\, P, \ Q\, )  $ to indicate that the declaration of method $m$ for class $C$ in module $M$ includes the per-post-condition pair  $(\, P, \ Q\, ) $. 
 ...
 {In the rules for external calls,  {\sc{ExtCall1}} and {\sc{ExtCall2}}, we use the  assertion push-function $\pushSymbol$ with  signature\\
...
\item
 Does the consequence rule require that the assertions are encapsulated? And if an assertion is encapsulated, is its consequence also encapsulated? TODO
\item
...
TODO explain. 
\item  Wened to add some dynamic type checking to the language, ie the method call crashes if actual params do not fir the formal types. OR we type them all as \prg{Object}.
\end{enumerate}
}
