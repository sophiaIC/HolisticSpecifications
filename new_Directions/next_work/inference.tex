...
is described in Fig. \ref{f:internal:calls}. {\sc{Call\_Int}} describes calling of internal methods, whether public or private;   {\sc{Call\_Pub}} describes calling public methods;   {\sc{Call\_Ext}} 
describes calling. external methods.
...
%		\promises {M} {\TwoStatesN  {\overline {x:C}} {A}}
	...
	{  \hprovesN {M} 
						{ \  y_0:C  \ \wedge\ \overline {y:C}\ \wedge \ {\PushAS {y}{A_1}}  \ } %\wedge A_3)}}}
						{ \ u:=y_0.m(y_1,.. y_n)\    }
						{ \ { \PushAS {y}{(A_2[ u/result]}}  \ }
						{  {A_3} }	
}
%% SD
%% DOES NOT HOLD AS IS
...
%		}
%	{  \hprovesN {M} 
%						{ \  y_0:C  \, \wedge\ ,\overline {y:C}\, \wedge \,  \overline{x:D}  \wedge\, {A}\ }
%						%\wedge A_3)}}}
%						{ \ u:=y_0.m(y_1,.. y_n)\   }
%						{   \ {A} \ }
%						{\  \red { A}\  }	
...
For internal methods, {\sc{Call\_Int}}, we start by  looking up (one of) the method's specifications: $ {\mprepostN{A_1}{\prg{private} C}{m}{y}{C}{A_2} {A_3}}  $  indicates that if $m$ starts in a state that satisfies $A_1$, then  its execution will lead to a state that satisfies $A_2$, and
...
Moreover, we require a weaker pre-condition ($  \PushAS {y}{A_1}$  rather than $A_1$) and also ensure a weaker post-condition ($ PushAS {y}{A_2}$, rather than $A_2$).
