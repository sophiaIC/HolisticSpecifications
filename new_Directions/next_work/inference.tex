...
\PushAS y  e\  =\  e & & \PushAS y   {(A_1  \wedge  A_2)}\   =\   \PushAS y  { A_1}  \wedge   \PushAS y  {A_2}  & & 
 \PushAS y { (A_1 \vee  A_2)}\  = \  \PushAS y {A_1}  \vee  \PushAS y {A_2} 
  \\ \\
\PushAS y  {(\neg A)}\  =\  \neg( {\PushAS y A} ) & &
 \PushAS y  {(\forall x:C.A)}\  =\  \forall x:C.({\PushAS y A} )&  &  
  \PushAS y  {(\exists x:C.A)}\  =\  \exists x:C.({\PushAS y A} )
 \\ \\
\PushAS y  {x:C}\  =\  x:C 
& &
\PushAS y  {(\internal x)}\  =\   {\internal x}
& &
\PushAS y  {(\external x)}\  =\   {\external x}
\end{array}
\\
\\
\begin{array}{cclcl}
& & \sdN{\PushAS y {(\inside x)}  \ = \ \protectedFrom x {\overline {y} \ \wedge\ x\neq \overline y} }
&\ \  &   \PushAS y {(\protectedFrom x {\overline {u}})} \ = \ \red{\protectedFrom x  {\overline {u}} } % {(\overline {y'}\setminus \overline {y})} 
  \\
  \\
\end{array}
\end{array}
