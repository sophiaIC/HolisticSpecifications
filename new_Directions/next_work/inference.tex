...
We will develop a  Hoare logic with judgments of the form  $M\ \vdash\  \{\, A \,  \}\ e\  \{\, A' \, \}$ which promise that execution of the expression $e$ in a configuration with satisfies $A$ will lead to a configuration that satisfied $A'$. We define the {\emph {meaning}} of such triples in the usual way, \ie that execution of $e$ leadst to a state which satisfies $A'$. In the definition below we require that $\sigma'.cont$ is a value in order to ensure that $e$ has been executed. \footnote{ \sd{TODO: Also, will need add more requirements  -- or use different rewrite relation --- to ensure that we did not pop the top frame}.}
...
$\strut ~ ~ ~ ~ [ \ \ M,\sigma \ \models \ A \ \wedge\  
 \sigma.cont$=$e$ $\ \ \wedge\  \ M\circ M', \sigma \leadsto^* \sigma' \ \ \wedge\ \ \sigma'.cont$ is a value
 \\
...
We also define soundness of a Hoare logic in the classical way:

\begin{definition}[Soundess of Hoare logic]
...
\inferrule[\sc{ExtCall}]
	{ 
		% old M \vdash A\ \rightarrow \ (\  \external  z \ \wedge \external  {\overline {u}}  \ \wedge\ \internal {\overline y} \ ) \\
		 M \vdash A\ \rightarrow \   \external  z  \hspace{4cm} \\
	        % old 	\overline v \mbox{ is a permutation of } \overline {u} , \overline {y}  \
		\promises M   {\TwoStatesQ {\overline {x:C}} {A_1}{A_2}}  \hspace{4cm} \\
		M \vdash {\lift A  {\{z,\overline {u}\}}  {\overline y}} \  \rightarrow \ (\  {\overline {x:C}}\ \sd{\wedge\ A_1}\  )
%	      \\\\
...
%	\\\\
%	M \vdash {\lift A  {\{z,\overline {u}\}}  {\overline y}} \  \rightarrow \ (\  {\overline {x:C}} \wedge A_1 \, \rightarrow\, A_2\  )
%	\\ \\ 
%	
		}
	{   \hproves{M} 
...
\sd{We assume that $\leadsto$ is determinstic up to renaming of addresses \footnote{TODO: revisit}}
...
$m_{cl}$ is the length of the maximal sequence of proof steps in "classical Hoare logic, ie excluding a step {\sc{ExtCall}} involved in proving that  ie excluding a step {\sc{ExtCall}}, and the $m_{ext}$ is the number of external calls that occurred ... \sd{TODO: this needs to be refined!}

\end{proof}

For the proof of the lemma above, we use the following two lemmas that give guarantees about preservation of properties when pushing new frames onto the stack, and when popping frames from the stack. We use the notation $\sigma \bullet \phi$ to indicate that the frame $\phi$ has been pushed on top of the 
...
We define some further 

\newcommand{\leadstoRec}[3]{\ensuremath{#1 \leadsto_{!#2} #3}}
\newcommand{\leadstoLoc}[3]{\ensuremath{#1 \leadsto_{!!#2} #3}}

\begin{definition}
We define the relations and //
\begin{itemize}
\item
$\leadstoRec {\sigma_1} {\sigma} {\sigma_2}$ iff ${\sigma_1} \leadsto {\sigma_2}$ and $\sigma = (\phi\cdot\psi, h)$, $\sigma_1 = (\psi_1, h_1)$ and
$\sigma_2 = (\psi_2, h_2)$ implies that  ...
\item
$\leadstoLoc {\sigma_1} {\sigma} {\sigma_2}$ iff ${\sigma_1} \leadsto {\sigma_2}$ and ( $\sigma = (\phi\cdot\psi, h)$ $\sigma = (\phi\cdot\psi, h)$, $\sigma_1 = (\psi_1, h_1)$ and
$\sigma_2 = (\psi_2, h_2)$ implies that  ... , method call and return.
\end{itemize}
\end{definition}

Note that $\leadstoLoc {\sigma_1} {\sigma} {\sigma_2}$  does not imply $\leadstoRec {\sigma_1} {\sigma} {\sigma_2}$, nor the other way round.
