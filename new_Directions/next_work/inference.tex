...
In all rules above  we require that the receiver and actual parameters are textually disjoint from the formal parameters and other free variables in the method's precondition, \ie that $\sdN{y_0,\overline y \ \#\ \prg{this},\overline x, \fv(A_1) }$ or that $\sdN{y_0,\overline y\  \#\   \overline x}$. For example, given a method $m$ with $\promises  M {\mprepostN{z_a:C_a \wedge ....}{...\ C_b}{m}{x_c}{C_c}{...} {...}} $ calls of the form $z_a.m(...)$, or  $x_c.m(...)$, or $\prg{this}.m(...)$ would not type check.
...
%through the implicit renamings defined in  Def. \ref{d:promises}, or
 through  the introduction and initialization of variables. For the example from above, we can introduce a fresh variable  $v$ and
