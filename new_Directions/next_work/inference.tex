...
\\ \\
	...
	\\
	{\hproves{M}  {\sdred{A \wedge} \re\!=\!z} {\ stmt\ }{ \re\!=\!z} }
	}
	{\hproves{M} 
						{\  \sdred{A \wedge} \inside{\re}  \ }
						{\  stmt \ }
						{\  \inside{\re}\ }
	}
}
& &
	...
	{
	\hproves{M} 
						{\ \sdred{A \wedge} \protectedFrom{\re}{\re'}\ }
						{\ stmt }
						{\ \protectedFrom{\re}{\re'}\ }
	}
}
...
\sdred{ As  discussed in \S  \ref{sec:howThird},   the $\pushSymbolInText$ operator  adapts an assertion from the view of the callee to that of the caller,
...
\sdred{substituting the formal  by the actual parameters parameters ($\prg{this}, \overline{x}$  by $y_0,\overline{y}$).} % in the method's pre- and post-condition.
...
%-- cf.  lemma \ref{lemma:push:ass:state}.
 ...
 \sdred{is} applicable whether the method is public or private.


For external calls, % {\sc{Call\_Ext\_Adapt}}, 
we consider the module's invariants. 
If the module promises to preserve $A$, \ie if  $\promises M   {\TwoStatesN {\overline {x:D}} {A}}$, \sdred{and  if its adapted version, $ \PushASLong {(y_0, \overline y)}{A}$},  holds before the call, then it also holds after  the call \ ({\sc{Call\_Ext\_Adapt}}).
\sdred{If, in addition, the un-adapted version also holds before the call, then it also holds after the call  ({\sc{Call\_Ext\_Adapt\_Strong}}).}
...
\sdred{This is sound, because  internal callees preserve  %variable-free, 
...
% $\Pos{\_}$-assertions are not necessarily preserved against pushing of frames
 \sdred{external callees do not necessarily preserve  $\Pos{\_}$-assertions} -- \cf Ex. \ref{push:does:not:preserve}. 
...
Remember that \sdred{popping frames does not necessarily preserve}
...
{  
 \prg{mBody}(m,D,M)=p \ (\overline{y:D})\{\  stmt \ \}       
    \\
  {\hprovesN{M} { \ \prg{this}:\prg{D}, \overline{y:D}\, \sdred{\wedge\, A_1}\  } %\, \sdN{\wedge\, \PushASLong {(\prg{this},\overline y)}  {A_1}}\, } 
...
{\sc{Invariant}} says that  a module satisfies \sdred{a} %an invariant 
specification $\TwoStatesN{ \overline{x:C}} {A}$,  if the method body of each public method
 has $A$ as its  pre-, post- and \midcond. 
\sdred{Moreover, the precondition is strengthened by $\PushASLong {(\prg{this},\overline y)} {A}$ -- this is sound because
...
\\
 \vspace{.03cm}
\noindent
If  $ \prg{this.key}\!=\!\prg{a'}\ \wedge\ \prg{this}\!=\!\prg{a}$, then $\prg{a.key}\!=\!\prg{key'}$, which contradicts that $\protectedFrom {\prg{a.key}} {\prg{key'}}$, and so by contradiction (\cf Fig. \ref{f:substructural:app}), we have proven   \textbf{(7?)}.
When  $\prg{this.key}\!\neq \!\prg{a}$, we would obtain from the underlying Hoare logic that the value of \prg{a.key} did not change. Thus, we can apply rule {\sc{Prot\_1}}, and obtain  \textbf{(7?)}. \ \ \ More details in \S \ref{l:set:sat}
\\
 
 \vspace{.06cm}
\noindent
This also demonstrates why \prg{set} from $M_{bad}$ cannot be proven to satisfy $S_3$. Namely, it % \prg{set} from $M_bad$ 
 does not have the condition \prg{this.key}=\prg{key'}, and would requires us to prove that\\
\vspace{.03cm}
{\small{  \begin{minipage}{.05\textwidth}
   \textbf{(??)}\ \ 
\end{minipage}
...
\\
and there is no way we can prove \textbf{(??)}.
\end{example}
