...
Proofs %of the lemma can be found
 in \A\ \ref{appendix:adaptation}. Example \ref{push:does:not:imply} shows that   the \prg{extl}  and \prg{intl}
...
\begin{example}[ ]  
\label{push:does:not:imply} Consider  Fig. \ref{fig:ProtectedBoth}.\\
--   \textit{In internal states, $A$ does not imply $\PushASLong {Rng(\phi} {A}$ after popping}:\  We have $\sigma_1 \models \inside{o_1}$, 
$o_1\in Rng(\phi_1)$But, since $o_1=o_1$, we do not have 
$\sigma_2 \not\models \protectedFrom {o_1} {o_1}.$.
%
\\
--  In internal states, \textit{$A$ does not imply $\PushASLong {Rng(\phi} {A}$}:\
