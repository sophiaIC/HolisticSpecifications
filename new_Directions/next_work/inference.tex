...
\red{Moreover, we assume that the assertions $A$, $A'$ and $A''$ do not mention protection.}
...
\\
 \inferruleSD{\hspace{4.7cm} [\sc{Call\_Int\_Adapt}]}
	...
	{  \hprovesN {M} 
						{ \  y_0:C, \overline {y:C} \wedge {\PushAS {y}{A_1[y_0/\prg{this}]}}  \ } %\wedge A_3)}}}
						{ \ u:=y_0.m(y_1,.. y_n)\    }
...
\sdN{ {\sc{Call\_Int}} is as expected for methods for which we have a specification;  We  look  up (one of) the method's specifications. 
We require the precondition, and guarantee the postcondition and midcondiition after the relevant renamings.
