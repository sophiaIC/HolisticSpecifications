...
In  Fig. \ref{f:external:calls}  we show how to reason about method calls, internal as well as external. 

\sdN{For rules {\sc{Call1}} and {\sc{Call2}} we distinguish between variables pointing to the same values $x=x'$, and variables been textually the same $u\equiv x$. It is possible that variables are aliases, without being textually the same, i.e. it is possible to have $x=x'$ while $u\not\equiv x'$ -- note that $u\equiv z$ is a textual assertion, that is, it is state-independent;  therefore it is not part of the Hoare triple's precondition.
}
...
$ M, C, m  :  (\, P, \ Q\, )  $ to indicate that the declaration of method $m$ for class $C$ in module $M$ includes the per-post-condition pair  $(\, P, \ Q\, ) $. 
 ...
 We have two rules for external calls,  {\sc{ExtCall1}} and {\sc{ExtCall2}}. In both, we are using the the  assertion push-function $\pushSymbol$ with  signature
 $ \strut \ \ \ \ \ \   \pushSymbol : Vars \times Assert \rightarrow Assert $,
 defined in Figure \ref{f:Push}. The assertion $\PushAS y A$ is \emph{hypothetical}: it guarantees
that if we were to push onto the current state a frame which contained values for $\overline y$, then $A$ would hold. This is expressed in lemma \ref{lemma:push:ass:state}, part (1). Conversely,  this means that if a state satisfies  $A$ after a top frame containing the  values of $\overline y, \overline z$ has been pushed, then 
after popping that frame, the assertion  $\PushAS y A$ will hold. This is expressed in lemma \ref{lemma:push:ass:state}, part (1).


\red{Both rule {\sc{ExtCall1}} and rule {\sc{ExtCall2}} require that the module  promises that  $\overline {x:C} \ \wedge\ A_1$ can only lead to $A_2$. Both rules require in their precondtion that right before the external call starts, that $\overline {x:C}$ and  $A_1$ will hold at the start of the external call ($\PushAS y A$). And both rules appeal to the module's promice, and guarantee that right after the external call returns, either $A_2$ or $\PushAS y {A_2}$ will hold.}

\red{Rule {\sc{ExtCall1}} says that if the state right before the external call guarantees that $\overline {x:C}$ and  $A_1$ will hold at the start of the external call ($\PushAS y A$), and that $A1$ also holds before the call, and if the module makes the promise that  $\overline {x:C} \ \wedge\ A_1$ can only lead to $A_2$, then right after the external call, $A_2$ will hold. 
..} 

\red{On the other hand, the precondition of {\sc{ExtCall2}} is slightly weaker: It does not ask that ..}
...
\inferruleSD{ [{\sc{Call1}]}}
	...
	{   \hproves{M}  { \ x=x'\ \wedge \ } 	{ \ u:= y.m(\overline y)\  } { \  x=x'\ }	 }
