...
\begin{mathpar}
\sdN{
\inferruleSD{\hspace{5.5cm} [\sc{ExtCall}]}
	{ 
	...
	{   \hproves{M} 
						{ \ A\  }
						{ \ z.m(\overline y)\  }
						{ \  A'  \ \wedge\ { \preserve  A  {(z,\overline y)} M }  \ }	
}
}
\end{mathpar}
...
\jm{I don't know that I agree with \ref{fig:si} PU-5. I think you need to check that x isn't being passed an argument, which is covered by PU-6}
...
\inferrule [PO-3]
	{ 
	{ \PopAA M  {A_1}    {A_1'} }\ \ \ \  { \PopAA M  {A_2}    {A_2'} }
	}
	{  \PopAA  M {(A_1 \wedge A_2)}     {(A_1' \wedge A_2')}
	}
\\
\\
\inferrule [PO-4]
...
\inferrule [PO-5]
	...
	{
	  \PopAA  M  {(\protectedFrom x {\overline {u}})}    {(\protectedFrom x {\overline {u}})}  
	}
 &  & 
 \\ \\
\end{array}
...
$
\label{fig:si}
\caption{The judgeement  $\PopAA M  {A }     {A'}$}
\end{figure}
}
