...
\infer
	{
	 {\TwoStatesQ {\overline {x:C}} {A \wedge \protectedFrom{y'}{x}}{A' \wedge \protectedFrom{y'}{x} }} \in HS(M)
	 \\
	{ M \ \vdash \ A_1   \Longrightarrow  A \wedge {\overline {x:C}} \wedge \external{z_1} \wedge \external{z_2} \wedge \protectedFrom{x}{z_1} \wedge \protectedFrom{x}{z_2} }
	}
	{\hproves{M} 
						{A_1}
						{\ z_1.m(z_2)\ }
						{A' \wedge \protectedFrom{x}{z_1} \wedge \protectedFrom{x}{z_2}}
	}
	\quad(\textsc{prot-3})
	\and
...
{ 
	      \\\\
		M \vdash A\ \rightarrow \  ( \ \internal x \ \wedge\  \external \{ z, z', z''\}\ \wedge \ y:C \ \wedge \ 
	%	)
	% \\\\
%	M \vdash A \rightarrow \ (
  ...
  \wedge\ \protectedFrom {y} {\{z,z',z''\} }\ )
	\\\\ 
	\strongerI M {HS(M)} {\TwoStatesQ {\overline {y:C}} {\inside y}{\inside y}} 
		}
	{\hproves{M} 
						{ A }
						{ \ z.m(z',x)\  }
...
\inferrule[\sc{ExtCall-2}]
	{ 
	      \\\\
		M \vdash A\ \rightarrow \  ( \ \internal x \ \wedge\  \external z\ \wedge\ \external {z'}\ \wedge \ y:C \ )
		\\\\
	M \vdash A \rightarrow \ ( \ \protectedFrom {y.f} {\{z,z'\} } \wedge x\neq y.f\ )
	\\\\ 
	\strongerI M {HS(M)} {\TwoStatesQ {\overline {y:C}} {\inside y.f}{\inside y.f}} 
		}
	{\hproves{M} 
						{ A }
						{ \ z.m(z',x)\  }
...
\infer
	...
	M \vdash \encaps{A}
	\\\\
	\textit{for all}\ \ % D \in dom(M), \ \textit{and}\ \  m \in M(D).\prg{exported\_mthods}, \\\\ 
	\textit{methods exported from } D,\ \textit{and } \prg{mBody}(M,D,m)=\overline{y:D}\{\  s \ \}\\\\
				% \strut \hspace{3cm}
				 [\ \  \hproves{M}{\ A_{init}  } {\ s\ } {\ A\ } \\\\
				 \wedge
				 \\\\
				% \strut \hspace{3cm}\ \ 
				 (\  s = s_1; z.m'(\overline{y}); \_\ \wedge \ 
				  \hproves{M}{\  A_{init}  } {\ s\ } {\ A'\ } 
				 \ \wedge \
				 M \ \vdash \ A' \rightarrow \external{z} \ \ 
				  \Longrightarrow\ \ M\ \vdash \ A' \rightarrow A\ \ \ ) \ \ ] \\\\
				 \textit{where} \\\\
				 A_{init} \ \triangleq \ \overline{x:C}\wedge \ A\ \prg{this}:\prg{D} \wedge\overline{y:D}\ 
	}
	{
...
\infer
	...
	M\ \vdash\ {\TwoStatesQ {\overline {x:C}} {A''} {A'''}}
	\\\\
	M \ \vdash \ {\overline {x:C}} \wedge A \rightarrow A'' \\  M \ \vdash \ {\overline {x:C}} \wedge A''' \rightarrow A' 
	}
	{
	M\ \vdash\ {\TwoStatesQ {\overline {x:C}} {A} {A'} }
	}
