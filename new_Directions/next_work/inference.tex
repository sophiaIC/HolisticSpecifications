...
\inferruleSD{\hspace{4.7cm} [\sc{ExtCall2Alternative}]}
	...
	{   \hproves{M} 
						{ \ {  \external{z}  }\ \wedge \ \PushASLong {(z,\overline y)} {( \overline {x:C}  \ \wedge\ A_1)} \   }
						{ \ u:=z.m(\overline y)\    }
						{ \ \PopASLong { (z,\overline y)} { A_2}  \ }	
}
}}
\\
\\
...
\sdN{
\begin{lemma} 
For any states  $\sigma$, $\sigma'$, variables $\overline y$, $\overline {y'}$, $\overline x$ disjoint with one another,  and assertion $A$, such that $\overline x \notin \sigma$ and $fv(A)\subseteq \overline x$:
\begin{itemize}
 \item
$M, \sigma \models \PushAS {y} {(\overline {x : C} \wedge A)}\ \ \wedge \ \ \sigma'\in\PushS {y} {\sigma} \ \ \   \Longrightarrow  \ \ \ \ M, \sigma'[\overline{x \mapsto {\interpret {\sigma} {x}}} ] \models { \overline {x : C} \wedge A} $
\item
$M, \sigma[\overline {x \mapsto {\interpret {\sigma'}  x}}]  \models \PopAS  {y} {A}\ \ \wedge \ \ \sigma'\in\PopS {y} {\sigma} \ \ \ \Longrightarrow  \ \ \ \ M, \sigma'  \models A$
\end{itemize}
\end{lemma}
}
...
\begin{itemize}
 \item
$M, \sigma \models \PushAS {y} {(\overline {x : C} \wedge A')}\ \ \ \ \ \ \   \Longrightarrow  \ \ \ \ M,  \PushS {y} {\sigma}[\overline{x \mapsto {\interpret {\sigma} {x}} } ] \models A$
\end{itemize}
