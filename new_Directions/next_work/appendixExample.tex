...
\sdN{We extend the notion of protection to also allow it to apply to scalars. }
...
\begin{proofO}
\newcommand{\Aparams}{\prg{A}_{dcls}}
\newcommand{\Alocals}{\prg{A}_{dcls,lcls}}
\newcommand{\stmtsP}{\prg{stmts}_{9,10,11}}
...
while using the shorthands\\
 $ \SPT  \Aparams\ \triangleq \  \prg{this}:\prg{Shop}, \prg{buyer} : \prg{external}, \prg{anItem} : \prg{Item},\, \prg{a}:\prg{Account},\, \prg{price} : \prg{int}$.\\
  $ \SPT  \Alocals\ \triangleq \  \Aparams, \prg{price} : \prg{int}, \prg{myAccnt} : \prg{Account},\, \prg{oldBalance}:: \prg{int}$.\\
 We also use the shorthand $\stmtsP$ for the statements on lines 9, 10, 11 of the code.
 



\step{1st Step: proving statements 9,10,11}

We apply the underlying Hoare logic and prove that the statements on lines do not affect the value of \prg{a.key}, ie that for a $n\notin \{ \prg{price}, \prg{myAccnt}, \prg{oldBalance} \}$, we have 

\begin{align*}
\text{(10)}  \ \ \ \ {M_{good} \vdash_{ul}} 
		&	\{  \ \Aparams\  \wedge\ z=\prg{a.key} \} \\
...
We then apply {\sc{Embed\_UL}}, and {\sc{Prot-1}} on (10) and use the shorthand $\stmtsP$, and obtain: 
\\
\begin{align*}
...
We apply  {\sc{Mid}},  and {\sc{Types-2}}\footnote{\red{do we need a rule that takes declarations of local variables from statements}}, on (11) and obtain 
\begin{align*}
...
\subsubsection{Demonstrating that $M_{good} \vdash S_4$}
