...
\newcommand{\Ids}{\prg{Id}_{dcls}}
\newcommand{\Aparamstr}{\prg{A}_{prms,tr}}
\newcommand{\Alocalstr}{\prg{A}_{dcls,tr}}
\newcommand{\Idstr}{\prg{Id}_{dcls,tr}}
\newcommand{\Aparamsset}{\prg{A}_{prms,set}}
\newcommand{\Alocalsset}{\prg{A}_{dcls,set}}
\newcommand{\Idsset}{\prg{Id}_{dcls,set}}
...
{ To write our proofs more succinctly, we will use \prg{ClassId}::\prg{Methodid}.\prg{body} as a shorthand for the method body of \prg{Methodid} defined in \prg{ClassId}.}
 ...
 \subsubsection{Proof outline for $M_{good} \vdash S_2$, and for $M_{fine} \vdash S_2$}
...
This leaves us with four subgoals, one for each method defined in class \prg{Account} or
 \prg{Shop}.
 That is, for each  class $C$  in $M_{good}$, and each method $m$ in $C$, with parameters $\overline{y:D}$ in the public interface of $M_{good}$, we have to prove that
 \small
\begin{align*}
M_{good} \vdash & \\
...
\normalsize
Thus, we need to prove   three Hoare quadruples, namely one for \prg{Shop::buy}, one for  \prg{Account::transfer}, and one for  \prg{Account::set}.
...
We do not prove anything for  \prg{Shop::send}, because we do not have a method body for it, and because it will have     

We outline the proof for \prg{Shop::buy} in Lemma \ref{l:sub:one} in \S{s:buy:sat}.
...
This leaves us further subgoals, one for each method defined in class \prg{Account} or
 \prg{Shop}.
 That is, for each  class $C$  in $M_{good}$, and each method $m$ in $C$, with parameters $\overline{y:D}$ in the public interface of $M_{good}$, we have to prove that
...
Thus, we need to prove   three Hoare quadruples: One for \prg{Shop::buy}, one for  \prg{Account::transfer}, and one for  \prg{Account::set}. That is:
 \small
\begin{align*}
\text{(1)}  \ \ \ \ M_{good} \vdash 
...
\\
\text{(2)}  \ \ \ \ M_{good} \vdash 
		&	\{  \ \Alocalstr\,\wedge\, {\inside{\prg{a.key}}} \, \wedge \, \protectedFrom {\prg{a.key}} {\Idstr}  \  \} \\
...
where we are using the shorthands\\
 \small
\begin{align*}
  \Aparams\ \triangleq \  &  \prg{this}:\prg{Shop}, \prg{buyer} : \prg{external}, \prg{anItem} : \prg{Item},\, \prg{a}:\prg{Account},\, \prg{price} : \prg{int}$.\\
 \Alocals\ \triangleq \ &  \Aparams,  \prg{myAccnt} : \prg{Account},\, \prg{oldBalance}:  \prg{int},  \prg{newBalance}:  \prg{int},  \prg{tmp}:  \prg{int}$.\\
 \Ids\ \triangleq \  &  \prg{this}, \prg{buyer}, \prg{anItem}$.\\
\end{align*}
...
We outline the proof (1) for \prg{Shop::buy} in Lemma \ref{l:sub:one} in \S{s:buy:sat}.
%, and the proof for \prg{Account::set} from module $M_{fine}$  in Lemma \ref{l:set:sat} in \S{s:set:sat}.
