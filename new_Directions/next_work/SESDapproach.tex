...
\vspace{.3cm}
\noindent
However, if \Mshop  makes some appropriate guarantees, then  the effects at line 9 are tamed. In particular,
...
\end{example}
 ...
 \noindent
 \textbf{NOTE} \ \  Our specifications are  in terms of externally observable effects (\eg the key stays protected), % the account's balance may increase), 
...
$
 ...
 with objects $\overline x$ of class $\overline C$ satisfies $A$, then all its \emph{scoped} external future  states will  {also} satisfy  {$A$}. 
...
This means that we are  interested in the states before and after a method (or statements), \emph{but  also}  in the external states reachable \emph{during} execution of that method (or statements).
To accommodate this, we extend   traditional Hoare triples to quadruples of  form\\
 $\strut \ \hspace{4cm} \quadruple {A} {\, stmt\, }{A'} {A''}$\\  
 % These promise 
 promising that terminating execution of $stmt$ in  states satisfying $A$  reaches    final states satisfying $A'$, and any intermediate external states reachable during execution of $stmt$ satisfy    $A''$ -- \cf Def. \ref{def:hoare:sem}.
...
\vspace{.1cm}
Moreover, we assume a  Hoare logic of  triples, and define an embedding into our quadruples. 
...
\vspace{.3cm}
In summary, we assume a Hoare logic of triples, and embed it into our logic of quadtuples. 
We add method specifications of quadruples, and scoped invariants. We extend the assertions 
to reason about external calls, ... TODO enumerate
