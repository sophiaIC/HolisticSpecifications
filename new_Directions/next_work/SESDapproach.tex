...
Now consider  modules \ModB and \ModC who differ from \ModA only in their \prg{set} methods. Whereas \ModA 's key is immutable, \ModB 's allows any client to reset an account's key at any time, whereas \ModC 's requires the existing key in order to change it.
