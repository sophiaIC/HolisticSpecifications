...
Our assertion language, \AssertLang, supports standard assertions as well \emph{object-capability} assertions. 
...
\sdN{Extended expressions, $\re$, may include addresses -- the reason is discussed later.}
\sdN{Capability assertions may be about objects being
\emph{Internal/external} or about   \emph{Protection} -- inspired by the OCAP literature.}
...
Expressions, $\re$, and assertions, $A$, in
...
\red{SD: Do you mean that there are easier ways to say what the notes say? Or thatr what they say is obvious? Would you rather we skipped them?}}
...
We go further than that, and say: "lack of eventual permission to the relevant capability implies that the given effect will \sdN{not} take place".
\susan{lack of eventual permission implies no effect ever?\red{SD: not "no effect", as other effects may take place, and also no "not ever" -- better now?.}}
...
\susan{it is easier to read with subscripts rather than primes - 3 and 4 primes look similar and need to be counted.
\red{true - is it better now?}}
\sdN{An object $o$ is \emph{protected  from} another object $o'$, if $o'$ can obtain direct access to   $o$ only if 
...
% or
 \item
\red{$\forall n\in\mathbb{N}. \forall f_1,...f_n.$
$
...
An object is protected, if it is protected from all locally reachable \sdN{external} objects. This can also be understood as 
...
\begin{enumerate}
\item
{$\forall \alpha.[ \  \LRelevant {\alpha}  {\sigma}\ \wedge\ \red{ \satisfiesA{M}{\sigma}{\external \alpha}} \ \ \Longrightarrow \ \  \satisfiesA{M}{\sigma}{\protectedFrom{\re} {{\alpha}}}\ ] $}, \ \ \ and 
\item
