...
side-effects. {{$\fv(A)$ returns the free variables in $A$; for example, $\fv(a\!:\!Account \wedge \forall b:int.[a.\balance = b])$=$\{ a \}$.}}
...
\sdN{We write $\external {\re}$ for $\neg (\internal {\re})$}, 
$\extThis$. resp. \sdN{$\intThis$} for $\external{\prg{this}}$ resp. $\internal{\prg{this}}$,%, and $\re:\prg{intl}$ as short for $ \neg \external {\re}}$. 
 and WLOG  we encode $A \rightarrow A'$,  $A \vee A'$, and $\exists x:C.A$.
...
to automated seperation-based invariants in
