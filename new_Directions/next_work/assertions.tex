...
\sdN{ In this section we discuss preservations of assertions' satisfaction   by substitution (Lemma \ref{lemma:vars:to:addresses}), or by pushing new frames onto the stack (Lemma \ref{lemma:rel:abs:prot}).
 }
 
 \sdN{ The following lemma says that the validity of an assertion $A$ in the context of a state $\sigma$ is equivalent to the 
 validity of the assertion resulting  from substituting free variables according to the top frame ($A[\sigma]$)}
  
 \sdN{ \begin{lemma}
 \label{lemma:vars:to:addresses}
  For any  module $M$, state $\sigma$, and assertion $A:$
  \begin{itemize}
 \item
...
Under some circumstances, "relative protection" ($o$ is protected from $o'$) implies absolute protection ($o$ is protected).
The lemma below says that if we make a method call, and if an object $o$ is protected from  the receiver and arguments of that method call, then in the context of this method call, $o$ is protected.
%$\oveline z$,  \textbf{TODO: praise the lemma below}
 ...
 For any  module $M$, state $\sigma$, variables $\overline u$ and $\overline z$, address $\alpha$, and statement $s:$.
  \begin{itemize}
 \item
...
% [ \  \satisfiesA{M}{\sigma}{\external {z_o}} \ \Longrightarrow\  \forall i\in 0..n.(\ \satisfiesA{M}{\sigma} {z_i} {\alpha}\ )\  \  ] $ \\
  $ \Longrightarrow$ \\
  \sdN{ $ \satisfiesA {M} {\Push {u} {\interpret  {\sigma}{z}} {s} {\sigma} } {\inside {\alpha}}$ }
 \end{itemize}
\end{lemma}
