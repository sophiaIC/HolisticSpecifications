...
%, or $\external {\re}$,
 these can be {encoded.} % with the given connectives.
...
We define satisfaction of an assertion $A$ by a % program  state $\sigma$ with 
 module $M$ \ \  through \ $\satisfiesA{M}{\sigma}{{A}}$ \ \ \ by cases on the shape of $A$, in definitions \ref{def:chainmail-semantics}, \ref{def:chainmail-protection-from}, and 
...
We highlight the protected objects in each pane with a golden halo.
 ...
 \subsection{Preservation of Sartisfaction}
...
Lemma \ref{l:assrt:unaffect} says that satisfaction of an assertion is not affected by the statements that are to be executed.

\begin{lemma}
\label{l:assrtr:unaffect}
For any modules $\Mtwo$, states $\sigma$ and statement $stmt$, and variable $y$ 
\begin{itemize}
\item
