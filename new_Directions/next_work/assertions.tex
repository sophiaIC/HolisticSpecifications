...
\sdN{supports} standard  as well \emph{object-capability} assertions. 
...
\sdN{They are about objects being
...
\sdN{Assertions, $A$,} may contain addresses; \eg   $\alpha.bal > 700$. 
...
\notesep The syntax does not %explicitly 
include the forms $A\rightarrow A'$, $A \vee A'$, ${\ex{x:C}{A}}$. --
%, or $\external {\re}$,
 these can be \sdN{encoded.} % with the given connectives.
...
\item
\label{cExternal}
$\satisfiesA{M}{\sigma}{\external{{\re}}}$ \ \ \ iff \ \ \   $\exists C.[\ \satisfiesA{M}{\sigma}{{{\re}} : C} \ \wedge \ C \notin M \ ]$
\end{enumerate}
\end{definition}
