...
\end{syntax}
\]
In the above, we expect that $f$ stands  for a field or a ghost field, but not a method -- \ie no side-effects. {\sdN{Moreover, $\fv(A)$ is defined in the obvious way to to return   the free variables in $A$; for example, $\fv(a\!:\!Account \wedge \forall b:int.[a.\balance = b])$=$\{ a \}$.}}
\end{definition}
...
\label{def:assertions:short}
We use $\extThis$ as  short  for $\external{\prg{this}}$. %, and $\re:\prg{intl}$ as short for $ \neg \external {\re}}$. 
While the syntax does not include the forms $A \rightarrow A'$, or $A \vee A'$, or $\exists x:C.A$, these can be encoded; we use these forms freely in the subsequent text.
...
We highlight the protected objects in each pane with a golden halo.
 ...
 \subsection{Preservation of Srtisfaction}


Lemma \ref{l:assrt:unaffect} says that satisfaction of an assertion is not affected by replacing a variable by its value, nor changing the continuation in a state.


\begin{lemma}
\label{lemma:addr:expr}
\label{l:assrtr:unaffect}
...
A corollary of   lemma \ref{lemma:addr:expr}.\ref{one:ad:exp} is $\satisfiesA{M}{\sigma}{A}  \ \ \Longleftrightarrow  \ \   \satisfiesA{M}{\sigma}{A[{\interpret \sigma x}/x]}$.
