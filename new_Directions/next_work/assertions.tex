...
\cf Def. \ref{def:chainmail-semantics}.(\ref{quant1}), and to two-state invariants, \cf Def. \ref{def:necessity-semantics}.(2).}
...
\noindent
\textbf{NOTE}  While execution takes place in the context of one or more modules, $\Mtwo$, satisfaction of assertions considers \emph{exactly one} module  $M$ -- the internal module. \susan{This is not surprising since the goal of this work is to ensure that external modules cannot break our (internal) module's assertions.}
...
\begin{enumerate}
\item
\sdN{$ \satisfiesA{M}{\sigma[x\mapsto \alpha]}{A}   \ \ \ \Longleftrightarrow\ \ \ \satisfiesA{M}{\sigma}{A[\alpha/x]} $}
\item
