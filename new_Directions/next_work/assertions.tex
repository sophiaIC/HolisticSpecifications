...
We use the notation $\PushA M {A} {z} {A'}$ for what we used to call "lift": namely, $A'$ is what the assertion $A$ guarantees after the values of $\overline z$ have been pushed into a name frame. Perhaps a better name would have been "pushing an assertion", or we could say something like "view at the start of  call". 
Similarly, we   use  $\PopA M {A}   {A'}$ for what we used to call "lower". Perhaps erhaps "popping" or view "after the call" would be a better name.
}


  \begin{lemma}
 \label{lemma:lifts}
Take any  module $M$, states $\sigma$, $\sigma'$, assertions $A$, $A'$,   and variables $\overline x$ and $\overline z$. If
\sdN{
\begin{itemize}
\item
  $ \satisfiesA {M} {\sigma}  {A$
  \item
  $\PushA M A {z} (\overline {x : C}} \wedge A')$  \ \ \ and \ \ \  $fv(A') \subseteq \overline x$ 
  \item
  $\sigma'\in   \Pushes {\interpret {\sigma} {z}} {\sigma}$\ \ \ and \ \ \  $ \overline x \notin \sigma'$
 \end{itemize} }
...
Take any  module $M$, states $\sigma$, $\sigma'$, assertions $A$, $A'$,  
  and variables $\overline x$ and $\overline z$. If 
\sdN{\begin{itemize}
\item
$ { \satisfiesA {M} {\sigma[\overline {x \mapsto {\interpret {\sigma'} x}}]}  {\overline {x : C} \wedge A}}$
\item
