...
Similarly, we   use  $\PopA  {\overline y} {A}   {A'}$ for what we used to call "lower". Perhaps "popping" or "from the caller's viewpoint" would be a better name.
}
...
$ { \satisfiesA {M} {\sigma'[\overline {x \mapsto {\interpret {\sigma} x}}]}  {\overline {x : C} \wedge A'}}$
\item
$\PopA  {\overline y}  {A'} A$  % \ \ \ and \ \ \  $fv(A') \subseteq \overline x$ 
\item
$\Pops {y}   {\sigma'} {\sigma}$. 
  \end{itemize}
}
