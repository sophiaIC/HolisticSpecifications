...
As   in \S \ref{sec:shop}, and also \cite{OOPSLA22}, if no external object holds eventual permission to a given capability, then the corresponding effect cannot occur.
...
\ie that   \sdNew{no locally reachable external objects have direct access to $o$.}
...
\red{We first define what it means for an object  to be protected from another object, $\protectedFrom{{\alpha}} {{\alpha_{o}}}$: 
The two objects must not be equal, and no external object reachable from $a_o$ has a field pointing to  $\alpha$.
This definition ensures  that any path leading from $\alpha_o$ to $\alpha$ should lead through an internal object.
}

\red{
We then define what it means for an object  to be protected,  $\inside{{\alpha}}$:
This requires that if the current receiver is external, then none of the arguments is $\alpha$, and 
that  no external object reachable from any of the arguments has a field pointing to $\alpha$.
This definition ensures that external objects reachable from the current receiver or arguments have direct access to $\alpha$ -- such direct access
 is either through a field, or by virtue of the receiver having acces to all the arguments.
}
...
$\exists \alpha, \alpha_{o}. [\  \eval{M}{\sigma}{{\re}}{\alpha}\ \wedge\ \eval{M}{\sigma}{{\re_0}}{\alpha_0} \  \wedge \ 
 ...
 \red{Thus,  $\protectedFrom{{\alpha}} {{\alpha_{o}}}$ ensures that $\alpha_o$ will get access to $\alpha$ only if an internal object grants that access.
...
%  
%% To define ``protected'', \sdNew{we first defne  "has no direct access to",  
...
%%\item
%%\label{dirAccess}
%%\sdNew{ $\satisfiesA{M}{\sigma}{\accessedBy {\alpha}{\alpha_{o}}} \ \ \triangleq \ \  
...
% \ \ \ \  and% \\
% \item
%\sdNew{$\forall \alpha' \in {\Relevant {\alpha_o} {\sigma}}.[\  \satisfiesA{M}{\sigma}{\external {\alpha'}} 
%\ \ \Longrightarrow \ \  
...
%%\begin{enumerate}
%%\item
%\sdNew{\forall \alpha' \in \LRelevantO  {\sigma}.[\ { \satisfiesA{M}{\sigma}{\external {\alpha'}}} \ \ \Longrightarrow \ \satisfiesA{M}{\sigma}{\noDirAccess {\alpha}{\alpha'}}\ ]}$.
...
\sdNew{Namely, $o_5$ is reachable from $o_2$, is external, and has direct access to $o_6$.}
...
\sdNew{An alternative definition might consider $\alpha$ as protected, 
if   any path from the frame to $\alpha$ goes through at least one internal object.
With this definition, $o_6$ would be protected in $\sigma_2$.
However,  $o_2$ can make a call to $o_3$, and  this call could  return $o_5$. 
Once $o_2$ has direct access to $o_5$, it can also get direct access to $o_6$. 
The example justifies our current definition. %why we require that the \emph{penultimate} object is internal.
}
