...
\sdN{A stack  is a sequence of frames, $\phi_1\!\cdot\!...\!\cdot\! \phi_n$.}
A  frame, $\phi$, consists of a local variable map, and a continuation, \ie a sequence of statements to be executed.
%\sdN{The result of pushing a frame $\phi$ onto a stack, $\phi_1\!\cdot\!...\!\cdot\! \phi_n$, is  $\phi_1\!\cdot\!...\!\cdot\! \phi_n \!\cdot\! \phi$. }
\sdN{
...
\item
$text_1 \txteq text_2$ expresses that $text_1$ and $text_2$ are textually equal.  \sdN{For statement $s_1$ and $s_2$, we write $s_1 \txtin   s_2$  when $s_1 \txteq  s_2$ or  $s_2 \txteq  s_1; s_3$ for some $s_3$.}
\item
\sdN{We define the depth of a stack as $\DepthFs {\phi_1...\phi_n} \triangleq n$, and for states, $\DepthSt {(\overline \phi, \chi)} \triangleq  \DepthFs {\overline \phi}$.}
\item
\sdN{ $\vs(stmt)$ returns the variables which appear in $stmt$; for example $\vs(u:=y.f)$=$\{u,y\}$.}
\end{itemize}
...
They are characterized through pushing/popping stack frames.  
...
\begin{itemize}
\item
 $ \PushSF  {\phi} {\sigma} \ \triangleq \ (\sdN{\overline{\phi}\cdot\phi}, \chi)$ \ \ \  if \ \ \  $\sigma=(\overline{\phi}, \chi)$.
\item
...
\item
\sdN{$ \sigma\, \popSymbol \ \ \  \triangleq\    ({\overline{\phi}}, \chi)[\prg{cont}\mapsto stmt$ \ \ \  if \ \ \  $\sigma=({\overline{\phi}\cdot\phi}, \chi)$, and for some $stmt$.}
\end{itemize}
 \end{definition}
...
Consider Fig. \ref{fig:UpSemantics} again: $\sigma_8\!\in\!   \PushS  {\phi} {\sigma_7}$ for some $\phi$ -- {thus $\sigma_8$ is a callee state for 
 $\sigma_7$}. \sdN{And $\sigma_{15}$=$\sigma_{14} \popSymbol$ --  thus $\sigma_{15}$ is the return state from 
 $\sigma_{14}$.}
...
\item
 ...
 \exists  \phi, \phi',\overline \phi. [\  \sigma\!=\!(\sdN{\overline \phi\cdot \phi},\_)\, \wedge\, \sigma\!=\!(\sdN{\overline \phi\cdot \phi'},\_)\ ] \ \  \vee\  \ 
 \exists \phi,\sigma''.[\  \EarlierS {\sigma} {\sigma''}\,  \wedge\,  \sigma'\!=\!\PushSF {\phi} {\sigma''}\ ]$ 
\item
\sdN{ $\leadstoBoundedThree {\Mtwo} {\sigma} {\sigma\bd}  {\sigma'}  \    \triangleq \ \    \leadstoOrig {\Mtwo} {\sigma} {\sigma'} \, \wedge\, 
 \EarlierS {\sigma\bd}  {\sigma} \, \wedge\,  \EarlierS {\sigma\bd}  {\sigma'} $}
\item
 $\leadstoBoundedStarThree  {\Mtwo}  {\sigma}  {\sigma\bd} {\sigma'} \ \ \ \triangleq \ \  \sigma=\sigma' \, \wedge\,  \EarlierS {\sigma\bd}  {\sigma} \ \wedge  \EarlierS {\sigma\bd}  {\sigma'} \ \ \vee$\\
$\strut  \hspace{3cm}\ \ \exists \sigma''.[\ \leadstoBoundedThree  {\Mtwo}  {\sigma}  {\sigma\bd} {\sigma''} \, \wedge\, 
...
We continue with  Fig. \ref{fig:UpSemantics}. \sdN{Here $\EarlierS {\sigma_8} {\sigma_9}$ and thus $\leadstoBoundedThree {\Mtwo} {\sigma_8} {\sigma_8} {\sigma_9}$.} Also,  {$\leadstoOrig {\Mtwo} {\sigma_{14}}  {\sigma_{15}}$ 
 but  \sdN{$\NotEarlierS {\sigma_{14}} {\sigma_{15}} $} and therefore  $\notLeadstoBoundedThree {\Mtwo}  {\sigma_{14}} {\sigma_{14}} {\sigma_{15}}$ and $\notLeadstoBounded  {\Mtwo}  {\sigma_{14}}   {\sigma_{15}}$
...
%  but  $\notLeadstoBoundedStarThree {\Mtwo} {\sigma_8} {\sigma_8} {\sigma_{15}}$, and  $\notLeadstoBoundedStarThree {\Mtwo}  {\sigma_8} {\sigma_{15}}$.
 ...
 \sdN{In particular, $\NotEarlierS {\sigma_{8}} {\sigma_{15}} $, and thus 
...
Lemma \ref{l:var:unaffect} says that the interpretation of a variable remains unaffected by scoped execution of statements  which do not mention that variable.
...
\right \}
 \mbox{implies} \ \ 
  \begin{cases}
     ...
     \ \strut \  \exists z.\ \sigma_3.\prg{cont}\sdN{\txtin z} \\
