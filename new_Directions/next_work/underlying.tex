...
{Method bodies are expected to store their return values in the implicitly defined variable \prg{res}.}
 {\footnote%{\prg{res} is implicit -- \sdred{like} \prg{this}. 
 \sdred{For ease of presentation, 
in} the examples in this paper \prg{void} methods omit the assignment to \prg{res}.}}
...
\sdred{ The function   $\Formals {\sigma}  {\Mtwo}$ returns the formal parameters of the method currently executing % in $\sigma$ 
-- \cf Def. \ref{def:params}}
...
%scoped execution of statements  which do not mention that variable.
 ...
 \sdred{
\begin{lemma}
\label{l:params:do:not:change} For all $\Mtwo$, $\sigma$ and $\sigma'$:
