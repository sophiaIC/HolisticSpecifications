...
\sdN{\emph{Scoped invariants}, \cf \S  \ref{sect:approach:scoped},}  ensure that if a state $\sigma$ satisfies $A$, then all future states reachable from $\sigma$—including nested method calls and returns but \forget{excluding}{\emph{stopping} before}  the return from the active call in $\sigma$—will also satisfy $A$. For example, let $\sigma$ make an external call, transitioning to $\sigma_1$,    execution of $\sigma_1$'s continuation result  to $\sigma_2$, and $\sigma_2$ return  to $\sigma'$. 
