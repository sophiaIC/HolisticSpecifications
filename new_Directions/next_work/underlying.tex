...
\item
\sdN{$\phi.\prg{local\_map}$ is the local variable map of $\phi$}, and $\sigma.\prg{cont}$ is the continuation in the top frame.
\item
...
\kjx{FWIW, I bet this does resolve to ``domination'' in a call graph
  (rather than a sequence of states)}
...
To obtain this  notion of \red{bounded} future, we define  \emph{execution} % from the viewpoint of an original state, 
\sdN{\emph{bounded by a state} $\sigma\bd$, which %  is ``bounded'' so as not to ever 
...
\kjx{I find $\leadsto$ and $\rightsquigarrow$ rather similar}
...
Bounded semantics impose restrictions on the set of future states, but only from the viewpoint of \sdN{the bounding} state. 
Lemma \ref{lemma:orig:to:bounded}  states that:  
\ (\ref{otbOne})\  Executions starting at an initial state  are bounded.
\ (\ref{otbTwo})\  \sdN{While an execution from $\sigma$ to $\sigma'$ need not be bounded ($\leadstoOrigStar {\Mtwo} {\sigma}  {\sigma'}$ does not imply $ \leadstoBoundedStar {\Mtwo} {\sigma}  {\sigma'}$), if $\sigma$ is in the future of some initial state $\sigma_{init}$, 
...
\sdN{We revisit  Fig. \ref{fig:UpSemantics}, and assume that $\sigma_6$ is an initial state.
...
\red{where}
\\
\red{$\strut\ \ \ \  \ \ \ \ \   \psi\sim\psi'$ iff $\psi=\phi_1\cdot\psi_0 \ \wedge \psi=\phi_2\cdot\psi_0\ \wedge \phi_1.\prg{local\_map}=\phi_2.\prg{local\_map}$ for some $\phi_1,\phi2,\psi_0$.}

\end{definition}

\sdN{Popping is the counterpart of pushing: $\sigma_a\in   \PushS  {\alpha} {\sigma_b}$ means that $\sigma_a$ is the result of pushing a frame onto $\sigma_b$, and also that  $\sigma_b$ is the result of popping a fame from $\sigma_a$.
 % below 
...
\end{lemma}
 ...
 {Consider Fig. \ref{fig:UpSemantics} again: $\sigma_8\!\in\!   \PushS  {\alpha} {\sigma_7}$ for some $\overline \alpha$ -- \sdN{thus $\sigma_8$ is a callee state for 
 $\sigma_7$}. Also, 
 $\sigma_{14}\!\in\!\PushS  {\alpha'} {\sigma_{15}}$ for some $\overline {\alpha'}$ -- \sdN{thus $\sigma_{15}$ is a caller state for 
 $\sigma_{14}$}.
% 
...
%  $\sigma_{10}\in\PushS  {\alpha''} {\sigma_9}$ for some $\overline \alpha''$, and  $\sigma_{12}\in\PushS  {\alpha'''} {\sigma_13}$ for some $\overline \alpha'''$, etc.
 ...
 \kjx{this magic with $\bigtriangledown$ seems a bit\ldots  magical?
   why do it this way?  and we expand on an example here?  or is this
   a standard approach somewhere?}
...
\item
\label{oneLR}
{$\sigma'\in \PushS {\alpha} {\sigma}  \ \ \wedge \ \  \LRelevant {\overline \alpha} {\sigma} \ \ \wedge\ \   \LRelevant \alpha {\sigma'} \ \ \ \Longrightarrow \ \ \  \LRelevant \alpha {\sigma}$
}
\item
\label{threeLR}
