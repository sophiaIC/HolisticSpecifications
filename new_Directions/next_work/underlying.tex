...
{For} the purposes of our work, we {also} need an {additional} notion, of    \emph{bounded} future state, which  can be reached through {any \susan{number of} steps, including method calls and returns, but \susan{stopping before returning} excluding returns from the currently executing method} -- \cf Def. \ref{def:necessity-semantics}(2). 
...
Bounded semantics impose restrictions on the set of future states, but only from the viewpoint of {the bounding} \susan{binding?} state. 
