...
\item  
 For statement $s_1$ and $s_2$, we write $s_1 \txtin   s_2$  when $s_1 \txteq  s_2$ or  $s_2 \txteq  s_1; s_3$ for some $s_3$. 
\item
...
\item
{$ \sigma\, \popSymbol \ \ \  \triangleq\    ({\overline{\phi}}, \chi)[\prg{cont}\mapsto stmt$ \ \ \  if \ \ \  $\sigma=({\overline{\phi}\cdot\phi}, \chi)$, and for some $stmt$.}
\end{itemize}
 \end{definition}
...
\item
 ...
 \sdN{\exists  \phi, \phi',\overline \phi, \overline {\phi'} [\  \sigma\!=\!({\overline \phi\cdot \phi},\_)\, \wedge\, \sigma'\!=\!(\overline \phi\cdot \phi'\cdot \overline {\phi'},\_)\ ] } $\footnote{Needless to say that   $\overline \phi$ as well as ${\overline {\phi'}}$ may be empty.}
 %\ \  \vee\  \ 
...
\item
 ...
 \sdN{\DepthFs \sigma = \DepthFs {\sigma'} \ \ \wedge \ \ \exists \va.[\ \sigma.\prg{cont}=\va\ ] } $
...
$
 \end{lemma}
  ...
  \subsection{{\sdN{Locally} Reachable  Objects}}
