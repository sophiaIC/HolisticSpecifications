...
%{For} the purposes of our work, we {also} need an {additional} notion, of    \emph{bounded} future state, which  can be reached through any  \red{number of} steps, including method calls and returns, but   {stopping before returning} %excluding returns 
...
% but  $\notLeadstoBoundedStar {\Mtwo} {\sigma_8}   {\sigma_{18}}$  -- even though $\sigma_8$ and $\sigma_{18}$ have the same depth of stack, they belong to different calls.
  ...
  \subsection{Scoped Execution, and scoped future}
\label{sect:bounded}
...
% This  is  is a perfectly natural semantics,  but
{For} the purposes of our work, we   need an {additional} notion, of    \emph{scoped} future: % state: 
the scoped future of a state consists of all states which  can be reached through any  %\red{number of} 
 steps, including method calls and returns, but   {stopping before returning} %excluding returns 
from the method executing in the scoping state}. 
Thus, the {scoped} future  of $\sigma_8$  only includes 
 % states which are being visited as a result of  executing $\sigma_8$'s top continuation}; that is, it includes the states 
 $\sigma_9$, $\sigma_{10}$, $\sigma_{11}$, $\sigma_{12}$, $\sigma_{13}$, and $\sigma_{14}$, but \emph{not} $\sigma_{15}$  -- the latter results from returning from $\sigma_{8}$'s top continuation.  
...
%
%To obtain this  notion, 
Execution % from the viewpoint of an original state, 
{\emph{scoped by a state} $\sigma\bd$, %  is ``bounded'' so as not to ever 
allows any steps, except for popping   $\sigma\bd$'s top frame:}
...
%Scoped execution impose restrictions on the set of future states, but only from the viewpoint of scoping state. 
...
the lemma does not promise that  objects locally reachable in $\sigma_{17}$ which already existed in $\sigma_{8}$, were locally accessible in $\sigma_{8}$ -- namely it could be that objects are made globally reachable upon method return, during the step from $\sigma_{14}$ to $\sigma_{15}$.}
 ...
 \subsection{\red{23 Aril - newest:} Method Calls and Returns}
...
push/pop frames  onto the stack, and thus restrict/extend the set of locally reachable objects. 
The operator   $ \PushS  {\alpha} {\sigma}$ describes the effect of pushing a stack frame,
and Lemma \ref{lemma:push:N} desibes its effect on locally reachable objects.
%
