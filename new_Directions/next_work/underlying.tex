...
\newcommand{\Pushes}[3] {\ensuremath{ #1,   \overline{#2} \pushSymbolL  #3}}
...
\item
Assertions, $A$, and simple expression, $e$, are described in Def. \ref{def:assert:syntax}. $A$'s support  the usual connectives as well as connectives about pretection. $e$'s may contain variables, addresses, field look-ups, and ghost-field look-ups, but \emph{not} method calls; the may appear in assertions, and \emph{not} in code.
\item
...
\item 
%$\Push{z}{\alpha}{s}{\sigma}$ pushes onto the stack of $\sigma$ a new frame, with the local variable mapping   
%$\overline {z \mapsto{\alpha}}$ and  continuation $s$,  and leaves the heap unmodified. It corresponds to the first step of a  method call, and is defined only if $z_0$ is \prg{this}, and   $\overline {z}$ has the same cardinality as  $\overline {\alpha}$. And
$ \Pushes  {\sigma} {\alpha} {\sigma'}$ holds if  $\sigma'$ is obtained by pushing onto the stack of $\sigma$ a new frame, with some continuations and some local variable map with range  $\overline \alpha$, and leaving the heap unmodified --
  $\sigma'$ is the next state after a method call in $\sigma$.
\item
%We overload the notation to allow for atoms, so that
 $\Pushes  {\sigma} {\va} {\sigma'}$  stands for $\Pushes  {\sigma} {\interpret \sigma \va} {\sigma'}$.
\item
$\Pop{\sigma}{\sigma'}$ holds when  $\sigma'$ is the  state which results out   out of popping the top frame on the stack of $\sigma$, and leaving heap unmodified -- $\sigma'$ is the next state after a method return $\sigma$.
\item
...
In appendix ??? we  define execution  of \Loo\footnoteSD{check whether it is Tool, or Loo, or what};
...
no globally unreachable object may become reachable.\footnoteSD{cite "only connectivity begets connectivity"}, and
