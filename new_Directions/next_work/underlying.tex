...
{
Calls , and in particular external calls, are a central consideration of our work. \sdN{In particular, every call, whether external or internal, pushes a new frame onto the stack, and restricts the set of locally reachable object to those that are reachble from the actual parameters.} We define $ \PushS  {\alpha} {\sigma}$ to describe the effect of pushing a stack frame:
%
...
\item
\label{pushTwo}
$  \sigma.\prg{cont}\txteq\alpha' \ \   \Longrightarrow \ \  
\exists stmt, \overline \alpha. \  [\ \  \sigma\!\in\! \PushS  {\alpha} {\sigma'}\  \wedge\  \sigma.\prg{cont}\txteq\alpha' \ \ \wedge   \sigma'.\prg{cont}\txteq x:=\alpha';stmt \ \ \wedge 
$\\
