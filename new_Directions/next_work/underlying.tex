...
{In our examples, we use numbers and booleans -- these can be encoded.}
 ...
 % {and fields} %with  $\re$  %% ghist expressions include fields
...
A  frame, $\phi$, consists of a local variable map and a continuation, \ie {the} %a sequence of 
...
The novel features of our assertions — protection and scoped invariants % \cf \S \ref{sect:approach:scoped} and \ref{sect:approach:scoped}
...
Method calls and returns % are  critical for our work.  They are 
