...
\label{sect:execution}
 ...
 -- {\cf Fig. \ref{f:evaluation}.} 
  ...
  \sdred{We use the functions $\class{\sigma}{x}$, $\meth{\Mtwo}{C}{m}$, $\Same {x} {y} {\sigma}{\Mtwo}$, and $\Formals {\sigma}  {\Mtwo}$,
...
\infer
    ...
    \meth{\Mtwo}{\class{(\phi_n,\chi)}{y_0}}{m} = p \ C::m(\overline{x : T}){:T}\{\ stmts\ \}\\
        	{{p=\prg{public} \ \vee \ \Same{\prg{this}} {y_0} {(\phi_n,\chi)}{\Mtwo} }} \\
	\phi_{n+1}= {  (\  \prg{this}\, \mapsto\, \interpret{\phi_n}{y_0},\overline{x\, \mapsto\, \interpret{\phi_n}{y}}; \ stmts\ ) } 
	}
...
\infer
	...
	\phi_n.\prg{cont}   \txteq  {x := y_0.m(\overline y)}; stmts \\
	\phi'_n= \phi[x \mapsto \interpret{\phi_{n+1}}{\prg{res}}][\prg{cont} \mapsto {stmts} ]
			}
...
\infer
	{
	{\sigma.\prg{cont}  \txteq  x := y.f; s} \ \ \ \ \ \ \ 
	 x \notin \Formals {\sigma_1} {\Mtwo} \\
	 \Same {\prg{this}}  {y}  {\sigma} {\Mtwo}
	 % \\
	% \sigma_2=\sigma_1[x\mapsto  \interpret{\sigma_1}{y.f} \} ][\prg{cont} \mapsto s ] 
	}
	{\exec{\Mtwo}{\sigma}{\sigma[x\mapsto  \interpret{\sigma_1}{y.f} \} ][\prg{cont} \mapsto s ] }}
	\quad(\textsc{Read})
	\and
\infer
	{
	\sigma .\prg{cont}  \txteq  x.f := y; s \ \ \ \ \ \ \ 
...
% \\	\sigma_2 = \sigma_1[\interpret{\sigma_1}{x}.f \mapsto\ \interpret{\sigma_1}{y} ][\prg{cont}\mapsto s]	
	...
	{\exec{\Mtwo}{\sigma}{\sigma[\interpret{\sigma}{x}.f \mapsto\ \interpret{\sigma_1}{y} ][\prg{cont}\mapsto s]}}
	{}
	\quad(\textsc{Write})
	\and
\infer
	...
	{\exec{\Mtwo}{\sigma}{\sigma1[x\mapsto \alpha][\alpha  \mapsto  (\ C;\  \overline{f\, \mapsto \, \nul} \  ] [\prg{cont}\mapsto s] }}
	\quad(\textsc{New})
\end{mathpar}
 }
\end{minipage}
...
% {\footnote%{\prg{res} is implicit -- \sdred{like} \prg{this}. }}
...
\sdred{Thus, when $\leadstoOrig {\Mtwo}{\sigma}   {\sigma'} $ is within the same method, we have that $\DepthSt {\sigma'}$= $\DepthSt {\sigma}$, when it is a call, we have
...
Fig. \ref{fig:illusrPreserve}  from \S \ref{s:outline} % \ref{fig:UpSemantics}
 illustrates  % such 
  ${\leadstoN}$ \sdred{executions}. We distinguish
...
\ $\sigma_9 \uparrow \sigma_{10}$ indicates that $\leadstoOrig {\Mtwo}{\sigma_9}   {\sigma_{10}} $ is a method entry, \sdred{\etc}
% \ with   $\sigma_{12} \downarrow \sigma_{13}$   the corresponding return. 
...
\label{l:params:do:not:change} 
  ...
  \sdred{For all $\Mtwo$, $\sigma$ and $\sigma'$:
 $\strut \hspace{1cm} \leadstoBoundedStar {\Mtwo}  {\sigma}  {\sigma'} \ \ \wedge\  \ \DepthFs{\sigma}=\DepthFs {\sigma'}\ \ \ \Longrightarrow\ \  \ \forall x\in \Formals {\Mtwo} {\sigma}.[ \ \interpret \sigma x = \interpret {\sigma'} x \ ]$
}
\end{lemma}
 

\sdred{Appendix \ref{app:aux} discusses further properties of scoped execution, and proofs.}
...
\sdred{
In well-formed states, $\Mtwo \models \sigma$,    the value of any parameter in any all callee   (${\RestictTo  \sigma {k}}$) is the same as the value of some variable in the caller (${\RestictTo  \sigma {k\!-\!1}}$),
...
Well--formedness is satisfied in initial states $(1)$, and is preserved by execution $(2)$.
}


\begin{definitionLemma}[Well-formed states]
\label{def:wf:state}
\sdred{ For modules $\Mtwo$, and  states $\sigma$, $\sigma'$:

$\Mtwo \models \sigma \ \ \triangleq \ \  \forall k. 1<k\leq \DepthFs {\sigma}.$\\
$\strut \hspace{2.2cm}[\ \ \forall   x \in   \Formals {\RestictTo  \sigma {k1}} {\Mtwo}.[\ \exists y. \ \interpret {\RestictTo  \sigma {k1}}  {x} = \interpret {\RestictTo  \sigma {k\!-\!1}}  {y} \ ]  \ \ \ \wedge$
\\
$\strut \hspace{2.2cm} \ \ \ \   {\LRelevantO   {\RestictTo  \sigma {k}} }\ = \ { \{ \ \alpha \ \mid \exists z\!\in\!  \Formals {\RestictTo  {\sigma} {k}} {\Mtwo} \ \wedge \ \alpha\in \Relevant { \interpret   {\RestictTo  \sigma {k}}  {z}}\sigma \ \} \ ] }$ 
...
\label{l:wf:state}
  $(1)   \  \ \DepthFs \sigma = 1 \ \      \Longrightarrow \ \  \Mtwo \models \sigma\ \ $

%\ -\!\!- 
$(2)\  \  \Mtwo \models \sigma \ \wedge \ {\exec{\Mtwo} {\sigma} {\sigma'}}  \ \    \Longrightarrow \ \ \Mtwo \models \sigma' \ $
}
\end{definitionLemma}
