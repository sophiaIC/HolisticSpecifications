...
{For} the purposes of our work, we {also} need an {additional} notion, of    \emph{bounded} future state, which  can be reached through {any steps, including method calls and returns, but excluding returns from the currently executing method} -- \cf Def. \ref{def:necessity-semantics}(2). 
...
\kjx{I find $\leadsto$ and $\rightsquigarrow$ rather similar\red{SD: good point! To replace $\leadsto$ by $\rightarrow$}}
...
Bounded semantics impose restrictions on the set of future states, but only from the viewpoint of {the bounding} state. 
...
{Popping is the counterpart of pushing: $\sigma_a\in   \PushS  {\alpha} {\sigma_b}$ means that $\sigma_a$ is the result of pushing a frame onto $\sigma_b$, and also that  $\sigma_b$ is the result of popping a fame from $\sigma_a$.
 % below 
...
(\ref{oneLR}) A locally reachable object  \sdN{right after entering a call} was also locally reachable before \sdN{entering} the call.
