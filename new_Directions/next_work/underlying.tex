...
Wlog, \sdN{to simplify some proofs} we   require that, as in Kotlin, method bodies do not assign to formal parameters, , \cf Def.  \ref{def:wf:state}.  
...
{The semantics of our assertions language % , \AssertLang, 
is based on three  {derived}, fundamental concepts on  top of  \LangOO: {method calls and returns, scoped execution}, and locally reachable objects.}
...
As discussed in \S \ref{sect:approach:scoped}, scoped invariants ensure that if a state $\sigma$ satisfies $A$, then all future states reachable from $\sigma$—including nested method calls and returns but excluding the return from the active call in $\sigma$—will also satisfy $A$. For example, let $\sigma$ make an external call, transitioning to $\sigma_1$,    execution of $\sigma_1$'s continuation results to $\sigma_2$, and $\sigma_2$ returns to $\sigma'$. Suppose the module guarantees $\TwoStatesN {\overline{x}} {A}$, and $\sigma \not\models A$, but $\sigma_1 \models A$. Scoped invariants ensure that $\sigma_2 \models A$, but \emph{not} that $\sigma' \models A$.
 
 This exclusion is necessary for protection.
%
Removing the exclusion % Guaranteeing $\sigma' \models A$ 
 (as in history invariants) would be both \emph{unenforceable} and overly \emph{restrictive}.\ \  \emph{Unenforceable}: \ Take $A \txteq \inside{a.\prg{key}}$,  assume  in $\sigma$ a path to an external object which has access to $a.\prg{key}$, assume that path is unknown in $\sigma_1$: then, the transition from $\sigma_1$ to $\sigma_2$ cannot eliminate that path—hence, $\sigma' \not\models \inside{a.\prg{key}}$.\ \  \emph{Restrictive}:\ Take $A \txteq \inside{a.\prg{key}}\wedge a.\prg{blnce}\geq b$.; then,  requiring  $A$   to hold in all states from $\sigma_1$ till termination would prevent all future withdrawals from $a$, rendering the account useless.

To formalize scoped invariants, we need a semantics that permits any steps except for returning from the currently active call.
