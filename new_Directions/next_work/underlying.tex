...
{For} the purposes of our work, we {also} need an {additional} notion, of  \emph{scoped}  future:  the scoped future of a state consists of all states which  can be reached through any  number of steps, including method calls and returns, but  {stopping before returning} %excluding returns 
from the currently executing method}  in the scoping state-- \cf Def. \ref{def:necessity-semantics}(2).  
...
Thus, the scoped future of $\sigma_8$  includes only
 ...
 {Similarly, $\sigma_{18}$ is not in the scoped future of $\sigma_8$,  even though they have the same stack depth -- between $\sigma_{8}$ 
