...
{In our examples, we use numbers and booleans -- these can be encoded.}
 ...
 Fig. \ref{f:loo-syntax} shows the syntax of \LangOO. \sdred{Statements, $stmt$, are three-address instructions, or   method calls, or empty, $\epsilon$.}  \sdred{Expressions, $\re$, may appear in assertions, but not in statements.}
 % \sue{and fields} %with  $\re$  %% ghist expressions include fields
%and may be used in writing specifications.
They   may contain fields, $\re.f$, or ghost fields, ${\re_0.gf(\overline {\re})}$, and so have no side-effects\footnote{
\sdred{For convenience,   $\re.gf$ is short for $\re.gf()$. Thus, expressions  like $x_1.f_1$ are field lookups in some modules, and   ghostfields in others. 
\Eg  $\prg{a}.\prg{\balance}$,    is a field lookup in \ModA, %while it would be 
...
A  frame, $\phi$, consists of a local variable map and a continuation, \ie \sdred{the} %a sequence of 
...
%			{\prg{ghost}\ \prg{intrnl}\ g\  (\overline{x : T})\{\ gt\ \} : T} -- DO NOT REMOVE THESE yet
		\endsyntaxline}
\endSyntaxElement\\
\syntaxElement{{\re}}{{\sdred{Expression}}}
		{
		\syntaxline
				{x}
				{v}
...
\end{syntax}
 & \hspace{1cm} & 
\begin{syntax}
\syntaxElement{\sdred{C, f, m, gf, x, y}}{ }
		{\sdred{Identifier}}
\endSyntaxElement\\
...
\caption{\LangOO Syntax. We use $x$, $y$, $z$ for variables, \ $C$, $D$ for class identifiers, $f$ for field identifier, $\sdred{gf}$ for ghost field identifiers, $m$ for method identifiers, $\alpha$ for addresses.
%, $i$ for integers.
}
\label{f:loo-syntax}
\end{figure}
...
return the class of $x$, the method $m$ for class $C$, \sue{the fields for class $C$,} whether $x$ and $y$ belong to the same module, and 
...
In general,  $\leadstoOrigStar  {\Mtwo} {\sigma}   {\sigma'}$ may involve \sue{any}  number of  calls or returns: \eg
...
is based on three concepts built on \LangOO: method calls and returns,
scoped execution, and
(in)directly accessible objects.
...
To define protection (that an object is not indirectly reachable from
another object, or from a stack frame then directly via an
external object. \S~\ref{sect:approach:protection}) we first define
