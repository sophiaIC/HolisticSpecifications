...
\label{sub:Loo} 
 \LangOO  is a {small}, imperative, sequential,  class based, typed, object-oriented language, whose fields are private to the class where they are defined, and where addresses are unforgeable, where there is no ambient authority (no static methods, no address maniputation)  -- similar to the one in OOPSLA\footnoteSD{any differences?}.
...
\item
$\alpha \in \sigma$ means that $\alpha$ is defined in the heap of $\sigma$, and $x\in \sigma$ means that $x$ is defined in the top frame of $\sigma$, and $x\in A$ means that $x$ appears free $A$. Conversely, % $\alpha$ is fresh, resp.$x$ is fresh in $\sigma$   in $\sigma$ means that 
...
In appendix TODO we  define execution  of \LangOO\footnoteSD{check whether it is Tool, or Loo, or what};
...
\red{Note that  $\leadstoOrigStar  {\_} {\_}   {\_}$ may involves any number of method calls and returns. 
For instance, in Fig. \ref{fig:UpSemantics}, we  have  $\leadstoOrigStar  {M} {\sigma_6}   {\sigma_{19}}$, which involves three method calls, and one method return; and 
we also have $\leadstoOrigStar  {M} {\sigma_10}   {\sigma_21}$, which  involves one method call and three method returns are involved. In section \ref{sect:bounded}, we will define a derived relation, called bounded execution, where the number of returns may not exceed the number of calls.}
...
\sdN{The semantics from the earlier section allows us to push and pop frames from the stack in arbitrary fashion. In particular it is possible to start with a state $\sigma$ and after a number of execution steps to reach a state $\sigma'$ with fewer frames on its stack. In some sense $\sigma'$ is not part of the future of $\sigma$, since $\sigma$'s top frame will have been popped before we reach $\sigma'$. 
}

\sdN{We therefore define another version of execution which is relative to a starting point, and call this ``bounded execution''}
...
\red{A central concept to our work is object \emph{protection}, which we will define in   Sect. \ref{sect:protect}: It requires that no external, locally 
reachable object can have unmitigated accessto that objecrt.}
...
Lemma  \ref{lemma:relevant} describes properties of global reachability.  (\ref{oneGR}) Locally reachable objects are globally reachable, and 
...
For all modules $M$, states $\sigma$, $\sigma'$, for all objects $o$, atoms ${\overline \va}:$\footnoteSD{\sdN{TODO decide whether $o$ or $\alpha$}}
...
\item
\label{twoLR}
${\leadstoBoundedStar {M}  {\sigma} {\sigma''} {\sigma'}} \ \ \wedge \ \  \GRelevant o {\sigma'} \ \ \wedge\ \  \sdN{o\in \sigma} \ \ \ \Longrightarrow \ \  \ \GRelevant o {\sigma}$.
\item
\label{threeLR}
${\leadstoBoundedStar {M}  {\sigma}  {\sigma} {\sigma'}} \ \ \wedge \ \   \LRelevant o {\sigma'}\  \ \wedge\ \  \sdN{o\in \sigma} \ \ \ \Longrightarrow \ \ \ \LRelevant o {\sigma}$.
\end{itemize}
}
\end{lemma}
