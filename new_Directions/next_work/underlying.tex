...
\susan{Having established  the difference between  scoped, history and object invariants, 
only scoped  have a semantics that permits any steps 
but stops before returning from the current active call.
In order to give semantics to scoped invariants (later, in Def.  \ref{def:necessity-semantics}), we need a new definition of execution, called scoped execution.}
