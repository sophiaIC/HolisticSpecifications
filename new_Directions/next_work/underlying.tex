...
$ \Pushes  {\sigma} {\alpha} {\sigma'}$ holds if  $\sigma'$ is obtained out of $\sigma$ by pushing onto the stack of $\sigma$ a new frame, with the local variable mapping   to $\overline \alpha$ and some continuation, and leaving the heap unmodified. When it
holds, then $\sigma'$ is the next state after a method call in $\sigma$.
%$\overline {z \mapsto{\alpha}}$ and  continuation $s$,  and leaves the heap unmodified.
% such that  $\sigma'=\Push{z}{\alpha}{s}{\sigma}$ for some $\overline z$ and some $s$.
\item
We oveload the notation to allow for atoms, so that $\Pushes  {\sigma} {\va} {\sigma'}$  stands for $\Pushes  {\sigma} {\interpret \sigma \va} {\sigma'}$.
\item
$\Pop{\sigma}{\sigma'}$ holds when  $\sigma'$ is the  state which results out   out of popping the top frame on the stack of $\sigma$, and leaving heap unmodified. When it
holds, then $\sigma'$ is the next state after a method return $\sigma$.
\item
