...
The  operaror $\RestictTo  \sigma k$, which returns a new state whose top frame is the $k$-th frame from $\sigma$. Namely, $\RestictTo {(\phi_1...\phi_i...\phi_n,\chi)} {i}\ \ \ \triangleq \ \ \ (\phi_1...\phi_i,\chi)$
\item
...
{Methods are expected to store their return values to the variable \prg{res}.}
...
$\models \sigma$, which guarantees that for all frames  on the stack, the actual parameters of a method call in a  caller frame have the same values as the formal parameters in the callee frame, \cf Def.  \ref{def:wf:state}. 
...
Lemma \ref{l:var:unaffect} says that the interpretation of a variable remains unaffected by scoped execution of statements  which do not mention that variable. Further lemmas can be found in Appendix~\ref{app:aux}.
...
An object $\alpha$ is  \emph{locally reachable}, $ \LRelevant \alpha \sigma $, if it is reachable from the top frame on the stack of $\sigma$.
 
\begin{definition} We define 
\begin{itemize}
\item
$ \LRelevant \alpha \sigma  \ \ \triangleq \ \  \  \exists n\!\in\!\mathbb{N}.\exists f_1,...f_n, x. [\ \interpret {\sigma} {x.f_1...f_n} = \alpha \ ]$
\item
\sdN{$ \LRelevantO   \sigma  \ \ \ \ \ \triangleq \ \  \  \{ \ \alpha \ \mid \ \LRelevant \alpha \sigma \ ]$}
\end{itemize}
\end{definition}
