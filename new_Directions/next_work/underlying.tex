...
\sdN{Bounded semantics impose a restriction on the set of future states, but only from the viewpoint of
a given origin state:  
Any execution starting at an initial state is bounded, - \cf Lemma \ref{lemma:orig:to:bounded}, part \ref{otbOne}.
Moroever, 
 $\leadstoOrigStar {\Mtwo} {\sigma}  {\sigma'} \ \wedge\
 \notLeadstoBoundedStar {\Mtwo} {\sigma}  {\sigma'}$ expresses that $\sigma'$ is not part of the "bounded" future of $\sigma$; but if, in addition, $\leadstoOrigStar {\Mtwo} {\sigma_{init}}  {\sigma}$ holds, then 
  both $\sigma$ and $\sigma'$ are  
part   of $\sigma_{init}$'s bounded future, and  from $\sigma_{init}$'s viewpoint, $\sigma'$ is a transitive bounded successor of $\sigma$ - \cf Lemma \ref{lemma:orig:to:bounded}, part \ref{otbTwo}.
Finally,  part \ref{otbThree}  genaralizes Lemma \ref{lemma:orig:to:bounded}, part \ref{otbTwo}.}
...
For all $\overline M$, all $n,m\in \mathbb{N}$ with $m\leq m$, $\sigma_0$, ... $\sigma_n$,  $\sigma_{init}$, $\sigma$, $\sigma'$, where
...
\item
\label{otbThree}
 $\forall i\!\in\! [0..m).\ \leadstoOrig  {\Mtwo} {\sigma_{i}}  {\sigma_{i+1}}\ \wedge\ \sigma_{init}=\sigma_0 \ \wedge \  \sigma=\sigma_m \ \wedge\ \sigma' =\sigma_n  $ \\
\strut \hspace{0.5cm} $\ \ \Longrightarrow \ \  \exists k\leq n.[\ \ \ \forall i\leq k.[\  \leadstoBoundedStar  {\Mtwo}   {\sigma_{init}} {\sigma_i}\ \wedge \ \leadstoBoundedStarThree  {\Mtwo}  {\sigma}  {\sigma_{i}} {\sigma'} ]\ \ \  \wedge$\\
\strut \hspace{3cm} $ \forall i> k. [\  \notLeadstoBoundedStarThree  {\Mtwo}  {\sigma}  {\sigma_{i}} {\sigma'}\ ] \ \ \ ]$.
\end{enumerate} 
\end{lemma}
}
