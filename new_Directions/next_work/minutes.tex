...
\item
\sl{Explain that local variables may be assigned to, but formal parameters will not. Write the corresponding lemma -- or axiom?
...
\item
\sl{Remove all internal discussions; remove colours from the rest}
...
\item
\sl{IMPORTANT: Add that assertions in the module invariants are encapsulated, and that protected may not appear in negative positions.}
...
\item
\sl{make the NOTEs more succinct}

\item
\sl{Show/Say where in our current example we are using which salient proof rules}

\item
\sl{Give a high level description of the main challenges and ideas in the proof of soundness. Gove a high level characterisation of $\_\ll\_$.}

\item
\sl{Push some of the proofs in the appendix.}
...
\item
\sl{add the complete definitions in the appendix. Esp. the oper semantics}
\item
...
\item
Complete the proof of \prg{Shop} and show that $M_{bad}$ does not satisfy the spec.  
Show/Say where in our current example we are using which salient proof rules

\item
write the specifications of the other case studies using our approach -- appendix
...
\item
Repair the text so as to reflect the fact that $S_1$ and $S_3$ are not satisfied by any module.
...
\item
\textbf{SMALL}:  add to oper semantics rules conditions so as to avoid call across modules and same for field read or write. Write out the lookup that determines  whether the currently executing  method is \prg{public} -- We have described informlly how it works
...
DONE? motivate "capabilities as guards" and then use to motivate "protected"
...
\item
Improve the explanatoins about the issues involved in pushing and popping frames or perhaps even move them to one section. 
