...
$\strut ~ ~ ~ ~ [ \ \ M,\sigma \ \models \ A \ \wedge\  
 ...
 \ \ \Longrightarrow\ \ M,\sigma'' \ \models \ \external{\prg{this}}  \rightarrow A''\ \ ]$
...
\subsection{Proving Soundness of the Hoare Triples Logic}
...
\begin{auxLemma}
\label{lemma:external_breakdown:term}[Summarised External Terminating Execution]
...
Here the same lemma, but with better notation:

\begin{definition}
For any module $M$  where $M$ is the internal module, external modules $\Mtwo$, and states $\sigma$ and $\sigma'$, we define:

$\leadstoBoundedThreeStarExt {\Mtwo\cdot M} {\sigma}  {\sigma'}  {\sigma''}$  \ \ if \ \ 
$\begin{cases}
\exists n, \sigma_1, ... \sigma_n  \mbox{s.t}\\
\strut \ \ \ \  \sigma' = \sigma_1 \ \wedge\ \ \sigma'' = \sigma_n \ \wedge \\
\strut \ \ \ \ \forall i\in [1..n).\ [ \ M, \sigma_i \models \external{\prg{this}} \ \wedge\  \leadstoBoundedThree {\Mtwo\cdot M} {\sigma_i}  {\sigma}   
{\sigma_{i+1}}  \ ] \\
\strut \ \ \ \  M, \sigma' \models  \external{\prg{this}}
...
then
\\
$\strut \ \ \ \ \ \exists n\! \in\! \mathbb{N},  \sigma_1, ... \sigma_{4n+1} \in States. $\\ % , m_1,...m_p, n_1, ... n_p \in \mathbb{N}.$\\  
$\strut \ \ \ \ \ \ \ \ \ \ [  $  \\
$\strut \ \ \ \ \ \ \ \ \ \  \ \  \sigma'=\sigma_1 \ \wedge\  \sigma''=\sigma_{4n+1} \ \ \wedge $\\
$\strut \ \ \ \ \ \ \ \ \ \  \ \  \forall i\in [0..n)[\ \ \  \leadstoBoundedThreeStarExt {\Mtwo\cdot M} {\sigma}   {\sigma_{4i+1}} {\sigma_{4i+2)}}\  \wedge$\\
$\strut \ \ \ \ \ \ \ \ \ \  \ \ \ \ \ \ \ \ \ \ \ \ \ \   \leadstoBoundedThree {\Mtwo\cdot M}  {\sigma_{4i+2}}  {\sigma}  {\sigma_{4i+3}} \  \wedge$ \\
\ 
$\strut \ \ \ \ \ \ \ \ \ \  \ \ \ \ \ \ \ \ \ \ \ \ \ \ M,\sigma_{4i+3} \models \internal {\texttt{this}} \ \wedge   \leadstoBoundedStarFin {M\cdot \Mtwo} {\sigma_{4i+3}}  {\sigma_{4i+4}} \ \wedge  $\\
$\strut \ \ \ \ \ \ \ \ \ \  \ \ \ \ \ \ \ \ \ \ \ \ \ \   \leadstoBoundedThree {\Mtwo\cdot M}  {\sigma_{4i+4}}  {\sigma}  {\sigma_{4i+5}}  \ \ \ ] $ \\
$\strut \ \ \ \ \ \ \ \ \ \ ]$
\end{auxLemma}
...
\subsection{Proving Soundness of the Hoare Quadruples Logic}
\label{sect:prove:sound}
