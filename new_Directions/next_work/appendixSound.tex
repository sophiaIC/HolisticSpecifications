% $ Do not know why this $ is needed for latex
...
\subsubsection{Preservation of assertions when pushing or popping frames}

Lemma \ref{l:push:pop} is  about preservation of assertions when pushing or popping frames. For this lemma, we define ... $\sigma \prec \alpha$ and $\sigma \prec \alpha$ to indicate that ....
...
\item
\label{pp:l:same:two}
If $\sigma'=\sigma[y \mapsto \alpha]$,   and $\sigma \prec \alpha$, then \\
%\begin{itemize}
%\item
$\strut \ \   \ \ \ \ \satDAssertFrom M  \sigma k   A \ \ \wedge\ \ M, \sigma' \models \intThis  \strut \hspace{3.6cm}  \Longrightarrow  \ \ \satDAssertFrom M  {\sigma'} k   A $
%\end{itemize}
...
\item
\label{pp:l:four}
$M, \sigma  \models A  \ \  \wedge\ \ k \leq \DepthSt{\sigma'} \ \ \wedge\ \      \sigma \prec {\overline \alpha} \ \ \wedge \ \ M, \sigma' \models \intThis $,\\
$\strut \hspace{8cm} \Longrightarrow \  
M, \sigma  \models {\PushAS   {\alpha} {A}}$.

\item
 \label{pp:l:six}
$\satDAssertFrom M  \sigma k     {A \wedge ( \PushAS   {\alpha} {A})} \ \ \wedge\ \  \sigma \prec {\overline {\alpha}} \ \ \wedge \ \ M, \sigma' \models \intThis $ \\
$\strut \hspace{8cm} \Longrightarrow \  \satDAssertFrom M  {\sigma'} k   {A \wedge( \PushAS   {\alpha} {A})}  $.
...
\begin{description}
\item[*]
For states $\sigma$, $\sigma'$ which have the same heap, if $\sigma \prec \sigma'$ and  $M, \sigma'  \models \intThis$, then $M, \sigma' \models \inside {\alpha}$  implies $M, \sigma' \models \inside {\alpha}$ 

\item[**]

For states $\sigma$, $\sigma'$ which have the same heap, if $\sigma \prec \sigma'$, then $M, \sigma' \models \inside {\alpha}$ implies $M, \sigma' \models \inside {\alpha}$
\end{description}
...
\item
We first prove (by induction on the definition of $M, \sigma \models A'$) that a modification to the \prg{cont} part of a frame does nor affect validity of $A'$. Note that $A'$ need not be $\Pos {\_}$.

\item

Then, by induction on the definition of $\Pos {\_}$, we can prove that adding a map  $[y \mapsto \alpha]$ to an $\alpha$ that is locally reachable, does not affect validiaty of $A$.


\item 
We now consider $\sigma$ and $\sigma'$, where $\sigma'$ is the result of pushing frame $\phi$ onto the stack of $\sigma$

 \begin{enumerate}
\item 
is proven by induction on the definition of $\Pos {\_}$ -- in particular, an assertion of the form $\inside \alpha$ is preserved when the set of locally reachable objects deceases.
...
If $\sigma=\sigma'  \pushSymbol \phi$ and $ \sigma' \prec \sigma$ and $M, \sigma' \models \intThis $, then
...
$M, \sigma  \models A  \ \  \wedge\ \ k \leq \DepthSt{\sigma'} \ \ \wedge\ \      \sigma \prec {\overline \alpha} $\\
$\strut \hspace{8cm} \Longrightarrow \  
M, \sigma  \models {\PushAS   {\alpha} {A}}$.
...
%\begin{lemma}
%\label{l:push:pop:applied}
%{\textbf{This will be the corollary}
%For all   variables $y$, statements $stmt$, states $\sigma$, $\sigma'$, numbers $k\in \mathbb{N}$, assertions  $A$,  frames $\overline {\phi}$, $\phi_n$, $\phi_{n+1}$ and $\phi$, heap $\chi$,  and 
%addresses $\alpha$ and $\overline \alpha$: % variables $\overline z$, $\overline u$:
%
%$\strut \\ $
%
%\noindent
...
%\item
%\label{pp:l:one}
%$\satDAssertFrom M  \sigma k   A \ \ \wedge \ \ \sigma'=\sigma  \pushSymbol \phi \ \ \wedge\ \  \sigma \prec \sigma' \ \ \wedge\ \ M, \sigma' \models \intThis $ \\
%$\strut \hspace{8cm} \Longrightarrow  \satDAssertFrom M  {\sigma'} k   A $.
%
%\item
%\label{pp:l:two}
%$\satDAssertFrom M  \sigma k   A  \ \  \wedge\ \  \sigma = ((\overline {\phi}\cdot \phi_n \cdot \phi_{n+1}),\chi)\ \ \wedge\ \ 
%\sigma' = ((\overline {\phi}\cdot \phi_n[x\mapsto \alpha][\prg{cont}\mapsto stmt],\chi)\ \ \wedge$ \\
%$ k \leq \DepthSt{\sigma'}\ \ \wedge
% {\sigma} \prec \alpha\ \ \wedge \ \  \sigma' \prec \sigma\ \ \wedge \ \  M, \sigma' \models \intThis $ \\
%$\strut \hspace{8cm} \Longrightarrow \satDAssertFrom M  {\sigma'} k   A  $.
% 
% 
%\item 
%\label{pp:l:three}
%$M, \sigma \models    {\PushAS   {\alpha} {A}} \ \ \wedge\ \  dom(\phi)=\overline {\alpha}\ \ \wedge 
%\ \ \sigma'=\sigma  \pushSymbol \phi  \ \ \wedge\ \  \sigma \prec \sigma' $ \\
%$\strut \hspace{8cm} \Longrightarrow M,  {\sigma'} \models   A $.
%
%\item
%\label{pp:l:four}
%%$\satDAssertFrom M  \sigma k   A \ \ \  \wedge\ \  \sigma = ((\overline {\phi}\cdot \phi_n \cdot \phi_{n+1}),\chi)\ \ \wedge\ \ 
%$M, \sigma  \models A  \ \ \wedge\ \  \sigma = ((\overline {\phi}\cdot \phi_n \cdot \phi_{n+1}),\chi)\ \ \wedge \sigma' = ((\overline {\phi}\cdot \phi_n[x\mapsto \alpha][\prg{cont}\mapsto stmt],\chi) \ \  \wedge$ \\
%% ΝΟΤΕ not included $ k \leq \DepthSt{\sigma'}\ \ \wedge\ \
%$ {\sigma} \prec \alpha\ \ \wedge \ \  \sigma' \prec \sigma\ \
%\ \wedge \ \  M, \sigma' \models \intThis \ \ \wedge {\overline {\alpha}} \subseteq dom (\phi_n)$ \\
%$\strut \hspace{8cm} \Longrightarrow \ \ \ \ 
%M, \sigma  \models {\PushAS   {\alpha} {A}}$.
%% the below follows, so not inserted
%%  \forall k\leq \DepthSt {\sigma}.[\ \satDAssertFrom M  \sigma k  {\PushAS   {\alpha} {A}} \ ]$
%
%
%\item
%\label{pp:l:five}
%$\satDAssertFrom M  \sigma k     {A \wedge ( \PushAS   {\alpha} {A})} \ \ \wedge\ \  dom(\phi)=\overline {\alpha}\ \ \wedge 
%\ \ \sigma'=\sigma  \pushSymbol \phi $ \\
%$\strut \hspace{8cm} \Longrightarrow  \satDAssertFrom M  {\sigma'} k   {A \wedge( \PushAS   {\alpha} {A})}  $.
%
%\item
% \label{pp:l:six}
%$\satDAssertFrom M  \sigma k     {A \wedge ( \PushAS   {\alpha} {A})} \ \ \wedge\ \  \sigma = ((\overline {\phi}\cdot \phi_n \cdot \phi_{n+1}),\chi)\ \ \wedge$\\
%$ \sigma' = ((\overline {\phi}\cdot \phi_n[x\mapsto \alpha][\prg{cont}\mapsto stmt],\chi) \ \  \wedge \ \ k \leq \DepthSt {\sigma'}\ \ \wedge$ \\
%$ {\sigma} \prec \alpha\ \ \wedge \ \  \sigma' \prec \sigma\ \
%\ \wedge \ \  M, \sigma' \models \intThis \ \ \wedge {\overline {\alpha}} \subseteq dom (\phi_n)$ \\
%$\strut \hspace{8cm} \Longrightarrow \  \satDAssertFrom M  {\sigma'} k   {A \wedge( \PushAS   {\alpha} {A})}  $.
%
...
%  ${\leadstoBoundedThreeStarExt {\Mtwo\cdot M} {\sigma\bd}  {\sigma}  {\sigma'}} \ \ \vee \ \ 
...
%\item
%$
%\leadstoBoundedExtPub {\Mtwo\cdot M}    {\sigma}  {\sigma_a} 
%\wedge\ \leadstoBounded  {\Mtwo\cdot M}    {\sigma_a}  {\sigma_b} 
%\wedge \ M, \sigma_2 \models \pubMeth \wedge \leadstoBoundedStar  {\Mtwo\cdot M}    {\sigma_2}  {\sigma'} \ ]
%$
%\\
%$\Longrightarrow $
...
\\
By (13), (14) and Lemma  \ref{l:shallow:deep} part \ref{fiveSD}\footnoteSD{TODO: check whether it needs some more, eg that in the penultimate frame in $\sigma_1'$ variable $y_0$ points to \prg{this} of the top in $\sigma$}, we obtain\\
...
We highlight the differences in \sdN{blue}.
\\
Therefore, there exist $u$, $y_o$, $C$, $\overline y$,  $A_1$ and $A_2$ such that \\
 $\SPS (5) \ \ \sigma.\prg{cont}\txteq u:=y_0.m(\overline y)$,  \ \ \ \ $\overline z = \fv(A)\setminus \{ u, y_0, ... y_n \}$
\\ 
...
$\SPS (10) \ \ (\prg{this}:D,\overline{y:D}\, \wedge\, A_{pre},\sigma_1,A_{post}, A_{mid}) \ll_{M,\Mtwo} (A,\sigma,A', A'')$. 
\\
By (1), (7) and Lemma   \sdN{\ref{lemma:push:ass:state} part \ref{lemma:push:ass:state:one}}, we obtain\\
...
$\SPS (16) \ \  \satDAssertFrom M  {\sigma'}  {\sdN{\DepthSt {\sigma'}}}  {\sdN {\PushAS y {(A_{post}[y_0/ \prg{this},u/res][\overline{\alpha/z}])}}}$.\\
 and using Lemma \sdN{\ref{l:adapt:stable} and \ref{l:shallow:deep} part \ref{fiveSD}}, and \sdN{because $\overline z \cap \overline y = \emptyset$}   we obtain \\
