...
\label{pp:l:same}
\item
If $\sigma'=\sigma[y \mapsto \alpha][\prg{cont}\mapsto stmt]$   and $\sigma \prec \alpha$, and  $M, \sigma' \models \intThis$, then 
\begin{itemize}
\item
$\satDAssertFrom M  \sigma k   A \ \ \Longrightarrow  \satDAssertFrom M  {\sigma'} k   A $
\end{itemize}
...
\item
\label{pp:l:four}
$M, \sigma  \models A  \ \ \wedge\ \      \sigma \prec {\overline \alpha} $\\
...
{\textbf{Discussion of Lemma \ref{l:push:pop}}} 
 ...
 apply these parts to internal as well external calls.
 In parts   (\ref{pp:l:two}), (\ref{pp:l:four}) and (\ref{pp:l:six})  we require that  $M, \sigma'  \models \intThis$ -- therefore these parts are applicable only if the caller is internal. This, however is not a restriction, really, because our quadruples are applicable to internal code only.
 ...
 This requirement is necessary do that ... 
