...
%Nevertheless,  
\sdN{Assertions} which do  not contain  $\inside {\_}$, called   $\Stable {\_}$, are preserved when pushing/popping  frames.  
\sdN{Less strictly}, assertions which do  not contain $\inside {\_}$ in \emph{negative} positions, called $\Pos {\_}$,  are preserved when pushing  {internal} frames provided that the range of the new frame contains locally reachable addresses -- 
...
% \vspace{.1cm}
\sdN{We work with  $Stb^+$  assertions   (the  $Stbl$ requirement is too strong). 
But  we need to address the lack of preservation of $Stb^+$ assertions  \susan{for external method calls and returns}.
...
but to be  satisfied at the called viewpoint \susan{(the callee)}. %:  right before a method call, $A$ might not be satisfied,
...
Only the first equation in  Def.  \ref{def:push}  is interesting,
  $\PushAS y {(\inside \re)}$.  
For $\re$ to be \sdN{protected at  the} called, it should be protected from all the call's arguments,
...
A caller   $\sigma$ satisfies  $\PushAS y A$, if and only if the called   ($\PushASLong {\phi} {\sigma}$) % (after pushing a frame with the values of $\overline y$) 
satisfies $A$.  
...
Moreover, $\FIXSymbolA$ turns an assertion into a stable assertion (Lemma \ref{l:push:stbl}), and \sdN{in internal states, an assertion implies its adapted version (Lemma \ref{lemma:internal:adapted}).}
...
\sdN{ In general, original versions of  assertions do  not apply adapted version, nor the other way round:}
...
\notesep   $A$ does not imply $\PushAS {y} {A}$:\  \Eg  take 
  a   $\sigma_1$ where $\interpret {\sigma_1} {\prg{this}}$ = $o_1$, and $o_1$ is \sdN{external},  and there is no other object. Then, we have
