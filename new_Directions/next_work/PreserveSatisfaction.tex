...
\end{lemma}
 ...
 As we discussed in section \ref{sec:howThird} it may happen that an assertion $A$ does not hold from the caller’s
viewpoint, but does hold from the callee’s viewpoint:  right before the method call, $A$ might   not hold,
but if  $\PushASLong  {\overline y} A$ holds (where $\overline y$ are the receiver and arguments to the call), then $A$
will hold right when the callee’s method body starts. Thus, the operator $\pushSymbolAA$
...
\sdN{The notation $\protectedFrom \re {y_0, ... y_n}$ stands for $\protectedFrom \re {y_0}\, \wedge\, ...  \protectedFrom \re {y_n}$.
...
{On the other hand, $M  \models \encaps{A}$ says that assertion $A$  is  \emph{encapsulated} by a module $M$, \ie in all possible states % which arise from execution of module $M$ with any other  module $\Mtwo$, t
