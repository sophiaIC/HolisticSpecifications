...
% For us it is relevant to discuss conditions under which  assertions preserved when pushing/popping frames, or modifying the heap.
...
Nevertheless, an assertion which does not contain  $\inside {\_}$ is stable against pushing and popping of frames. We call such assertions $\Stable {\_}$, \cf Def. \ref{def:Basic}  in Appendix \ref{app:preserve}. 
Similarly, an assertion which does not contain $\inside {\_}$ in negative positions is stable against pushing  of frames. We call such assertions $\Pos {\_}$, \cf Def. \ref{def:Pos}.
Also, an assertion which does not contain $\inside {\_}$ in positive positions is stable against pushing  of frames. We call such assertions $\Neg {\_}$, \cf Def. \ref{def:Pos}.
