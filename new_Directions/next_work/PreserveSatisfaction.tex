...
We define the operator $\pushSymbolInText$ which  translates an assertion from the viewpoint of the callee, to that of the caller.
...
Lemma \ref{lemma:push:ass:state}  states that  $\pushSymbolInText$ indeed adapts assertions from the callee to the caller.
...
\sdNew{
 $M,  \PushSLong {\phi} {\sigma}   \models  A  \wedge \extThis  \ \  \ \ \ \ \  \ \ \    \Longrightarrow  \ \ \ \ M, \sigma \models \PushAS  {y} {A}$
 }
\end{itemize}  % {enumerate}
\end{lemma}


Moreover, $\pushSymbolInText$ turns an assertion into a stable assertion (Lemma \ref{l:push:stbl}), and {in internal states, an assertion implies its adapted version (Lemma \ref{lemma:internal:adapted}).}
