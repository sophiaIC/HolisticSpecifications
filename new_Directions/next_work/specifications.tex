...
{\emph{Well-formedness}},  $\vdash S$,  is   defined by cases on $S$:\\
  $\strut \ \  \vdash {\TwoStatesN {\overline {x:C}} {A}}$\ \ \ \  if  \ \ \ \  $\fv(A)\subseteq\{  \overline x \}\ \ \wedge \ \ \ {M \vdash \encaps {\sdN{\overline {x: C}} \wedge A}} $;\\
 $\strut \ \  \vdash {\mprepostN{A}{p\ C}{m}{y}{C}{A'} {A''}}$ \ \ \ \  if  \ \ \ \  \\
  $\strut \ \  \hspace{1cm}  \exists \overline x, \overline {C'}.[   \ \  \sdN{A \txteq (\overline {x: C'} \wedge A_0)} \ \ \ \wedge$\\
 $\strut \ \  \hspace{2.1cm} \ \ \ \sdN{\fv(A_0)\subseteq \overline{x}\! \cup\! \{ \prg{this} \}\! \cup\! \overline y} \  \ \wedge \ \ \fv(A')\subseteq  \fv(A)\!\cup\! \{\prg{res}\}\ \ \wedge\ \  \fv(A'')\subseteq  \sdN{\overline{x}}\ \ \ \wedge $\\
   $\strut \ \  \hspace{2.1cm} \ \ \   \Pos A\ \wedge\ \Pos {A'}\ \wedge \ \Pos {A''}\ \wedge  \  M \vdash \encaps  {\overline {x: C'} \wedge A''}\ \ \  ]$ \\
...
%		{\strut \ \ \ \ \ \ \ \ \ S_{9}}
%		 {% a:\prg{Account}\wedge 
%		 a.\prg{\password}\neq \prg{key'} \wedge a.\prg{\balance}=b}
%				       {\prg{Account}} {\prg{transfer}} {\prg{dst}:\prg{Account},\prg{key'}:\prg{Key},\prg{amt}:\prg{int}}
%		{ a.\prg{\balance}\geq b}
%}
...
$\strut \hspace{.2cm} M,  \sigma \models  {A} % \, \wedge\,    {\GRelevant {\overline \alpha}  \sigma} 
  \  \ \Longrightarrow$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ \ \  {\leadstoBoundedStarFin { \Mtwo\madd M}{\sigma}  {\sigma'} }\ \ \Longrightarrow\ \  \sdN{ M,  \sigma' \models  {A'[\overline{\interpret \sigma z/z}] } }\ \ \ \  ] \ \ \ \wedge$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ \  \ {\leadstoBoundedStar  {\Mtwo\madd M}{\sigma}  {\sigma''} }\ \  \ \Longrightarrow\   \   M,  \sigma'' \models  {(\extThis \rightarrow A''[\sdN{\overline{\interpret \sigma w/w}}])}\ \ \  ] $\\
