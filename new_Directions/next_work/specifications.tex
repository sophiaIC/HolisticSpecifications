...
We now move to the semantics of Specifications, where   $M \vDash S$ expresses that  module  $M$   satisfies a specification  $S$.  
...
\begin{itemize}
\item
$ {\satAssertQuadruple  \Mtwo  M     {A} \sigma {A'} {A''} } \ \ \triangleq \ \ \forall \overline{\alpha}, \overline{x}, \sigma',\sigma''.[
$  \\
$\strut \hspace{.2cm} M,  \sigma \models  {A[\overline{\alpha/x}]} \ \wedge \ fv(A)=\overline x \, \wedge\,    {\GRelevant {\overline \alpha}  \sigma}  \  \ \Longrightarrow$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ \ \  {\leadstoBoundedStarFin { \Mtwo\madd M}{\sigma}  {\sigma'} }\ \ \Longrightarrow\ \   M,  \sigma' \models  {A'[\overline{\alpha/x}]}  \ \ \ \  ] \ \ \ \wedge$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ \  \ {\leadstoBoundedStar  {\Mtwo\madd M}{\sigma}  {\sigma''} }\ \  \ \Longrightarrow\   \   M,  \sigma'' \models  {(\extThis \rightarrow A''[\overline{\alpha/x}])} \ \ \  ] \ \ \ \  \ \ \ \  \ \ ]$
\end{itemize}
\end{auxDefinition}
}
...
\begin{enumerate}
 ...
 $\satisfies{M} { \mprepostN {A}{p\ D}{m}{y}{D}{A'} {A''} }\  \ \ \   \triangleq    \ \ \ \forall   \Mtwo,  \sigma.[\ $    \\
  \sdN{
$\strut \hspace{.2cm} \sigma.\prg{cont}=u:=y_0.m(\overline y)\ \ \Longrightarrow $\\
 $\strut   \ \ \ \  \  \ \ \ \ {\satAssertQuadruple  \Mtwo  M    {y_0\!:\!D,\overline{y\:\!D}\wedge A[y_0/\prg{this}]}  \sigma {A'[y_0/\prg{this},u/res]} {A''[y_0/\prg{this}]}}\ \ ]$ }
 \item
