...
\sdN{For this, we fist define what it means for a state to satisfy a triple}

\sdN{
\begin{auxDefinition}
For modules $\Mtwo$, $M$, state $\sigma$, and assertions $A$, $A'$ and  $A''$, we define:
%    $ {\satAssertQuadruple  \Mtwo  M  \sigma   {A} {A'} {A''} }$: 
\begin{itemize}
\item
$ {\satAssertQuadruple  \Mtwo  M     {A} \sigma {A'} {A''} } \ \ \triangleq \ \ $  \\
$\strut \hspace{.5cm} \forall \overline{\alpha}, \overline{x}, \sigma',\sigma''.[
\   \ fv(A)=\overline x \, \wedge\,    {\GRelevant {\overline \alpha}  \sigma}\, \wedge\,  M,  \sigma \models  {A[\overline{\alpha/x}]} \  \ \Longrightarrow$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ {\leadstoBoundedStarFin {M\madd \Mtwo}{\sigma}  {\sigma'} }\  \Longrightarrow\    M,  \sigma' \models  {A'[\overline{\alpha/x}]}\   ] \ \ \ \wedge$\\
 $\strut   \ \ \ \  \  \ \ \ \   [ \ {\leadstoBoundedStar  {M\madd \Mtwo}{\sigma}  {\sigma''} }\  \Longrightarrow\      M,  \sigma'' \models  {\extThis \rightarrow A''[\overline{\alpha/x}]} \ ] \ \ \ ]$
\end{itemize}
\end{auxDefinition}
}
...
% }
  \item
 $\satisfies{M} { \mprepostN {A}{p\ D}{m}{y}{D}{A'} {A''} }\  \ \ \   \triangleq $ \\ %\ \ \ \forall   \Mtwo,  \sigma.$    \\
 %, \sigma',  \overline{x}, \alpha_0 \mbox{ and }  \overline{\alpha}$    \\
 \sdN{
 $\strut   \ \ \ \  \  \ \ \ \   \forall   \Mtwo,  \sigma.[ \ \sigma.\prg{cont}=u:=y_0.m(\overline y)\ \ \Longrightarrow \ \  {\satAssertQuadruple  \Mtwo  M    {y_0:D,\overline{y:D}\wedge A}  \sigma {A'} {A''} }\ ]$
 }
% 
% 
%$\strut   \ \ \ \  \  \ \ \ \  [ \ \ { fv(A)=\overline x \  \ \wedge \ \ \arising{\sigma}{M\madd \Mtwo }\   \  \wedge\ \ \GRelevant {\overline \alpha}  \sigma}\ \ \ \wedge \ \  $\\
