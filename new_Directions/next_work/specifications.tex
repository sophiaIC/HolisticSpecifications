...
\end{syntax}
\]
where for any specification ${\TwoStatesQ {\overline {x:C}} {A} {A} }$ we require that   \sdN{$fv(A), fv(A')\subseteq \overline x$}.
\end{definition}
...
\label{def:arising}
For modules $M$ and $M_{ext}$ we define arising  states as follows:
...
We now move to the semantics of \SpecLang Specifications.
We now  define what it means for  a module  $M$ to satisfy specification  $S$, written as $M \vDash S$. ....
 
\begin{definition}% [Semantics of \SpecLang Specifications]

We define $\satisfies{M}{{S}}$ by cases over the \sdN{two} %three 
possible syntactic forms.
% For any assertions   $A$, $A'$: \\
...
for all $M_{ext}$, $\sigma$, $\overline{x}$, $\overline{\alpha}$ such t$\overline{x}\notin \sigma:$  \\
 & & & ${ \arising{\sigma}{M_{ext}\!\circ \! M}\   \  \wedge\ \ \GRelevant {\overline \alpha}  \sigma}\ \ \ \wedge$ \\
 & & & $ \satisfiesA {M}   {\sigma[\overline{x\mapsto \alpha}]}{(\overline {x:C}\ \wedge\  A\ \wedge \  {\external{\prg{this}}}) } $ \\
 % \ \ \wedge \satisfiesA{M}  \sigma   {\external{\prg{this}}} \ \ \wedge $ \\
 %\ \    \satisfiesA{M}   {\sigma[\overline{x\mapsto \alpha}]}{(\overline {x:C} \wedge A)} \ \ \ \wedge}$ \\ 
 & & & ${\leadstoBoundedStar {M_{ext}\!\circ \!M}{\sigma}{\sigma} {\sigma'} } \ \ \wedge\ \  M, \sigma' \models {\external{\prg{this}}}$ \\
& & & $ \Longrightarrow $ \\
& & & $ \satisfiesA{M}{\sigma'[\overline{x\mapsto \alpha}]}{A'} $
\\
\\
...
\item
\sdN{
$M \vDash A$   \ \ \ iff \ \ \  $\forall M_{ext}. \forall \sigma
[\ \    \arising{\sigma}{M_{ext}\!\circ \! M}\   \  \wedge\ \  \satisfiesA {M}   {\sigma} {\external{\prg{this}}} 
