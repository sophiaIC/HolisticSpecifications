...
Our specifications language % \SpecLang,  
 supports scoped invariants,   method specifications, and \sdN{conjunctions}. 
...
{\emph{Well-formedness},  $\vdash S$,  is   defined by cases on $S$:\\
 ...
 $\strut \ \  \vdash {\mprepostN{A}{p\ C}{m}{x}{C}{A'} {A''}}$ \ \ \ \  if  \ \ \ \  \\
 $\strut \ \  \hspace{1cm}  \fv(A')\subseteq  \fv(A)\cup \{\prg{res}, \prg{this}, \overline x\}\ \wedge\  \fv(A'')\subseteq  \fv(A)\cup \{ \prg{this}, \overline x\}\ \wedge\ $\\
  $\strut \ \  \hspace{1cm}   \Pos A\ \wedge\ \Pos {A'}\ \wedge \  M \vdash \encaps  {A'}$; \\
...
\se{I ADDED A LABEL BECAUSE THERE IS A REFERENCE. IS THIS CORRECT}
{
