...
\sdN{We demonstrate the meaning of ${\TwoStatesQ {\overline {x:C}} {A_0}{A_0}}$ in Fig. \ref{fig:illusrPreserve} where we refine the execution shown in Fig. \ref{fig:UpSemantics}, and assume that the pink states, \ie   ${\sigma_6}$-${\sigma_9}$ and $\sigma_{13}$-$\sigma_{20}$, are external, and the green states, \ie   ${\sigma_{10}}$,  ${\sigma_{11}}$,  and ${\sigma_{12}}$, are internal.}\footnote{TODO: things would probably be clearer if there was one more state between $\sigma_{15}$ and $\sigma_{16}$. To redo the diagram and text soon.}
