...
We now move to the semantics of specifications:   $M \vDash S$ expresses that  module  $M$   satisfies a specification  $S$.  
\sdN{For this, we fist define what it means for a state $\sigma$ to satisfy a triple of assertions:}\footnote{ {\red{TODO} \sdN{defne  $\fv(stmt)$ and $\fv(A)$ -- easy and boring}}}
...
\sdN{.
And $\vs(stmt)$ returns the variables which appear in $stmt$; for example $\vs(u:=y.f)$=$\{u,y\}$.}
