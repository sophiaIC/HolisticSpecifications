...
similar model of object capabilities, but without tackling external effects;}
\cite{FASE}    proposes  ``holistic specifications'' to describe a module's emergent behaviour;
\cite{OOPSLA22}  proposes
a   logic to prove that modules {\emph {which do not contain}} external calls adhere to holistic specifications.
%\citet{CassezFQ24} model external calls as an unbounded number of invocations to a module's public interface.
