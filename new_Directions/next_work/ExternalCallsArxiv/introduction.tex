...
have been proposed in
 \cite{BirkedalL:caps-mmio-conf,ddd,vmsl-pldi2023,irisWasm23},   primarily in object-based settings, and  where capabilities are statically embedded within objects. In such models, authority is conferred through the object’s interface. 
 By contrast, our work adopts a class-based approach, and models object capabilities in a manner that is, in a sense, the dual of theirs: capabilities are not intrinsic to the objects themselves but are dynamically provided by the calling context—typically through method arguments or ambient authority in the execution environment. }
 
 \sdnew{This distinction carries important implications for reasoning  about   effects. 
 In  \cite{ddd,dd,vmsl-pldi2023,irisWasm23}, one can argue that if an object is exported with an interface that omits certain capabilities,then the corresponding effects are statically ruled out. 
 However, this argument hinges on the assumption that the exporting site has precise knowledge of the object’s embedded capabilities at the time of export. Our model requires no such assumption. Instead, we reason about potential effects based on the capabilities available in the receiving context. Provided that the target context lacks access to certain capabilities, we can guarantee that the corresponding effects are unobservable—regardless of the object’s interface.
 }
...
\sdnew{Our earlier work} \sdnew{\cite{FASE,OOPSLA22}   follows a
...
presents a logic for proving that modules devoid of external calls conform to their holistic specifications.
%\citet{CassezFQ24} model external calls as an unbounded number of invocations to a module's public interface.
...
Fuller technical details can be found in the appendices \sdnew{in the extended version \cite{externalCallsFull}.}
