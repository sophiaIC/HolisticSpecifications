...
$S_{10,bad}$ is impossible to satisfy: Even if the method \prg{leak} were to pass the \prg{key} to the external object
\prg{x}, there is no way to ensure that \prg{x}, or some other external object will store the key, and thus no way to guarantee that    $\neg \inside{\prg{this}.\prg{key}}$     holds after the call.

On the other hand, $S_{11,bad}$ can only be satisfied, if \prg{leak\_preserve} makes no external call.
