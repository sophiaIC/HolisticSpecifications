...
\sdN{Interestingly, even though in earlier  works% in the space
  \cite{FASE,OOPSLA22} \sdnew{we} employed \emph{necessary} conditions for effects (\ie under-approximate pre-conditions), we can, instead, employ \emph{sufficient} conditions for the lack of effects (over-approximate postconditions).
...
%SD chopped the below as I am not sure I agree
...
\citet{dd} deploy step-indexing, Kripke worlds, and % epresenting objects as
...
%This stream of work has culminated in 
VMSL is an Iris-based separation logic for
...
%
Cerise uses Iris invariants to support proofs of programs with outgoing calls and callbacks,
on capability-safe CPUs \cite{cerise-jacm2024}.
\cite{BirkedalL:caps-mmio-conf}  \sdnew{supports several different wrappers on the same object.}
%via problem-specific proofs in Iris's logic.
\sdnew{Besides the distinction between the object-based and class-based approach}, our work differs from
\sdnew{\cite{iris-wasm-pldi2023,vmsl-pldi2023,dd,ddd,cerise-jacm2024,BirkedalL:caps-mmio-conf,RustHornBelt-pldi2022}}
%  \sdnew{these works} 
in that \sdnew{they follow a ``static'' capability model whereby authority is conferred though an object's interface, whereas we follow a ``dynamic'' model whereby authority may be stored in the callee's context.}
 
\sdnew{\citet{stack-safety-csf2023} extend memory safety} arguments to ``stack
safety'':  
ensuring that method calls and returns are well bracketed,
and preserving   the integrity of
caller and callee.  
\sdnew{\citet{schaeferCbC}    adopt}
...
assumes  instance-private and unaliased contract fields ,
and  employs a %per-contract 
``finished'' flag to manage  callbacks. % and termination.
Scilla \cite{sergey-scilla-oopsla2019},
a minimalist functional alternative to Ethereum, helps prevent common contract errors.
...
\paragraphSD{Future work}  includes 
 the application of our techniques to
 languages that rely on lexical nesting for access
control, \sdnew{\eg}   Javascript \cite{ooToSecurity},
% rather than public/private annotations,
to languages that support ownership types, \sdnew{\eg} Rust, %that can be
 leveraged for verification
\cite{leveragingRust-oopsla2019,RustHornBelt-pldi2022,verus-oopsla2023},
and to
% languages from the
% functional tradition such as OCAML, with % which are gaining imperative
\sdnew{features} such as %ownership and 
uniqueness\cite{funk-ownership-oopsla2024,ocaml-ownership-icfp2024}. 
\sdnew{These} different language paradigms may lead to a \sdnew{refined view of the meaning
of protection}. %, and faming operations.
\sdnew{We also want to investigate tighter integrations with the underlying Hoare logic and   its framing operators.}
%us to refine our ideas for eventual access, footprints and framing operators.
