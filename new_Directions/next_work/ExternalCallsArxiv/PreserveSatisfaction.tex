...
Judgment $M\ \vdash \encaps{A}$, %\sdnew
{defined in} %Def 
\aref{D.4}{\ref{d:encaps:sytactic}}, %\sdnew
{requires that} $A$ looks up the contents of
internal objects only,  %\sdnew
{that $A$} does not contain  $\inside {\_}$  in negative positions, %\sdnew
{nor does it} contain  $\protectedFrom {\_} {\_}$ %\sdnew
{in any position}.  
Lemma \ref{lem:encap-soundness} says that %encapsulation judgment  
$M\ \vdash \encaps{A}$  %\sdnew
{implies}
