...
\textit{\textbf{Third Phase:} } We prove modules' adherences to  specifications. 
For method specifications we prove that the body maps the precondition to the postcondition and preserves the method's  mid-condition. 
...
Polymorphic Calls} Our rules do  not directly address uncertainty about whether the receiver is
 % might belong to one class or another class, or even
  internal or external, and where  the choice is made at runtime. 
However, such scenaria  are supported through our rules of consequence and for case-split.
...
\\
 \vspace{.03cm}
\noindent
If  $ \prg{this.key}\!=\!\prg{key'} \wedge\ \prg{this}\!=\!\prg{a}$, then $\prg{a.key}\!=\!\prg{key'}$, while   $\protectedFrom {\prg{a.key}} {\prg{key'}}$ and rule {\sc{Prot-Neq}} from Fig. \ref{f:protection:conseq:ext} give $\prg{a.key}\!=\!\prg{key'}$. So,  by contradiction (\cf Fig. \ref{f:substructural:app}), we can prove    \textbf{(7?)}.
If  $\prg{this.key}\!\neq \!\prg{key'}$, we  obtain from the underlying Hoare logic that the value of \prg{a.key} did not change. Thus, we can apply rule {\sc{Prot\_1}}, and obtain  \textbf{(8?)}. \ \ \ More details in \S \ref{l:set:sat}.
