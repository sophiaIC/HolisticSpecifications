...
\infer
	...
	\sigma_1 = (\sdN{\overline{\phi}\cdot\phi},  \chi)\\
        \sdN{\phi}.(\prg{contn})  \txteq   x := y_0.m(\overline{y}); s \\
	...
	\phi' = \{\prg{local}:= ([\prg{this}\ \mapsto\ \interpret{\sigma_1}{y_0}]\overline{[p\ \mapsto\ \interpret{\sigma_1}{y}]}), \prg{contn}:=s'\}
	}
...
\infer
	...
	\phi'.(\prg{contn}) \txteq  \red{z; \_} \\ % )\ \textit{or}\ \phi_1.(\prg{contn}) = (\red{z})\\
	\phi.(\prg{contn})   \txteq  \red{x := y_0.m(\overline y)}; s \\
	\phi''= (\prg{local} := \phi.(\prg{local})[x \mapsto \interpret{\sigma_1}{z}],
	                \prg{contn}:= \red{s} )   \\
