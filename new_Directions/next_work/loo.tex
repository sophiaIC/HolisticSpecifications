\section{Appendix to Section \ref{s:underlying} -- The programming language \LangOO}
\label{app:loo}
...
The syntax of \LangOO is given in Fig. \ref{f:loo-syntax}\footnote{\se{Our motivating example is provided in a slightly richer language for greater readability.}}.
...
\infer
	{
	   \sdN{\phi_n}(\prg{cont})  \txteq   u := y_0.m(\overline{y}); s \\
...
\infer
	{
		\phi_{n+1}(\prg{cont}) \txteq  \red{\epsilon} \\ % )\ \textit{or}\ \phi_1(\prg{cont}) = (\red{z})\\
	\phi_n(\prg{cont})   \txteq  \red{x := y_0.m(\overline y)}; s \\
	\phi'_n= \phi[x \mapsto \interpret{\phi_{n+1}}{\prg{res}}][\prg{cont} \mapsto \red{s} ]\\ 
		}
...
%	\sigma_2 = (\chi, \phi_2 \cdot\psi) \\
	\sdN{\sigma_1(\prg{cont})  \txteq  x := y.f; s} \\
...
\infer
	{
	\sigma_1(\prg{cont})\  \txteq\  x := \red{\prg{new}\ C }; s \\
	fields(C)=\overline{f} \\
