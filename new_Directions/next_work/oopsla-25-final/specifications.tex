...
Moreover, $M_{good} \models S_2 \wedge S_3 \wedge S_4$, and  $M_{fine} \models S_2 \wedge S_3 \wedge S_4$,
 while $M_{bad} \not\models S_2$.
More examples  below and in  % can be found in 
...
\subsection{\textbf{Well-formed Specifications}} We now define what it means for a specification to be well-formed:
...
Def \ref{f:holistic-wff}'s  requirements about  free variables are relatively straightforward -- \cf  \S \aref{E.1.1}{\ref{wff:spec:free:more}}.
...
State $\sigma_a$ is making a call  transitioning to $\sigma_b$,    execution of $\sigma_b$'s continuation   eventually results in $\sigma_c$, and $\sigma_c$ returns  to $\sigma_d$. 
