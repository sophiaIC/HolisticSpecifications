...
Consider below the internal module \Mshop, % which also includes the 
%containing 
...
% \end{wrapfigure
\noindent
%WHY DOES THE BLOCK indent to the right?
The sketch below shows a possible heap snippet.

\begin{minipage}{.23\textwidth}
% \raggedleft
\includegraphics[width=30mm]{diagrams/ShopAtrimmed.png}
\end{minipage}
\hfill
...
%followed by 
and an abbreviated class name: $o_1$, $o_2$ and $o_5$ are a \prg{Shop}, an \prg{Inventory}, and an external object. 
Curved arrows
...
The critical point in our code is the external call on line 9,   where the \prg{Shop} asks the \prg{buyer} to pay the price of that item,
...
\item  The external  call on line 9 can never result in a decrease in the shop's account balance.
\end{enumerate}
...
\begin{minipage}{.95\textwidth}
\begin{flushleft}
If $A$ holds, \  and \    if no external access to  $o_c$ \ \ \ \ then\  \ \  \ $A$ holds in the future. 
\end{flushleft}
\end{minipage}
...
%\begin{flushleft}
  If $A$ holds, \  and \ if no external object  reachable from the current point of execution  has access to $o_c$,   
\   and\   if no  internal objects pass $o_c$ to external objects,  \\
  then  \\
 $A$ holds in  \emph{the future scoped by the current point of execution}.  
% \end{flushleft}
\end{minipage}
...
\subsubsection{Hoare Quadruples} Scoped invariants require quadruples, rather than  classical triples.
Specifically, \\
...
%\vspace{.1cm}
\subsubsection{Well-formed modules and External Calls} A module is well-formed, if  its specification is well-formed,   its public methods preserve   the module's scoped invariants, and  all  methods satisfy their specifications - \cf  Fig.  \ref{f:wf}.
...
{The proof}  {proceeds using}   {our Hoare quadruples logic}. % discussed in this section.
The treatment of external calls is of special interest. 
% For example,   % consider  the 
{Here,} the challenge is % how to reason  about 
the external call  {on line 9}. % (from \prg{buy} in \prg{Shop}). 
...
\\
\noindent
which says that  if the shop's account's key is protected from \prg{buyer}, then the account's balance will not decrease after the call {of
\prg{pay}.}
 \vspace{.05cm}
