...
\infer
	...
	\quad(\textsc{End})
\end{mathpar}
\caption{Selected rules for \emph{Only Through} -- the rest can be found in \jm[]{the full paper \cite{necessityFull}}} % Figure \ref{app:f:only-through-full}}
...
\infer
	...
	\quad(\textsc{If-Start})
\end{mathpar}
\caption{Selected rules for \emph{Only If} -- the rest can be found in \jm[]{the full paper \cite{necessityFull}}} %Figure \ref{app:f:only-if-full}}
\label{f:only-if}
\end{figure}
