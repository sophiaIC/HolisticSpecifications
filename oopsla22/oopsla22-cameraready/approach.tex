...
\jm[]{
A  proof sketch of the  safety provided by \SrobustB appears in 
\jm[]{the appendices %of the full paper 
\cite{necessityFull}.} %Appendix \ref{app:safety}.
}
...
\jm[]{the appendices %of the full paper 
\cite{necessityFull}.} %\S \ref{app:loo}. 
But from the external module's viewpoint, there may be more than one external module: for example, in \S \ref{sec:how}, 
module $\ModParam{1}$ is external to the module implementing class \prg{Account}, and the module of \prg{untrusted} is external to
...
\begin{figure}[t]
\includegraphics[width=0.95\textwidth]{diagrams/dependency.png}
   \caption{
