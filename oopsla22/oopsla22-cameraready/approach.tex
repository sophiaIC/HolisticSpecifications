...
{The method \prg{cautious} has as  argument an object \prg{untrusted}, of unknown provenance.
...
thus,  line 7, or   line {9} \jm[]{may decrease the balance}.
}
...
\jm[]{
A  proof sketch of the  safety provided by \SrobustB appears in \jm[]{the appendices of the full paper \cite{necessityFull}.} %Appendix \ref{app:safety}.
}
...
{In the code snippet from \S \ref{sec:how}, the call to \prg{set} on line 6 is an 
 internal call, while the call to \prg{make\_payment} is an external call -- 
 \jm[]{\prg{this} to \prg{untrusted} (both external objects).}
 %from the external 
...
for this we provide a module linking operator with  the expected semantics -- more details in Def. \ref{def:pair-reduce} and \jm[]{the appendices of the full paper \cite{necessityFull}.} %\S \ref{app:loo}. 
