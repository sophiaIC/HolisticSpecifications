...
A {summary} of this proof has already been discussed in \S \ref{s:approach}.
 ...
 with the use ghost fields
 defined through functions, and proofs of assertion encapsulation when such assertions involve reading the values of 
 several fields.}
...
internal computation is required. We use a conservative approach to an encapsulation system, detailed in \jm[]{Appendix} \ref{s:encap-proof}, 
...
{Moreover,} \textbf{\prg{balEnc}} states that since bal is an integer, {its} value is constant
...
We similarly prove that \prg{a.pwd} {is encapsulated} (\textbf{\prg{PwdEncaps}}), and
that \wrapped{\prg{a.pwd}} {is encapsulated} (\textbf{\prg{PwdInsideEncaps}}). 
That is, if only internal objects have access
to an account's \prg{pwd}, then only internal computation may grant \jm[]{access to \prg{pwd}}
...
\emph{any} single step in program execution, \jm[]{potentially dangerous} changes
