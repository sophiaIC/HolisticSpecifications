...
\sdfootnote{\sd{here the old text}
...
We explain this in detail in   \S{sub:adapt:full}, but for now, the reader may ignore the applications of that operator and
...
\end{tabular} 
%}
\sdfootnote{\sd{Would have prefered $\exists k\!\in\![1..n]$, but cannot latex it}}

\sdfootnote{\sd{the old version here:}
...
$\SRobustNextAcc$. But $\ModB$ does not satisfy $\SRobustIfAcc$: as  already discussed in \S{s:bank}, 
...
\sd{In fact, with such a semantics,  a module would satisfy $\Sadapt$ only if it did not support decrease of balance by $100$, or
is states where an account's balance is \prg{350} were unreachable!}
