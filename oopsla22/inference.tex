...
\item[(Part 3)]
Proving Per-Step \Nec specifications by combining per-method \Nec specifications (\S \ref{s:module-proof})
\item[(Part 4)]
Raising necessary conditions to construct proofs of emergent behaviour (\S \ref{s:emergent-proof})
\end{description}
...
\sdr[no breakthrough]{The second key idea in \S \ref{s:approaxh}} allows us to
...
\sdr[no breakthrough]{The third key idea in \S \ref{s:approaxh}} allows us to
...
This would not be sound because a 
...
\textsc{Trans}$_1$ and \textsc{Trans}$_2$  {are transitivity rules}
that  {state} that necessary conditions to reach intermediate states or 
...
there is only a single rule. \textsc{If-Class} \jm[]{expresses that}
