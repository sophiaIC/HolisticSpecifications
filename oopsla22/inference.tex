...
$M\ \vdash A'\ \Rightarrow\ \encaps{A}$ provided b y some external system. 
Thus, \Nec is parametric over the derivation of the encapsulation
     judgment; in fact, several ways are possible \cite{TAME2003,ownEncaps,objInvars}. In Appendix~\ref{s:encap-proof} and
...
if in all possible states which arise from module $M$ with any other module $M'$, and which satisfy $A'$, 
the validity of $A$} 
...
${\sigma' \triangleleft \sigma}$. We need that,
because we  interpret the assertion $A$ in the current state ,$\sigma$, and the assertion $\neg A$ in 
...
for illustrative purposes, we will ouitline %here how it can be derived with the use of 
...
$x.m(\overline{z})$.
In more detail, \sdNr[added details]{ the premise} of \textsc{If1-Inside} implies that  $P$ is
...
\sdN{rule is applicable only to external states semantics, which means that}
the call  $x.m(\overline{z})$ is an 
external call; this ensures soundness of the rule.
\sdN{Namely,} there are only four ways
...
neither object creation or field writes may result in an external object gaining access \sdNr[was from]{to}  an internal object.
...
must have $A$ as a necessary precondition.  \sdN{The other rule  can be found in Fig. \ref{f:app:only-if-single}.}
