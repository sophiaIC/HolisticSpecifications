...
\jm[]{there are four parts of a} proof of a \Nec specification:
\begin{description} 
\item[(Part 1)]
Proving Assertion Encapsulation (\S \ref{s:encaps-proof})
\item[(Part 2)]
...
\item[(Part 3)]
Proving Per-Step \Nec specifications by combining \jm[]{per-call} \Nec specifications (\S \ref{s:module-proof})
\item[(Part 4)]
Raising necessary conditions to construct proofs of emergent behaviour (\S \ref{s:emergent-proof})
\end{description}

\sdr[added]{We consider the first concern to be, to a certain extent, orthogonal to the main aims of this work,
and in this paper propose a simple approach based on the type system, while also acknowledging that 
better solutions are possible.
To address the next three concerns, we  came up with the key ideas outlined in  \S \ref{s:approach}, which we
...
because the invalidation of it} requires calling a method on an internal object.
%
\\
\strut \hspace{1cm}
...
\noindent 
The key consequence of soundness is that \jm[]{for an object $o$, $\wrapped{o}$ is encapsulated}. i.e. \jm[]{If an} object \jm[]{is only accessible by module internal objects,}
  \jm[]{then it will only ever subsequently be accessible externally} via an internal call.
...
are never returned from method bodies, this is even simpler than in \cite{confined} \jm[]{not sure what to do with this sentence}. 
...
That is, for some method $m$ in class $C$, we \jm[]{prove} a specification of the form:
...
%In order to do this
% Our first breakthrough observation (\S \ref{intro:this:work})
\sdr[no breakthrough]{The first key idea in \S \ref{s:approach}}
...
if by some classical logic, the execution of $x.m(\overline{z})$, with precondition $P \wedge \neg P_1$,
...
\jm[]{\textsc{If1-Inside} states that if given the precondition $\neg P$, the result of a function call
$x.m(\overline{z})$ is not $y$, then $P$ is a necessary pre-condition to invalidate $\wrapped{y}$ by calling
$x.m(\overline{z})$}.
...
\sdr[no breakthrough]{The second key idea in \S \ref{s:approach}} allows us to
\sdr[rephrase]{leverage several per-method \Nec specifications 
...
have the same condition to the \jm[]{invalidating} of that assertion, then 
% we are able to generalize} 
this condition is \sdr[]{a necessary program-wide, single-step condition}   to the invalidation of that assertion.
...
%\sophiaPonder[chopped: "The full set of proof rules for \emph{Single Step Only If} can be found in Fig. \ref{f:app:only-if-single}" as we say it earlier]{}
...
\infer
	...
	\quad(\textsc{End})
\end{mathpar}
\caption{\scd{Selected rules for} \emph{Only Through} -- the rest can be found in Figure \ref{app:f:only-through-full}}
...
\infer
	...
	\quad(\textsc{If-Start})
\end{mathpar}
\caption{\scd{Selected rules for} \emph{Only If} -- the rest can be found in Figure \ref{app:f:only-if-full}}
\label{f:only-if}
\end{figure}
...
\sdr[no breakthrough]{The third key idea in \S \ref{s:approach}} allows us to
\sdr[rephrase]{leverage one per-step \Nec specification to obtain 
multiple step \Nec specifications, as well as the combination 
of several such multiple step \Nec specifications to obtain further multiple step \Nec specifications.}
...
The \jm[I couldn't read Susan's comment on this]{full rules} can be found in Appendix \ref{a:necSpec}.
...
then there must be some specific intermediate state where that change occurred.}
 ...
 proofs of 
emergent properties.
\jm[]{Since proofs of necessary pre-conditions are concerned with change in program state,
proofs typically hinge around when certain properties of the program state change. For example,
under what conditions does our balance decrease?}
...
\jm[]{Such a rule} would not be sound because a 
